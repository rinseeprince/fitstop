...
> ⚠️ **SUPERSEDED IN PART (2026-08-11, migration 144 — Session 1B of the goals/blocks workstream).** This spec's central lifecycle design — **Mig B / the one-durable-plan in-place upsert** (its `ON CONFLICT (client_id) WHERE status='active'` shape, and every "no versioning", "single durable plan", "edited in place" claim below) — was **reversed by owner decision**: `nutrition_plans` now holds **date-ranged VERSIONS** resolved by date (`coversDate`), saved close-and-insert. What this spec shipped that still stands: events-as-SOT, per-day calendar editing (`is_modified`/`note`), event→plan FKs `SET NULL`, promotion removed. **Authoritative current model: `docs/ARCHITECTURE.md → Nutrition plan versions` + `CONVENTIONS.md §8`.** The document stays as the design-decision record of its era.
>
> **Status: ✅ IMPLEMENTED** (Sessions 1-5 of the events-SOT overhaul, migrations 113-118; 2026-06-17 → 2026-06-21). This spec shipped as designed — one durable nutrition plan + per-day `nutrition_events` calendar editing (`is_modified`/`note`), event→plan FKs `SET NULL`, promotion removed. **Authoritative shipped model: `docs/ARCHITECTURE.md → Nutrition & Training Events`** (the events-SOT execution plan was deleted after shipping; its record is in git history). Retained as the design-decision record (the §2 D1-D6 rationale); the prescriptive "Delete X / remove Y / migrate Z" language below is **historical** (that work is done). **Superseded 2026-09-10 (migrations 169–170):** the `nutrition_events` table is gone — a nutrition day is computed from the version covering it and the per-day edit is a `nutrition_day_edits` row (`docs/ARCHITECTURE.md` → "The window is the row"); every mention of the table below is historical. **§8 is moot:** roadmaps/phases were removed entirely on 2026-07-25 (tag `roadmap-v2-pre-removal`) — there is no phase transition, so the re-window it planned no longer applies. Phase/roadmap mentions below are historical design context.
...
- **Check-in:** stored adherence (`nutrition_days_on_target`, `adherence_percentage` via `getNutritionSummaryForPeriod`) was frozen-logs-only → invariant. **No longer true since 2026-08-30 (check-ins workstream C5, D5.2):** the denominator is now the WHOLE period's targets — every day's, resolved through `buildNutritionSummary` — not just the logged days'. A client who logged three of seven perfect days used to store 100%; they now store ~43%. Rows written before that date carry the old meaning and were deliberately not backfilled: reconstructing what each historical week's targets were would invent numbers, since plans get replaced and events get edited. The **AI day-by-day snapshot** (`check-in-snapshot-service` → `buildNutritionSummary` → `ai-prompt-builder`) becomes correct automatically once overrides are materialized (§3); the only residual is pre-backfill historical gaps, mitigated by dense generation + backfill.
...
2. **The cascade triggers are consolidated** — the 8 training event-write routes (`place-from-library`, `events/[eventId]` DELETE — its PATCH was removed in the 2026-08-26 dead-code sweep, no caller — `events/[eventId]/move`, `events/[eventId]/duplicate`, `[planId]/sessions/[sessionId]`, `[planId]`, and the client-level `training` DELETE) all call `cascadeNutritionAfterTrainingChange`. The consolidation this item asked for has shipped; `duplicate-week` and `regenerate-events` no longer exist as routes. What remains load-bearing is the anchor-date discipline: each route computes and threads its OWN anchor (`min(source,target)` on moves, `targetDate` on duplicate, client-local today on deletes).
