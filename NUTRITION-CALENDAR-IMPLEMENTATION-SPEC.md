...
2. **The cascade triggers are consolidated** — the 8 training event-write routes (`place-from-library`, `events/[eventId]` DELETE — its PATCH was removed in the 2026-08-26 dead-code sweep, no caller — `events/[eventId]/move`, `events/[eventId]/duplicate`, `[planId]/sessions/[sessionId]`, `[planId]/amendment`, `[planId]`, and the client-level `training` DELETE) all call `cascadeNutritionAfterTrainingChange`. The consolidation this item asked for has shipped; `duplicate-week` and `regenerate-events` no longer exist as routes. What remains load-bearing is the anchor-date discipline: each route computes and threads its OWN anchor (`min(source,target)` on moves, `targetDate` on duplicate, the rewrite floor on an amendment, client-local today on deletes).
