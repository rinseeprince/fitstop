...
> **Status:** spec only, no code written. Written to be merged with the training-plan-builder spec.
> **Owner decisions baked in as recommended defaults** (see §2) — flip any before building.
> Line numbers are omitted or marked ~approx because the in-flight training overhaul will shift them; references are by **file + symbol**.
...
2. **The 7 inline cascade-trigger loops** live in **training routes** (`place-from-library`, `events/[eventId]` PATCH/DELETE, `move`, `duplicate`, `duplicate-week`, `regenerate-events`, `sessions/[id]`, clear-plan). They each call into nutrition regeneration. If the training overhaul restructures these routes, **consolidate them onto `cascadeNutritionAfterTrainingChange`** as part of that work (it's cleanup both specs want). Preserve the anchor-date discipline (`min(source,target)` on moves; client-local today).
