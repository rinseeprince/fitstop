...
// The block delete's confirm: one CTA — a block contains its plans, so deleting
// it always takes them — the block named in the title. It takes `open` apart
// from the block it names, so a closing card keeps the name while it fades
...
describe("DeleteBlockDialog", () => {
  it("names the block, says its plans go with it, and confirms with one button", () => {
    const onConfirm = vi.fn();
    render(<DeleteBlockDialog open block={block} isDeleting={false} onCancel={vi.fn()} onConfirm={onConfirm} />);

    expect(screen.getByRole("dialog")).toBeDefined();
    expect(screen.getByRole("heading").textContent).toBe("Delete Cut 2?");
    expect(screen.getByText(/Any plans in it end, and their upcoming sessions and targets are removed\./)).toBeDefined();
    expect(screen.getAllByRole("button").map((button) => button.textContent)).toEqual(
      expect.not.arrayContaining(["Delete block and its plans"])
    );
  ...
  it("is inert mid-delete", () => {
    render(<DeleteBlockDialog open block={block} isDeleting onCancel={vi.fn()} onConfirm={vi.fn()} />);
  ...
  it("is closed whenever `open` is false, even with a block to name", () => {
    render(<DeleteBlockDialog open={false} block={block} isDeleting={false} onCancel={vi.fn()} onConfirm={vi.fn()} />);
    expect(screen.queryByRole("dialog")).toBeNull();
  });

  it("still names its block while it fades out", () => {
    holdExitAnimation();
    const props = { block, isDeleting: false, onCancel: vi.fn(), onConfirm: vi.fn() };
