...
type MeasurementLogSectionProps = ReadingActionHandlers & {
  ...
  /** Newest-first rows for the CURRENT pane, every metric of it — one per reading. */
  rows: LogRow[];
};
...
type ColumnContext = ReadingActionHandlers & { withActions: boolean };
...
function buildColumns(ctx: ColumnContext): ColumnDef<LogRow>[] {
  const columns: ColumnDef<LogRow>[] = [
    {
      key: "date",
      label: "Date",
      render: (_v, row) => (
        <span className={cn(MONO, "tabular-nums text-[#93b0b4]")}>{formatLogDate(row.date)}</span>
      ),
    },
    {
      key: "day",
      label: "Day",
      render: (_v, row) => <span className="text-[#93b0b4]">{formatDayName(row.date)}</span>,
    },
    {
      key: "value",
      label: "Value",
...
// A removed reading is muted — it is in no figure and can be restored.
