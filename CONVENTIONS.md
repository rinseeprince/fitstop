  ...
  **In every plan, the data model is a decision, not a detail.** State the proposed tables,
  ...
  - **No policy grants a write to `anon`, `authenticated` or PUBLIC.** A write goes through a route and `supabaseAdmin`. The one exception is activation's `clients` UPDATE (`app/api/clients/[id]/activate/route.ts`), under the coach's session through "Coaches can update their own clients"; `lib/session-client-ownership.test.ts` holds the code to it.
