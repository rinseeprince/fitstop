  ...
  **Reporting — separate what you verified from what you inferred.** Reading code is not measuring.
  ...
  - **`program-builder/`** — the real authoring surface: the weeks × Day-1-7 grid, session editor, library panel, assistant dock, and `ProgramDraftProvider`. Mounted by `/dashboard/programs/[savedPlanId]`, remounted inside the client Training drawer via `target="client-draft"`, and mounted over a client's plan as it is on the calendar via `target="placed-plan"` (the plan editor — its one date rule in `program-builder-lock-model.ts`, saved through the plan editor's PUT). All new training authoring goes here.
  ...
  **Denormalisation is allowed only when named and documented.** Submit-time snapshot columns
  (`check_ins.workouts_completed` — every workout the client logged over the period's workouts, written
  once by `submitCheckIn` through `summariseTraining` and never updated after), the per-event
  ...
  **In every plan, the data model is a decision, not a detail.** State the proposed tables,
  ...
  - RLS is enabled on **every** table in `public` (verified against the live catalog by `npm run check:rls`). For the app — every query is `supabaseAdmin`'s — it is a safety net, because service_role bypasses it; for anyone hitting PostgREST directly with the browser-shipped anon key **it is the only perimeter**.
  - Do NOT write new app-code that relies on RLS to enforce access. If the route layer is broken, RLS under service_role does nothing (service_role bypasses RLS entirely — and every query the app makes is service_role's).
  - **No policy grants a write to `anon`, `authenticated` or PUBLIC.** A write goes through a route and `supabaseAdmin`, and nothing writes through a session client; `lib/session-client-ownership.test.ts` holds the code to it. The one write rule left in the database, "Coaches can update their own clients" on `clients`, has no user.
  ...
  - **A new table's privileges are set explicitly, in the migration — a REVOKE, then exactly the grants it needs.** Supabase's stock default privileges hand ALL on a new table to `anon`, `authenticated` and `service_role` the moment it exists, on both projects, whatever "Automatically expose new tables" says in Settings → API (probed on `client_phases` and `check_in_forms`, 2026-09-02; on 2026-09-14 every table created without a REVOKE still held the grant, on both projects). Left to the defaults, a table is reachable through PostgREST with the browser-shipped anon key and RLS as its only perimeter — silently, never as an error. So revoke first, then grant `service_role` what the service layer uses (ALL for an ordinary table; `SELECT, INSERT` for an append-only one; `SELECT` alone when every write is a database function, as the goal tables are — migration 193), and `anon`/`authenticated` nothing unless a specific non-service_role caller provably needs the table, scoped by a policy. `158_client_measurements.sql` is the shape, its one client-scoped policy beside the grant.
    ...
    ```
    Put it in the migration, never in the dashboard — a grant made in Studio is invisible to source and drifts, which is how migration 125's `DROP POLICY` became a silent no-op. **Do not retrofit the tables that carry policies one by one.** The 26 tables created before this rule that carry policies keep the stock `GRANT ALL … TO anon, authenticated`. No app query uses those grants — every one is `supabaseAdmin`'s — so they come out together with every policy, in one migration (docs/DATA-ACCESS-LOCKDOWN-PLAN.md commit 6), not table by table. A table with no policy is different: through the API only the service role can read or write it, so an `anon` or `authenticated` grant on it buys nothing and leaves RLS as its only perimeter, which `check:rls` cannot fully judge (§2 review, item 5).
  ...
  - **Status-based lifecycle**: Entities with richer states use a status column instead of is_active. The lifecycle is **not uniform across entities** — match the one already in place:
    ...
    - **Goals** are **one row per goal with a start day and no end** (migration 193): a goal runs until the next goal starts, so goals can neither overlap nor leave a gap, and a row dated ahead is a planned goal, which no "goal now" read sees before its day. There is no status. Which goal — and which deadline — a day has is decided once, by `goalOnDay` / `deadlineOnDay` (`lib/goals/goal-timeline.ts`); the goal now is always asked with the client's today from `getClientTodayString`, so a planned goal takes over at the client's midnight on every surface at once. A goal that has started never changes its type, targets or start day — changing them is a new goal — while its deadline changes are rows recorded against it (`client_goal_deadlines`). Every write is one of five database functions and the app holds `SELECT` alone; delete is the hard-delete exception above. Full model: `docs/ARCHITECTURE.md` → "client_goals table".
  ...
  **Two things an events-SOT edit must not break:**
  - **Adherence math is its own decision.** A training count has ONE source, `summariseTraining` (`lib/training-adherence.ts`), over rows carrying a workout's status and the quality on its log. Every done-count in the product reads its `completed` — full **and** partial, because a partly completed workout was still done — and `full` / `partial` are the breakdown printed beside the number, never a second count. `lib/training-adherence-ownership.test.ts` fails on a coach surface that reads the stored `check_ins.workouts_completed` instead: that column is frozen when the client sends the check-in, so beside a live figure it is a second answer to the same question. Do not change adherence math under the guise of an events-SOT edit.
  ...
  - **Every exercise names its columns.** `prescribed_fields` is `NOT NULL` with no default and required by every write schema: a writer that forgets the list is refused, never silently given the strength columns. The nineteen names live once in `PRESCRIBED_FIELDS` (`utils/prescribed-fields.ts`), mirrored by the CHECK and held together by a test that reads the migration; a new exercise starts on its catalog type's preset (`presetColumnsForType`; every catalog exercise has a type, `EXERCISE_TYPES` in `utils/exercise-types.ts`, mirrored by migration 185's CHECK and never copied off the catalog row). Every exercise added to a session is a catalog exercise with a type: the New exercise form is the only way one is made, and a save never adds to the catalog. The seven column presets a coach applies to an exercise or to a whole group are one table, `COLUMN_PRESET_FIELDS` (`utils/column-presets.ts`), read by the selector, the group edit and the assistant alike — never a second list. A path that saves or copies exercises carries the list and every target key, proved in `services/exercise-groups-survival.test.ts` THROUGH the path's route schema — a validator strips an untaught key without an error, so a survival test that bypasses the validator proves nothing.
  - **Read through `expandSetSpecs`, not the columns.** It returns authored specs when present and otherwise synthesizes N `working` specs from the compact columns, so every prescription yields per-set rows carrying a `set_type`. A reader that ignores `set_specs` sees a truthful but lossy summary — it loses warm-ups, drop and failure sets, per-set loads and per-set rest.
  ...
  - **Set type is coach-prescribed, never client-chosen.** `set_logs.set_type` is seeded from the prescription snapshot; the log schema accepts-but-ignores any client value. **The four set types — `warmup`, `working`, `drop`, `failure` — are one list**, `SET_TYPES` (`utils/exercise-set-specs.ts`), which the zod schemas, the assistant's tool and the builder's Type menu derive from and migration 187's CHECK on `set_logs` mirrors (`utils/exercise-set-specs.test.ts` reads the migration); there is no AMRAP set type — AMRAP is a group format, and a set taken to failure is `failure` (owner, 2026-09-19). Analytics exclude `warmup` from every performance metric; the assistant's uniform load (`update_exercise`) moves `working`-type sets only, and its program state counts and prints those alone (`services/assistant/draft-exercise-summary.ts`). **These two filters are deliberately different — don't unify them.**
  - **A logged set's actuals are real columns, from one table** (migration 184). `set_logs` carries every measure a coach can prescribe — `reps`, `weight`, `rpe`, `rir`, `tempo`, `distance_meters`, `duration_seconds`, `pace_seconds_per_km`, `split_seconds_per_500m`, `calories`, `cadence`, `stroke_rate`, `resistance`, `heart_rate_zone`, `heart_rate`, `power`, `ftp_percent`, `rest_seconds` — nullable, each CHECKed to its target's limit and scaled to its resolution. `SET_LOG_MEASURES` (`utils/set-log-measures.ts`) is the one table: the migration test reads the migration against it, and the wire schema, the log writer, the row mapper, the client's boxes, the coach's logged-workout table and the check-in AI's exercise lines derive from it, so a measure is added there and in the next migration, never in a second list. No JSON bag and no key-value child table: the actuals are typed and charted — `get_exercise_progression_window` (migration 188) returns every one of them and the chart markers and the Sessions table's figures (`utils/exercise-progress-markers.ts`, `utils/exercise-session-figures.ts`, both over the one kernel `utils/exercise-session-markers.ts`) are computed off them at read — and a set of seventeen columns is a closed set.
  - **A timed group's score is a row on the log** (migration 186): `session_log_group_scores`, one per scored group, keyed to the log (cascade) and the client group (SET NULL), carrying the group's snapshot and the score in one of two shapes — rounds and reps, or a finish time — that `utils/group-scores.ts` defines and the migration's CHECKs mirror (its test reads the file). Only an AMRAP or a For time scores; an EMOM logs its rows. The log POST treats both its lists the same way: a list present replaces what the log holds (an empty one clears it), an absent key leaves it alone; a score is judged against the group's format before anything is written. Never a JSONB bag on `session_logs` and never a column on `exercise_logs` — a score has identity per group and belongs to the group, not an exercise.
  - **Every exercise sits in a group (migration 178).** A session is an ordered list of groups, a group an ordered list of exercises; a lone exercise is a straight-sets group of one. Every clone/serialize/placement path carries the groups exactly — each group's format, settings and place, each exercise's place in its group — through the one row builder per tier (`services/coach-library-helpers.ts`, `services/training-group-writes.ts`); a path that rebuilt groups by default would silently erase a coach's supersets. A new path that saves or copies exercises joins `services/exercise-groups-survival.test.ts`. **Group edits go through one pure module**, `components/clients/training/program-builder/program-builder-groups.ts`, shared by the builder's mutators and the assistant's ops so the two cannot drift, and four rules hold after every edit: a straight-sets group of one is a plain exercise with nothing set, while a timed group (AMRAP, EMOM, For time) of one keeps its format; where rounds are a setting — a superset or circuit, an EMOM, a For time — every exercise has exactly one row per round, so changing how many sets one has is a change to the group's rounds, never to that exercise alone; in an AMRAP every exercise has one row, the work of a round; and a group stores no setting its format doesn't use — the settings each format uses and needs are one table, `GROUP_FORMAT_SETTINGS` (`utils/exercise-groups.ts`), and `groupRuleIssue` beside it is the one statement of the rules, which every write schema refuses a group for breaking. Never spell a format's settings in a second place. Full model: `docs/ARCHITECTURE.md` → "Groups".
  ...
  - **A day can hold several sessions** (migrations 179, 180), in order, each its own workout — opened, logged and counted on its own — on the calendar and in a program alike. On the calendar the order is `training_events.day_order`: every read that lists a day orders it `(day_order, id)`, and every writer that puts a session on a day gives it a place — a move joins its new day LAST (`move_training_events_atomic` for the client's week and the coach's drag, `move_training_plan_atomic` for a start-date move, `placeSessionOnCalendar` for a library drop), several arriving in one write land in the order the write gives them, placement lays each program day's sessions in the day's order, and Edit plan's save (`edit_training_plan_atomic`) writes each day's list. In a program the order is `coach_saved_sessions.day_order` / `training_sessions.day_order`: every writer of a program's rows writes each row's place, and the save schemas refuse two sessions at one place. In the builder a session added to or moved onto a day joins it LAST and its place changes only by reordering the day (`program-builder-model.ts`, shared by the coach's gestures and the assistant's ops); a day holds at most `MAX_SESSIONS_PER_DAY`. **Nothing refuses a day for holding a session** — do not add an occupancy check, a "that day already has a session" sentence, a swap rule or a unique index on `(client_id, date)`; a swap is two moves. Sessions per week count sessions and can exceed seven: never clamp `frequency_per_week`. A view that shows days shows every session on them: a per-day list shows one row per workout, and a reader that needs one answer for a day (a plan stamp) takes the day's first session by that order, named as such. A day's training calories add every session's surplus (`sumSurplusPercentages`). Both move functions park the moving rows before placing them, because `(client_id, training_session_id, date)` is still unique. Full model: `docs/ARCHITECTURE.md` → "Several sessions a day".
  - **A logged day's prescription is frozen.** `assertSessionUnlogged` (`services/training-session-lock.ts`) refuses a session-editing write when ANY linked `training_event` has left `status = 'scheduled'` — that is, when it reads `completed`, the only other word the column holds (migration 182) — the one predicate for it: do not invent a second. It lives INSIDE `replaceSessionFull` so a future caller inherits it, runs AFTER it proves the session belongs to the client so a foreign id still 404s, and the tray's save route translates it to a 409 carrying its message. Rewriting the exercise rows under a logged day orphans the client's `exercise_logs`, and since `completion_quality` became server-derived it records a full workout as `partial`. Any new path that rewrites a placed session's exercises calls it.
  ...
  **Reading and writing a box.** Every conversion and every entry rule lives in
  ...
  The builder's set boxes speak the same grammar through the same two functions, and a
  range of them — "400-800 m", "3:45-3:50 /km", "Z2-Z3" — through `parseEntryRange` /
  `formatEntryRange` (`utils/target-range.ts`), each end an entry, a unit written once on the
  right end reading for both.

  **Comparing a value with its target** happens at the precision both are SHOWN, in the
  viewer's units (`utils/target-gap.ts`): each side is formatted as its readout and read
  back, a load through `formatLoad`'s snap, so a value and a target that read the same are
  never outside each other — an imperial client who types the "220 lbs" their hint showed
  has lifted 99.79 kg against a 100 kg target, and 5,004 m reads "5 km". A surface that
  judges a logged value against a prescription asks that module; never compare the stored
  numbers directly.
