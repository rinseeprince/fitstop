  ...
  **In every plan, the data model is a decision, not a detail.** State the proposed tables,
  ...
  - **When adding a new table: `ALTER TABLE … ENABLE ROW LEVEL SECURITY`, write NO policies, and set its privileges explicitly — a REVOKE, then exactly the grants the service layer needs.** Since migration 201 a new table arrives closed to `anon` and `authenticated` (their default privileges are revoked), and the REVOKE stays in every migration as the belt: it makes the table's privileges explicit in source whatever the defaults say, and it is where a narrower grant is written — ALL for an ordinary table; `SELECT, INSERT` for an append-only one; `SELECT` alone when every write is a database function, as the goal tables are (migration 193).
    ```sql
