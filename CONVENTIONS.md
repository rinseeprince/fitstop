  ...
  **Known gap:** this is a rule for new and touched code, not a claim about the
  ...
  - **Status-based lifecycle**: Entities with richer states use a status column instead of is_active. The lifecycle is **not uniform across entities** — match the one already in place:
    ...
    - **Nutrition plans** are **date-ranged VERSIONS** (migration 144): N rows per client whose `[effective_from, effective_until]` windows tile the timeline; `effective_until IS NULL` marks the open (latest-saved) version, guarded by `idx_nutrition_plans_open_unique`; saving closes the open row at `new_start − 1` and inserts the new version (`create_nutrition_plan_atomic`, close-and-insert). "Active" resolves **BY DATE** (`coversDate` — the same window predicate as training), never by picking the newest `status='active'` row. `status` records coach acts only (`active` | `archived` on legacy rows); superseded versions stay `active` with closed windows; there is **no `planned` status** — planned/current/ended derive from dates. Delete closes the covering version at the client's today (status untouched) and removes never-effective queued versions. *(This REVERSES the previously recorded "one durable plan, edited in place, no versioning" rule — owner decision 2026-08-11, stale premise: once migration 143 let `effective_from` sit in the future, a single row could only describe the NEXT prescription and nothing described the time before it.)* What survives the reversal: events remain the per-date SOT, per-day coach edits still materialize onto `nutrition_events` (`is_modified`) and never mint versions, and one target per day stays correct (`nutrition_events UNIQUE(client_id, date)`).
