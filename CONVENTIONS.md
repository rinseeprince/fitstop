  ...
  **Two things an events-SOT edit must not break:**
  ...
  - **A day can hold several sessions** (migration 179), in order, each its own workout — opened, logged and counted on its own. The order is `training_events.day_order`: every read that lists a day orders it `(day_order, id)`, and every writer that puts a session on a day gives it a place — a move joins its new day LAST (`move_training_events_atomic` for the client's week and the coach's drag, `move_training_plan_atomic` for a start-date move, `placeSessionOnCalendar` for a library drop), several arriving in one write land in the order the write gives them, and Edit plan's save (`edit_training_plan_atomic`) writes each day's list. **Nothing refuses a day for holding a session** — do not add an occupancy check, a "that day already has a session" sentence or a unique index on `(client_id, date)`; a swap is two moves. A view that shows days shows every session on them: a per-day list shows one row per workout, and a reader that needs one answer for a day (a plan stamp) takes the day's first session by that order, named as such. A day's training calories add every session's surplus (`sumSurplusPercentages`). Both move functions park the moving rows before placing them, because `(client_id, training_session_id, date)` is still unique. Full model: `docs/ARCHITECTURE.md` → "Several sessions a day".
