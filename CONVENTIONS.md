  ...
  **In every plan, the data model is a decision, not a detail.** State the proposed tables,
  ...
  - **Status-based lifecycle**: Entities with richer states use a status column instead of is_active. The lifecycle is **not uniform across entities** — match the one already in place:
    ...
    - **Goals** are **one row per goal with a start day and no end** (migration 193): a goal runs until the next goal starts, so goals can neither overlap nor leave a gap, and a row dated ahead is a planned goal, invisible to every reader until its day. There is no status. Which goal — and which deadline — a day has is decided once, by `goalOnDay` / `deadlineOnDay` (`lib/goals/goal-timeline.ts`), always asked with the client's today from `getClientTodayString`, so a planned goal takes over at the client's midnight on every surface at once. A goal that has started never changes its type, targets or start day — changing them is a new goal — while its deadline changes are rows recorded against it (`client_goal_deadlines`). Every write is one of six database functions and the app holds `SELECT` alone; delete is the hard-delete exception below. Full model: `docs/ARCHITECTURE.md` → "client_goals table".
