...
**Objective**: The `clients.weight_unit` column already exists (migration 009, `'lbs' | 'kg'`, default `'lbs'`). Verify types plus services expose it correctly and surface it wherever weight inputs need a default. **⊘ That column was DROPPED by migration 141** — storage is canonical kg/cm and the unit belongs to the viewer, so nothing surfaces it any more. Add the client timezone column that Session 0.1 confirmed is missing.
...
- **Resolve the `unit_preference` vs `weight_unit` overlap** — **✅ ANSWERED 2026-08-07.** Neither. `weight_unit` is gone (migration 141) and `unit_preference` is not a storage tag at all: it decides only what a VIEWER is shown, and coaches have their own alongside clients'. Deriving one from the other was in fact the platform's worst data bug — it flipped the tag while converting zero stored numbers, so a 180 lbs client picking Metric became a 180 kg client. See `CONVENTIONS.md` §20.
...
- Added `evaluatePartialTrainingPattern` — works for any program shape, event-count-based (3+ partials in last 9 resolved events).
...
**Post-Session-2.9 amendment**: The session-sequence chip row originally rendered on the card (e.g. "Push · Pull · Rest · Legs · Rest") was removed during Session 2.9 so the training card collapses to a single row (plan name + session count + chevron) and matches the new nutrition card's height. The chips never moved anywhere else — clients see the full program on the drill-in page (`/client/program/training`), which is unchanged. Card test dropped the two chip-related assertions; the session-count assertion stays.
...
**Objective**: Surface the client's active training plan structure on `/client/program` as a read-only card below the phase list. The card shows the session sequence as horizontal chips (e.g. "Push · Pull · Rest · Legs · Rest"). Tapping the card navigates to a new sub-page `/client/program/training` that lists each session in program order, each row expandable to reveal the prescribed exercises (sets / reps / RPE / tempo / rest).
...
- ARCHITECTURE.md, sections "Coach Library" (program_duration_weeks, order_index) and "Roadmap/Phase Architecture" (how training_plans link via phase_id).
...
- New type `types/client-training-plan.ts` defining `ClientTrainingPlan` (planId, planName, sessions[]; each session: id, name, focus, orderIndex, isRest, estimatedDurationMinutes, exercises[]; each exercise: id, name, orderIndex, sets, repsMin/repsMax/repsTarget, rpeTarget, tempo, restSeconds, isWarmup, supersetGroup).
...
- New component `components/client-portal/program/training-plan-card.tsx`: renders on `/client/program` below the phase list. Header line: plan name + session count. Body: horizontal chip row of the session sequence (rest slots render with a different chip style). Wrap the whole card in a `<Link href="/client/program/training">`.
...
- `components/client-portal/program/training-plan-card.test.tsx`: renders the session-sequence chips in order; rest chips render with the "Rest" label even when the underlying session has a placeholder name.
...
- **Two known trade-offs accepted, logged as TECHNICAL-DEBT.** (1) `getClientNutritionTargets:77` calls `promoteNutritionPlanIfReady`, which means our GET inherits a potential write (lazy plan promotion). For our use case this is actually desirable — without it, a client whose planned plan flipped active today would see the old plan on the card until they hit some other nutrition endpoint. **⊘ Superseded by events-SOT (Session 3):** `promoteNutritionPlanIfReady` was deleted — there is now **one durable nutrition plan per client** (no planned→active flip), so this GET no longer inherits a promotion write; `getClientNutritionTargets` was repointed (S5◆1) to read the durable plan + dense `nutrition_events`. *(⊘ Superseded again 2026-08-11: migration 144 replaced the durable singleton with date-ranged VERSIONS; the read now resolves the version covering the client's today. No promotion returned — versions resolve by date. ⊘ And again 2026-09-10: the day table is gone — migrations 169–170 — and the week's days are computed from the covering version.)* (2) The reused service uses the legacy session-scoped `createPortalClient` rather than `supabaseAdmin` per CONVENTIONS §8. Functionally identical for our access pattern (both `.eq("client_id", clientId)` with a `requireClientAuth`-verified id). Both are tracked under TECHNICAL-DEBT.md → Auth Architecture Hygiene H2 as a single "pure-composer refactor" item; the right structural fix factors `composeNutritionTargets(plan, ...)` out of the legacy service so callers can pick `promote+fetch+compose` vs `fetch+compose` (admin-scoped). Out of scope for 2.9.
...
> **⊘ Superseded by events-SOT (Sessions 4-5).** The "rest-day vs training-day" 2-block model below was already replaced mid-session by a 7-day view (see the 2.10 deviation note above), and the events-SOT overhaul completed the shift: the client now reads a **per-date `dailyTargets` window** built from computed nutrition days (`getClientNutritionTargets`; the day table went in migration 170 — a day is computed from the version covering it) — coach per-day edits (`isModified`) + notes show through, the `clients.surplus_as_carbs` split is honored, and event→plan FKs are SET NULL. Phases auto-activate by date (no promotion). The Implement/Test steps below are historical. Current shape: `CLIENT-APP-REFERENCE.md → Nutrition`; full model: `docs/ARCHITECTURE.md → Nutrition & Training Events`.
...
**Plan (report before implementing)**:
- How to compute a representative "training day" target without picking a specific date. Options: (a) use the active phase's nutrition plan baseline plus the highest `calorie_surplus_percentage` across its linked training sessions, (b) use the plan's `default_surplus_percentage`, (c) compute an average across the program. Pick one and document the choice in the prompt response before coding. Recommendation: use the highest non-null `calorie_surplus_percentage` from any training session linked to the active phase's training plan, since that represents the upper-bound target the client will see during the program. If no training plan or no surplus values exist, omit the training-day block from the card.
...
**Do NOT**: Touch roadmap-completion status, goal-outcome columns, or any client-portal card (7.9/7.10). Build the `asOf` historical resolver (deferred). Canonicalize unit storage broadly (Phase 8). — ⊘ moot: canonicalization SHIPPED 2026-08-07 (migrations 140 + 141); Phase 8 was superseded. Re-resolve effective goal for historical check-ins. Let the long-term goal drive nutrition while a phase is active.
...
**Plan (report before implementing)**:
- Migration: `coaches.timezone TEXT NOT NULL DEFAULT 'UTC' CHECK(...)` mirroring mig 089. Next free is **109** as of 2026-06-10 (105–108 = the security pass); always re-verify with `ls supabase/migrations`.
...
**Status**: COMPLETE (commit `567e0b2`; migration `110_plan_rpcs_client_local_today.sql` — explicit-signature DROP of all 7 legacy overloads + CREATE + re-applied 106 lockdown; `p_effective_from` default also moved to NULL→`v_today` (same bug, no-date path); applying it required upgrading the supabase CLI from v2.45.5, whose statement splitter could not parse the file. Review pass additionally anchored every delete/regen pair to one explicit date — a dateless pair would split across the UTC/client-local boundary and duplicate the client's local-today event — and pulled the plan-archive DELETE route + skew route forward from 7.84's list, both being placement writes outside the RPC.) **Follow-up (2026-06-12):** `getClientTodayString` had silently resolved UTC for EVERY client since this session shipped — its bare `coaches(...)` embed became ambiguous once `coach_client_views` (mig 090) added a second clients↔coaches relationship, PostgREST rejected the query (PGRST201), and the swallowed error fell through to the UTC fallback. Mocked unit tests couldn't see it; found via a live cross-tz placement repro. Fixed by naming the FK (`coaches!clients_coach_id_fkey`), logging the error loudly, and pinning the select string in the test — all clientId-only "today" sites (placement guard, `p_today`, promotion, streaks, check-in windows) healed at once.
...
- `services/training-event-calendar-service.ts` (move/duplicate "past date" guards + cascade anchor — multiple sites).
...
- Migration number: next free after 7.81's coach-tz migration — **110** as of 2026-06-10 (105–108 = the security pass, 7.81 = 109); always re-verify with `ls supabase/migrations`.
...
4. `training-event-calendar-service.ts` move/duplicate past-date guards + cascade anchor → client-tz today.
...
- `services/training-service.ts` `promoteTrainingPlanIfReady`; `services/nutrition-plan-service.ts` `promoteNutritionPlanIfReady`. *(⊘ Both functions were later **deleted** in events-SOT S2-3 — training plans coexist as date-driven provenance and nutrition is one durable plan, so neither promotes. *(⊘ Nutrition superseded again 2026-08-11: migration 144 versions are likewise date-resolved — still no promotion.)* The surviving date-driven flip is **phase auto-activation**, `promotePhaseIfReady`, also client-tz.)*
...
**Status**: COMPLETE (commit `2e227b3`, with an approved extension: ALL same-class sites found by audit, not just the named list. Notable deltas: `getCoachingWeekSummaryLive` was RECLASSIFIED to client-tz — its only live consumer is the client portal's weekly-nutrition card, not a coach view; the attention-feed's day-deciding triggers (training misses, habit dropoff, activity mismatch, no-engagement) receive the coach-local window anchor instead of defaulting to the server clock; `transitionPhase` threads the same coach-local today as the GET preview so the persisted phase_summary covers the window the coach reviewed; the session surplus-edit cascade anchors client-tz (client-calendar mutation); nutrition deadline/days-to-goal and check-in adherence stats anchor to the client's day. Accepted behavior change: the due day counts as "due today" — overdue starts the next local day. Reminder cron stays unwired (TECHNICAL-DEBT).) **Follow-up (2026-06-11):** coach-UX fix for scheduled plans — with no active plan, the planned plan is now returned as the coach's working plan (`scheduledFor` + "Starts {date}" chip/cancel in the builder, banner only when both plans exist, tz copy only on skew); place-from-library rejects client-past start dates via `getClientTodayString`; apply dialog mins to the client's local today. **⊘ Superseded by events-SOT (Session 2, mig 114):** additive placement removed the `planned`-plan concept — plans coexist as provenance and "active" is **date-driven**, so there is no planned→active promotion. A future plan is simply one whose `effective_from` is after today; the `scheduledFor` / "Starts {date}" coach affordance now reflects that, not a promotion.
...
**Status**: COMPLETE. Three deviations from the plan as written: **(1)** the calendar `clientToday` uses the `clientTimezone && clientTimezone !== "UTC" ? getTodayDateStringInTimezone(...) : todayDate` form (the apply-dialog precedent), NOT the literal `clientTimezone ? … : todayDate` — the `'UTC'` sentinel is the never-synced case and must fall back to the coach device, matching `getClientTodayString`'s coach-tz fallback (the literal form would have yielded *UTC* today and disagreed with the 7.82 server guard). **(2)** `calendar-event-card.tsx` did NOT already receive `clientTimezone` (the "Read first" note was stale); the fix adds a new required `clientToday: string` prop threaded view → week-row → day-cell → card, and also passes it at the `DragOverlay`'s direct `CalendarEventCard` render (a second render site the note didn't mention). **(3)** the gating test asserts via the Duplicate/Delete menu trigger queried as a `button` *tag* (not role) — when an event is draggable dnd-kit stamps `role="button"` on the card's root `<div>`, so a role query matches two elements.
...
> **REMOVED (2026-07-25):** Sessions 7.9–7.12 (goal/roadmap lifecycle) were deleted along
> with the entire roadmaps/phases feature — owner decision to unblock launch; it will be
> rebuilt from scratch post-launch. The session specs, the merge-coordination plan, and the
...
> weight rows) and `client_metric_entries` never carried a tag, and the tags that
> existed were inferred from a mutable column. What shipped instead is canonical
> kg/cm storage with every unit-tag column dropped, the preference on the VIEWER,
> and conversion only at the render boundary — never at the API boundary, which
> 8.2 proposed. `formatWeight` takes `(value, viewerPreference)`, not the
> three-argument form specified here.
>
> **Current rule: `CONVENTIONS.md` §20 Units.** Session specs live in git history.
