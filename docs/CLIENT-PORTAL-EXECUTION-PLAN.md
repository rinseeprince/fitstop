...
> 5. **Security: preserve migration 106's RPC lockdown (same as 7.82).** `transition_phase_atomic` and `archive_roadmap_atomic` are hardened by `106_lock_down_security_definer_rpcs.sql` (REVOKE EXECUTE from PUBLIC/anon/authenticated, GRANT service_role, `SET search_path = public`). When `CREATE OR REPLACE`-ing them here, re-apply that lockdown in the same migration (and prefer DROP+CREATE if the signature changes), or the redefinition silently re-opens the hole 106 closed. Read `106_…sql` first.
