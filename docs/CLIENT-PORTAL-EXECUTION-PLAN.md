...
> **⊘ Superseded by events-SOT (Sessions 4-5).** The "rest-day vs training-day" 2-block model below was already replaced mid-session by a 7-day view (see the 2.10 deviation note above), and the events-SOT overhaul completed the shift: the client now reads a **per-date `dailyTargets` window** built from `nutrition_events` (`getClientNutritionTargets`, repointed in S5◆1) — coach per-day edits (`is_modified`) + notes show through, the `clients.surplus_as_carbs` split is honored, and event→plan FKs are SET NULL. Phases auto-activate by date (no promotion). The Implement/Test steps below are historical. Current shape: `CLIENT-APP-REFERENCE.md → Nutrition`; full model: `docs/ARCHITECTURE.md → Nutrition & Training Events`.
