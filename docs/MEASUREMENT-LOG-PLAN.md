...
**STATUS: NOT STARTED. Owner decision 2026-09-03, CLOSED, reversing D1's correction rows, D9's correction path and D21: EDIT = REPLACE (the row's value changes in place — same row, same day, same source, same check-in stamp); ADD = ADD (a second reading on a day is another row); EDIT CHECK-IN = the check-in's own row edited in place, stamp kept, so its report follows; DELETE = deactivate with Restore (unchanged). D23 answered 2026-09-03: the day's value is the MOST RECENT reading — the one written or edited last. After commit 7; the wellness commits follow it. One migration, DEV then PROD.**
