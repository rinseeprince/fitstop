...
**Status: commits 1–7 SHIPPED — `55ed1242` (2026-09-02), `29456d71` (2026-09-02), `24a2f667` (2026-09-03), `1872eeeb` (2026-09-03), `e8c58cad` (2026-09-03), `6ced90de` (2026-09-03), `83a0f061` (2026-09-03); commits 1–6 smoked, commit 7's fold smoke owed; migrations 158–160 on DEV and PROD; the old stores are gone; a reading can be corrected, removed and restored; a logged day is derived once; the log is the selected metric's, and the metric is `?metric=`; the wellness series is a route over the client's daily log. Next: commit 8 — a reading is EDITED IN PLACE (owner decision 2026-09-03, reversing the correction rows of D1 and D9 and the fold of commit 7 and D21: edit replaces, add adds, editing a check-in's reading edits its own row so the check-in follows, delete deactivates with Restore; D23: the day's value is the reading written or edited last). Commits 8–11 are planned; D1–D23 are answered (D17 moot; D18 reversed — coach wellness logging is retired, commit 10). Commit 12, the day-spine planning step, runs LAST because it ends by deleting this document (renumbered three times on 2026-09-03: the day-spine step went from 6 to 9 and 7–9 became 6–8; then the retirement of coach wellness logging and the client app's wellness series were inserted as 9 and 10 and the day-spine step became 11; then edit-in-place was inserted as 8, the wellness commits became 9–11 and the day-spine step 12).** Twelve commits (the fourth conditional on D9; the fifth independent of the rest; the sixth the log's metric filter; the seventh the wellness series over the client's daily log; the eighth a reading edited in place; the ninth the Wellness pane on that series; the tenth the retirement of coach wellness logging; the eleventh the client app's wellness series through the same kernel; the twelfth a planning step, not a build, and the last to run), each with a pasteable prompt (§6), each independently revertable in order, each gated. Commit 1 stands on its own: it fixed the goal strip on main.
