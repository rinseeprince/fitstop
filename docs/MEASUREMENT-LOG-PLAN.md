...
**STATUS: NOT STARTED. After commit 8 (a check-in's reading is its own row, edited in place). Owner decision 2026-09-03: everything on a check-in review reflects where the client was at that time; the Overview and the Journey keep reading now. Reverses commit 1's rule for the REVIEW page only — "position reads the client record's current reading, never a check-in" — which was written to fix a weightless check-in reading "No goals" and did so by reading today. Lettered, not numbered: nothing renumbers. No migration.**
