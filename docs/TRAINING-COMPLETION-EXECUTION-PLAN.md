...
**Status: APPROVED 2026-09-02 (owner). No code written. Four commits, each with a pasteable
prompt in §5. Delete this file after commit 4 ships; the model then lives in
`docs/ARCHITECTURE.md` and the STATUS ledger below lives in git history.**
...
| Migrations applied on dev | 157 (`157_check_in_forms.sql` is the latest file). Next numbers: 158, 159 |
...
| 3 | **Attendance lives on the calendar workout.** The hero counts calendar workouts by date; `countEventsInRange` is deleted; skipped stops being produced (zero-set saves refused, the fill-in selector loses Skipped, the RN quick path accepts full/partial); **Clear log** ships as an atomic RPC + `DELETE …/events/[eventId]/log`. | 158 — the Clear log RPC | Unchanged counts. A client can un-log; nothing produces a skip. |
| 4 | **The flip.** Migration 159 tightens both constraints and rewrites the values; the type unions shrink; the log link always writes `completed`; every done-count now includes partials by construction; the ownership scan loses its count half; labels per M8; the docs say the shipped shape and nothing else. | 159 | **Partials count everywhere.** The test week reads 5 of 5 with 1 partial on every surface. |
...
E. The ownership scan (moved in commit 2) loses its "no hand-rolled count" rule — with no partial status a count cannot leave partials out, so the guard has nothing to guard — and keeps its "no coach surface reads the stored workouts_completed column" rule, because that column is a submit-time snapshot while the review derives live and a client can still backfill a never-logged day after submitting. Mutation-test the surviving rule (plant a coach read of workoutsCompleted, watch it fail, remove it, say so).
