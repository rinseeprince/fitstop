...
- **Link** is an icon action on the Exercises rail, left of the add-exercise `+` in a `gap-3` cluster (`Link2`, the rail icon-action recipe, `disabled:opacity-50` with no exercises). Picking swaps the rail's meta for `N selected` and its actions for uppercase text actions in `LABEL_CLASS text-[11px]`: Cancel (teal hover), then the four groups Link makes — `Make superset` / `Make circuit` by the count, `Make AMRAP`, `Make EMOM`, `Make For time` (`font-semibold text-[#0d9488] hover:text-[#0b7f75] disabled:text-[#d5e0dd]`; a superset needs two picks, a timed group one or more). One click makes the group and ends the picking. While picking, each card's header row is one `role="checkbox"` button; its grip, remove and chevron give up their places but keep their space, so nothing on the row moves; a picked card carries `ring-1 ring-inset ring-[#0d9488]` and its ordinal circle turns `bg-[#0d9488] text-white` around a `Check h-3.5 w-3.5`.
- **Settings** — the 320px Popover (`align="end" sideOffset={6}`, `p-0`): header `px-3.5 pb-2 pt-3`, the group's name in `text-sm font-semibold` over `N exercises` (`1 exercise` for a timed group of one) in `MONO_LABEL_CLASS normal-case tracking-normal`; body `space-y-3 px-3.5 pb-3.5`, every row `flex items-center justify-between gap-3` of a `LABEL_CLASS` label and an `h-8` control (one height per row). First **Format**, a `<Select>` (`SelectTrigger h-8 w-[150px] text-xs`, aria "Format"): Superset or Circuit by the count (offered from two exercises), Straight sets (on a group of one, the way back to a plain exercise), AMRAP, EMOM, For time — five names, which the segmented control cannot hold at this width. Then the rows the format uses, from `GROUP_FORMAT_SETTINGS` and nothing else: **Rounds** (a superset or circuit, an EMOM, a For time), a number input `MONO_INPUT_CLASS h-8 w-20 text-xs` + `FOCUS_RING`, 1–30; **Time cap** (an AMRAP, required; a For time, optional) and **Interval** (an EMOM, required) — a text box in the same class, typed as minutes or m:ss (`"12"`, `"1:30"`, `"0:40"`; `commitDuration`, `commit-input.ts`) and read back as m:ss, a muted `text-[11px]` `m:ss` after it, stored as seconds; an emptied required box is put back, a For time's cap clears; **Rest between exercises** (straight sets, a superset or circuit, a For time) and **Rest between rounds** (a superset or circuit, a For time), number inputs followed by `s`; and Notes, a `LABEL_CLASS` label over a `min-h-14 bg-white px-2 py-1.5 text-xs` Textarea. An AMRAP has no rests and an EMOM none and no cap, so their rows are not shown. Every field commits on blur, like the rest of the editor; a format switch is one edit, so the heading, the grid and the rows change in one render; no footer.
