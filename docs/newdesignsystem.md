...
- **Rows are rounds** in a superset or circuit: the set grid's first column reads Round; a row has no duplicate or remove (its action cell stays, empty, so the columns align); there is no Add set, no Rest column, and no Rest in the Columns menu — the group's rests are what the client gets. The card's summary reads the reps round by round, `21-15-9 reps`. Rows change only with the group's Rounds.
- **Dragging draws a line; nothing moves.** The dragged card or group follows the pointer at `opacity-40 z-10`, and a `h-0.5 rounded-[1px] bg-[#0d9488]` line (`pointer-events-none absolute inset-x-0 z-20`) marks where it will land, centred in the gap: `-top-[5px]` / `-bottom-[5px]` between the list's items (their 8px gaps), `-top-[6px]` / `-bottom-[6px]` between a rail's cards (their 10px gaps) — the rail's indent is what tells the coach the exercise joins the group — and `top-0` on the list's first item, where the scroll area would clip a line above it. The list keeps `pb-2` so a line under its last item shows. A drop that would change nothing draws no line.
