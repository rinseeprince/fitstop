...
**Typed variant.** Deleting something that can't be put back as it was — something current, or a record of the past that can never be made again — adds a `Type DELETE to confirm` field under the sentence; the CTA stays disabled until it reads DELETE. Reference: `components/clients/goals/delete-goal-dialog.tsx` (the current goal and an ended one).
...
`flex flex-col items-center gap-1 pt-2.5` (`group/wk`). W# chip `rounded-[6px] bg-[rgba(13,148,136,0.08)] px-[7px] py-1 font-mono-display text-[10.5px] font-semibold text-[#0a5c55]`; frequency `font-mono-display text-[9.5px] text-[#c2d0cc]`. Hover control stack `hidden group-hover/wk:flex`; each control `grid h-5 w-5 place-items-center rounded hover:bg-[rgba(13,148,136,0.08)]` (`CTRL_BTN`) with `h-3 w-3` icons (grip, collapse chevron, `Copy` duplicate, `Trash2` delete → `text-destructive hover:bg-red-50`).
