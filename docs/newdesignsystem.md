...
Reference: `components/training/exercise-data/exercise-sessions-table.tsx`, beneath every exercise's chart in the coach's Journey → Training pane and the client's Performance view. One row per logged session in the window, newest first; Date, then one column per measure the window recorded (`utils/exercise-session-columns.ts`).

- **The rail** is a `<SectionLabel label="Sessions">` whose `actions` cluster (`flex items-center gap-3`) reads, left to right: the Columns menu (coach only), the sort, the pager. On the client's view the rail is the section heading instead — `text-[14px] font-semibold` like Personal Records, in a `mb-3 flex items-center justify-between` row — carrying the sort and the pager.
- **Columns menu** — the builder's column selector without its presets (`session-columns-menu.tsx`): the `Columns3` rail icon action (`rounded p-1`, `h-3.5 w-3.5 strokeWidth={1.5}`, aria "Columns for the sessions table", `FOCUS_RING`; disabled `#d5e0dd` only once settled with nothing recorded, never while loading) opening a `DropdownMenuContent` `align="end" sideOffset={6}` headed "Columns", then the recorded columns under `LABEL_CLASS` group labels (Strength, Endurance, Framework — only those holding a column), one `grid` track per group (`w-44` / `w-[320px]` / `w-[460px]` for one, two, three), each column a `DropdownMenuCheckboxItem` with the teal tick. The menu stays open across ticks (`onSelect` prevents default). Date is the row and is not in it.
- **Sort** — the rail dropdown (`<RailDropdown pairs>`): the trigger reads the sort ("Newest first", "Heaviest load"), the menu `w-[320px]` lays the options two to a row, each column's pair on one line, its lead word first ("Fastest pace" · "Slowest pace"). No header sorts.
...
- **The card** is the history tables' `rounded-[6px] bg-white p-5` around the `Table` primitive — rows and headings inherited, the header row `hover:bg-transparent`. A heading is the measure's word (`BOX_LABELS`' where there is one); a load's names the unit its bare numbers are in, "Load (kg)", "e1RM (lbs)".
- **Cells** are `MONO_CELL_CLASS` ink, read in the logged-workout table's grammar ("5 km", "4:45 /km", "Z3", a bare number under its heading); a Time cell carries its distance beside it, `ml-1.5 text-[11px] text-[#93b0b4]`; Sets reads "3/3". Nothing recorded is a `#c2d0cc` dash. The Date cell is `MONO_CELL_CLASS` in `TEXT_SECONDARY`, "Sep 17, 2026" (the PR cards' form).
- **Sideways scroll**: the Date column is pinned like the logged-workout table's Set column — `sticky left-0 z-[1] bg-white`, the row hover's opaque twin `group-hover/row:bg-[#f8fcfb]` on its cell (the row carries `group/row`) — and the measures scroll under it inside the `Table` primitive's own container.
- **States**: loading is a page of skeleton rows at the table's row height, nothing claimed about the columns (the chevrons are absent until rows exist, as a divider pager is); a failed read is `SessionsLoadError` — "Couldn't load the sessions" with a teal Try again, the same words the chart shows in its slot; no session at all is "No sessions logged yet", muted, `py-12`.
- **Frames**: the view — columns ticked, sort, page — is local state. A tick changes the header and every row in one render; a sort pick re-orders the rows, names itself on the trigger and returns the pager to page 1 in one render; the lens switch above leaves the table untouched; another exercise remounts it (the host keys it by the exercise); a new window starts it on page 1 (the window keys its page) with its columns and sort kept. No animation of its own.
