...
**A toast is never an outside click, and neither is the program assistant's panel** (both in `lib/outside-interaction.ts`; the panel is a Radix layer of its own so it stays usable over the modal session sheet). The toaster sits above every overlay and a toast keeps its pointer events under a modal, so it can be hovered and closed over an open drawer — and the `Dialog` and `Sheet` primitives ignore a pointer-down or interaction whose target is inside the toaster or the assistant panel, so closing the toast, or typing into the assistant, leaves the drawer open.
