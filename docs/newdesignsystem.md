...
- **Session card:** `flex h-full cursor-pointer flex-col rounded-[6px] bg-white px-[11px] py-2.5` + `TRAINING_CARD_BORDER` + `FOCUS_RING`; expanded hover-lift `hover:-translate-y-px hover:shadow-[0_6px_20px_rgba(13,148,136,0.08)]`; drag-over `border-[#0d9488]`. Collapsed `min-h-9` (36px), expanded `min-h-[148px]`.
  ...
  - Exercise list `mt-1.5 space-y-[3px]` (first 3): index `w-2 font-mono-display text-[9.5px] text-[#c2d0cc]`, name `text-[11px] text-[#5a7d82]`, sets×reps `font-mono-display text-[10px] text-[#93b0b4]`; "+N more" `font-mono-display text-[10px] text-[#c2d0cc]`. A linked group's lines, and a timed group's of any size, sit on its rail, indented: `space-y-[3px] border-l-2 border-[rgba(13,148,136,0.15)] pl-1.5`; a timed group's rail leads with its heading in `MONO_META_CLASS text-[10px]` (`truncate`) — `AMRAP · 12m`, `EMOM · 6 rounds · every 1m`, `For time · 3 rounds · 12m cap` — a superset's has none; the numbers run straight on, and an exercise whose rows are rounds reads its rounds, `3×8-10`, or `21-15-9` when rounds differ, an AMRAP's the reps of its one round, `10`.
...
- **A linked group (two or more exercises), or a timed group,** is `<section aria-label={title} className="flex flex-col gap-2">`:
  - Heading `px-1`: the name `text-[13px] font-semibold text-[#0c1a1e]`, then `·` (`mx-1.5`) and the rounds in `font-normal text-[#5a7d82]`, the count in `MONO`, then the clock the same way — an AMRAP's cap "AMRAP · 12m", a For time's "For time · 3 rounds · 12m cap", an EMOM's "EMOM · 8 rounds · every 1m", the duration in `MONO` and its word in sans (`GroupHeadingTitle`). Under it the rests, `mt-0.5 text-[12px] text-[#5a7d82]`, each duration in `MONO` — "30s rest between exercises · 1m 30s rest between rounds". **No notes here** (`GroupHeadingLines withNotes={false}`; owner, 2026-09-19): the group's notes are the coach's instructions to the client, and the coach wrote them. The session editor's heading keeps them, `mt-0.5 whitespace-pre-wrap text-[12px] text-[#93b0b4]`, since that is where they are edited.
...
### Sessions table (readout)

Reference: `components/training/exercise-data/exercise-sessions-table.tsx`, beneath every exercise's chart in the coach's Journey → Training pane and the client's Performance view. One row per logged session in the window, newest first, read the way a coach reads a session (owner, 2026-09-21): the date, the session's working sets in shorthand, then a fixed few figures by the exercise's type, the main one first (`utils/exercise-session-figures.ts`). It never uses the builder's per-set columns and has no Columns menu.

- **The rail** is a `<SectionLabel label="Sessions">` whose `actions` are the pager's chevrons. On the client's view the rail is the section heading instead — `text-[14px] font-semibold` like Personal Records, in a `mb-3 flex items-center justify-between` row — carrying the chevrons.
- **Sort** — the figures' headings (`SortHeading`, `components/training/exercise-data/sort-heading.tsx`, shared with the All exercises table), never a picker on the rail: each `TableHead`'s words are a `button` in `LABEL_CLASS` (a button doesn't inherit the heading's uppercase) + `inline-flex items-center gap-1 rounded-[4px]`, hover `text-[#0d9488]`, `FOCUS_RING`; the sorted heading stays `text-[#0d9488]` with an `ArrowDown` / `ArrowUp` `h-3 w-3 strokeWidth={1.5}` after its words (down = high to low), `aria-sort` on its `th`, and every heading's `title` words what a click will do ("Highest e1RM first", "Oldest first"). A first click sorts the way the figure leads — highest e1RM, fastest pace, longest hold, newest first — a second click the other way; one sort, ties newest first, blanks last. The Sets heading is plain words: the sets don't sort.
- **Pager** — the pager's chevrons alone (`<PagerArrows>`), ten a page (`HISTORY_PAGE_SIZE`), paging the rows in memory: no "Showing X of Y sessions", since the session window on the rail above already says how many (owner, 2026-09-21). Disabled, never hidden, on a single page, so they don't pop in when a wider window needs a second.
- **The card** is the history tables' `rounded-[6px] bg-white p-5` around the `Table` primitive — rows and headings inherited, the header row `hover:bg-transparent`. A load's heading names the unit its bare numbers are in: "Sets (kg)", "e1RM (kg)", "Top set (lbs)".
- **The Date cell** is `MONO_CELL_CLASS` in `TEXT_SECONDARY`, "Sep 17, 2026" (the PR cards' form); a session holding a record carries a `ml-1.5 text-[12px] text-[#d97706]` ★ after it — the chart's PR star colour — `role="img"` with the record in its label and `title` ("5 Rep Max · 105 kg").
- **The Sets cell** is `MONO_CELL_CLASS` ink, `min-w-[200px] whitespace-normal` so a long session wraps instead of widening the table: `100 × 8 · 102.5 × 8`, `12 · 11 · 10`, `60 × 40 m`, `3 × 1 km` and `64 × 3 × 40 m` (reps on a distance are repeats), `5 km`, `6 × 800 m` `1:30 · 1:20` for holds — the shape only, never the times (the Time and Pace figures hold them).
- **Figure cells** are `MONO_CELL_CLASS` ink in the logged-workout table's grammar ("129.8", "102.5 × 8", "4:50 /km", "5 km", "Z3", a bare number under its heading) — the number alone, nothing beside it. Nothing recorded is a `#c2d0cc` dash.
- **A row opens its workout** — `cursor-pointer` on the row and its click, like the Data table's rows; a row with nothing to open (a client's log with no calendar workout) has neither.
- **Sideways scroll**: the Date column is pinned like the logged-workout table's Set column — `sticky left-0 z-[1] bg-white`, the row hover's opaque twin `group-hover/row:bg-[#f8fcfb]` on its cell (the row carries `group/row`) — and the rest scroll under it inside the `Table` primitive's own container.
- **States**: loading is a page of skeleton rows at the table's row height, nothing claimed about the figures, held until the sessions, the exercise's type and its records have all landed (the chevrons are absent until rows exist, as a divider pager is); a failed read is `SessionsLoadError` — "Couldn't load the sessions" with a teal Try again, the same words the chart shows in its slot; no session at all is "No sessions logged yet", muted, `py-12`.
- **Frames**: the sort and the page are local state. A heading click re-orders the rows, moves the arrow and returns to page 1 in one render; a row click opens the workout in one; the lens switch above leaves the table untouched; another exercise remounts it (the host keys it by the exercise); a new window starts it on page 1 (the window keys its page) with its sort kept. No animation of its own.

### All exercises table (readout)

Reference: `components/training/exercise-data/all-exercises-table.tsx` — what the exercise picker's first row, **All exercises**, shows under the hero in the coach's Journey → Training pane and the client's Performance view, and what both open on with no exercise picked. One row per exercise the client has logged, with its type, its sessions, its last day and its bests (`utils/exercise-bests-table.ts`); no chart, no lenses, no KPI strip and no session window above it — the hero loses its lens row.

- **The rail** is a `<SectionLabel label="Exercises">` whose `actions` are the history tables' `<DividerPager noun="exercises">` — the count and the chevrons, ten a page ("Showing 10 of 23 exercises"): unlike the Sessions table, no window above it says how many rows there are. On the client's view the rail is the section heading instead — `text-[14px] font-semibold`, like Personal Records, in a `mb-3 flex items-center justify-between` row — carrying the same pager.
- **The card** is the history tables' `rounded-[6px] bg-white p-5` around the `Table` primitive — rows and headings inherited, the header row `hover:bg-transparent`.
- **Columns**, in order: Exercise, Type, Sessions, Last logged, Heaviest load, Best e1RM, Most reps, Best time, Heaviest carry, Longest hold. A load's heading names its unit: "Heaviest load (kg)", "Best e1RM (lbs)", "Heaviest carry (kg)".
- **Cells**: the Exercise `text-[13px] font-medium` ink, pinned like the Sessions table's Date (`sticky left-0 z-[1] bg-white`, the row hover's opaque twin `group-hover/row:bg-[#f8fcfb]`); the Type `text-[12.5px]` in `TEXT_SECONDARY`, sans — it is a word; every other cell `MONO_CELL_CLASS` — ink, the Last logged in `TEXT_SECONDARY` like a date ("Sep 20, 2026") — the number alone for a load or a count, a best time as the race and the clock ("Half marathon · 1:32:10", a number-bearing string, so mono throughout), a carry as its load and distance in the Sets shorthand ("70 × 40 m"), a hold as a clock. Nothing recorded is a `#c2d0cc` dash.
- **Sort** — every heading is a `SortHeading`: the first click the way the column leads (name and type A to Z, most sessions, newest, heaviest, highest, most, fastest, longest), a second the other way, one sort, ties by name, blanks last; the table opens on most sessions first, the picker's order. The title words what a click will do ("Fastest time first", "Name Z to A").
- **A row opens its exercise** — `cursor-pointer` on the row and its click, the same one address replace as a pick from the picker.
- **States**: loading is a page of skeleton rows at the row height, nothing claimed; a failed read is `SessionsLoadError what="exercises"` — "Couldn't load the exercises" with a teal Try again; no exercise at all is "No exercises logged yet", muted, `py-12`. The pager is absent until rows exist.
- **Frames**: the pick is the address alone — All exercises removes the exercise in one replace, so the hero, the rail and the table change in the render the address commits in; until then the previous exercise stays. The sort and the page are local state: a heading click re-orders the rows, moves the arrow and returns to page 1 in one render; the table mounts only while All exercises is picked, so a return to it starts on its default sort. No animation of its own.
...
- **Link** is an icon action on the Exercises rail, left of the add-exercise `+` in a `gap-3` cluster (`Link2`, the rail icon-action recipe, `disabled:opacity-50` with no exercises). Picking swaps the rail's meta for `N selected` and its actions for uppercase text actions in `LABEL_CLASS text-[11px]`: Cancel (teal hover), then the four groups Link makes, each named by its format alone — `Superset` / `Circuit` by the count, `AMRAP`, `EMOM`, `For time` (`font-semibold text-[#0d9488] hover:text-[#0b7f75] disabled:text-[#d5e0dd]`; a superset needs two picks, a timed group one or more; no "Make" before the name, owner 2026-09-19). One click makes the group and ends the picking. While picking, each card's header row is one `role="checkbox"` button; its grip, remove and chevron give up their places but keep their space, so nothing on the row moves; a picked card carries `ring-1 ring-inset ring-[#0d9488]` and its ordinal circle turns `bg-[#0d9488] text-white` around a `Check h-3.5 w-3.5`.
- **Settings** — the 320px Popover (`align="end" sideOffset={6}`, `p-0`): header `px-3.5 pb-2 pt-3`, the group's name in `text-sm font-semibold` over `N exercises` (`1 exercise` for a timed group of one) in `MONO_LABEL_CLASS normal-case tracking-normal`; body `space-y-3 px-3.5 pb-3.5`, every row `flex items-center justify-between gap-3` of a `LABEL_CLASS` label and an `h-8` control, **every control `w-[140px]`** so the boxes line up (owner, 2026-09-19). First **Format**, a `<Select>` (`SelectTrigger h-8 text-xs`, aria "Format"): Superset or Circuit by the count (offered from two exercises), Straight sets (on a group of one, the way back to a plain exercise), AMRAP, EMOM, For time — five names, which the segmented control cannot hold at this width. Then the rows the format uses, from `GROUP_FORMAT_SETTINGS` and nothing else: **Rounds** (a superset or circuit, an EMOM, a For time), a number input `MONO_INPUT_CLASS h-8 text-xs` + `FOCUS_RING`, 1–30; **Time cap** (an AMRAP, required; a For time, optional) and **Interval** (an EMOM, required) — a text box in the same class, typed as minutes or m:ss (`"12"`, `"1:30"`, `"0:40"`; `commitDuration`, `commit-input.ts`) and read back as m:ss, stored as seconds; an emptied required box is put back, a For time's cap clears; **Rest between exercises** (straight sets, a superset or circuit, a For time) and **Rest between rounds** (a superset or circuit, a For time), number inputs in seconds; and Notes, a `LABEL_CLASS` label over a `min-h-14 bg-white px-2 py-1.5 text-xs` Textarea. **A box says its grammar as its placeholder** — `m:ss` on the clocks, `s` on the rests — and nothing sits beside a box. An AMRAP has no rests and an EMOM none and no cap, so their rows are not shown. Every field commits on blur, like the rest of the editor; a format switch is one edit, so the heading, the grid and the rows change in one render; no footer. **The card stays open until a click outside it or Escape**: focus leaving it never closes it (`onFocusOutside` declined), because a commit remounts the box it came from and can re-lay the grid behind the card.
- **Rows are rounds** where a group's rows are rounds — a superset or circuit, an AMRAP, an EMOM, a For time of any size: the set grid's first column reads Round and is 44px wide, the client tracker's number column (`#` needs only 20px), in the heading and every row alike; a row has no duplicate or remove (its action cell stays, empty, so the columns align); there is no Add set, no Rest column, and no Rest in the Columns menu — the group's rests are what the client gets. The card's summary reads the reps round by round, `21-15-9 reps`. Rows change only with the group's Rounds; in an AMRAP every exercise has one row, the work of a round, and Rounds is not a setting.
