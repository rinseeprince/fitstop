...
- **Range boxes.** Reps, Load and RPE are each ONE text box taking a value or a range typed with a hyphen — "8-12", "100-105", "7-8" (`utils/target-range.ts`, one grammar; en and em dashes are accepted, the box shows a hyphen). The box seeds from the stored pair, selects all on focus, commits on blur behind the seeded-string guard (a focus-through writes nothing), reverts a typo to what was there, and clamps each end to the column's bounds — a typed RPE 0 becomes 1. The load box (`LoadRangeInput`, `w-20`) carries the viewer's unit or `%` as its placeholder and stays disabled until a load type is chosen; a drop's load is still one number (`LoadValueInput`). Readouts never use the hyphen: the client grid's hint and the coach's log view read a range with an en dash — "7–8", "100–105 kg", "70–75% 1RM" (`formatTargetReadout`, `formatPrescribedLoad`, `formatPrescribedRpe`).
- **Columns menu.** Offers today's five (Set type, Reps, Load, RPE, Rest) with the teal `DropdownMenuCheckboxItem` tick; an exercise's last column can't be unticked; "Show all columns" ticks the five. A column the menu doesn't offer — one the exercise already carries — is kept by every tick.
...
- **A linked group** is `<section aria-label={title} className="group/grp relative flex flex-col gap-2">`. Heading row `flex items-start gap-2 px-1`: a hover-revealed grip that drags the whole group (`GripVertical h-3.5 w-3.5`, `-ml-0.5 mt-px rounded p-0.5 opacity-0 group-hover/grp:opacity-100`), the readout's heading lines in `min-w-0 flex-1`, then a hover-revealed action cluster (`opacity-0 group-hover/grp:opacity-100`, held open with its popover by `has-[[data-state=open]]:opacity-100`): settings (`SlidersHorizontal`, aria "Superset settings") and Unlink (`Unlink2`, aria "Unlink superset"), each `rounded p-1 text-[#93b0b4] hover:text-[#0d9488]` + `FOCUS_RING`, icons `h-3.5 w-3.5 strokeWidth={1.5}`. Under it the readout's rail, `flex flex-col gap-[10px] border-l-2 border-[rgba(13,148,136,0.15)] pl-3`. View mode drops the grip and the actions.
