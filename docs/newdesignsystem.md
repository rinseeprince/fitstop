...
- **Sort** — the column headings (`SortHeading`), never a picker on the rail: each `TableHead`'s words are a `button` in `LABEL_CLASS` (a button doesn't inherit the heading's uppercase) + `inline-flex items-center gap-1 rounded-[4px]`, hover `text-[#0d9488]`, `FOCUS_RING`; the sorted heading stays `text-[#0d9488]` with an `ArrowDown` / `ArrowUp` `h-3 w-3 strokeWidth={1.5}` after its words (down = high to low), `aria-sort` on its `th`, and every heading's `title` words what a click will do ("Heaviest load first", "Oldest first"). A first click sorts by the column the way it leads — heaviest, most, fastest, longest, highest, newest first — a second click the other way; one sort, ties newest first, blanks last.
...
- **Frames**: the view — columns ticked, sort, page — is local state. A tick changes the header and every row in one render; a heading click re-orders the rows, moves the arrow and returns to page 1 in one render; the lens switch above leaves the table untouched; another exercise remounts it (the host keys it by the exercise); a new window starts it on page 1 (the window keys its page) with its columns and sort kept. No animation of its own.
