...
- **`getClientProgressData` reads the goal mirror on `clients` (`goal_weight`, `goal_body_fat_percentage`) rather than `client_goals`.** Consolidation candidate alongside the `createPortalClient()` one. The readings themselves come from the measurement log's two views, embedded beside those columns.
