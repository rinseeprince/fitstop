...
- **`getClientProgressData` reads the goal in force on the client's today (`getCurrentGoal`, `client_goals`) beside its session-client reads.** Consolidation candidate alongside the `createPortalClient()` one. The readings themselves come from the measurement log's two views, embedded in the client read.
