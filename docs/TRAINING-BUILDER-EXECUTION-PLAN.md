...
- **D2 — Explicit weeks.** `week_index INTEGER NOT NULL DEFAULT 0` on `coach_saved_sessions` + `training_sessions`; order by `(week_index, order_index)`. **Required** `coach_saved_plans.week_mode TEXT CHECK ('repeat','sequential')` so placement distinguishes "repeat 1 authored week × `program_duration_weeks`" from "N distinct authored weeks placed once." `week_index` does **not** go on `training_events` (session FK resolves it). Rewrite `generateCycleAwareEvents`, `recomputePlanCycleInfo` (`coach-library-helpers.ts`), and the `overwriteSavedPlan` cycle derivation to be week-scoped; legacy `week_index=0`/`week_mode='repeat'` plans keep today's flat-cycle behavior.
...
- **Migrations:** **119** (Phase 1: `set_specs` + `video_url` + `set_logs.set_type`), **120** (Phase 1 ckpt 1c: analytics RPCs), **121** (Phase 2: `week_index` + `week_mode`). Phases 3–6: none.
...
### Phase 2 — "Programs" nav + shared multi-week builder + week model

> **Prompt to paste:**
> "You are implementing **Phase 2** of `docs/TRAINING-BUILDER-EXECUTION-PLAN.md`. Read `docs/ARCHITECTURE.md`, `CONVENTIONS.md`, and this doc's §2, §3, §4, then the Phase 2 'Read first' files. Confirm scope/out-of-scope before coding. Implement **only** Phase 2: migration 121 (`week_index` + `week_mode`), the week-aware rewrite of placement + cycle-derivation helpers, the new **Programs** nav item + `/dashboard/programs` route absorbing the training-library page (with redirect), and the shared composed multi-week builder component (`components/clients/training/program-builder/`) with a `target` props abstraction — read-only render first, then authoring (add/duplicate week, day slots, exercise-library insert, per-set compact+expand, warm-up, superset, note/video). Build as a tree of ≤250-line components; use internal commit checkpoints. Do not wire the client editor (Phase 5), saved-workout insert (Phase 3), or progression (Phase 4). Run the §2 gates; commit with the Phase 2 message. Ask me to run `supabase db push` for mig 121."

**Objective.** A first-class full-page builder authoring distinct multi-week programs on the new set model, built as the reusable component Phase 5 remounts.

**Read first.** `services/library-placement-service.ts` (`generateCycleAwareEvents`, `placePlaceablePlanOnCalendar`, `calculatePlacementEndDate`, `getNextPlanStartCap`), `services/coach-library-helpers.ts` (`recomputePlanCycleInfo`), `services/coach-saved-plan-service.ts` (`overwriteSavedPlan` cycle derivation), `components/clients/training/builder/draft-editor.tsx` (current editor to supersede), `app/dashboard/training-library/page.tsx` + `components/training-library/apply-to-client-dialog.tsx`, `lib/navigation.ts`, `components/clients/training/library/preview-session-card.tsx` + `preview-exercise-row.tsx`, `utils/exercise-set-specs.ts` (from Phase 1), `docs/newdesignsystem.md`.

**Implement.**
1. **Migration 121** — `week_index INTEGER NOT NULL DEFAULT 0` on `coach_saved_sessions` + `training_sessions`; `week_mode TEXT NOT NULL DEFAULT 'repeat' CHECK (week_mode IN ('repeat','sequential'))` on `coach_saved_plans`. Additive. Gen types + commit.
2. **Week-aware placement** — rewrite `generateCycleAwareEvents` to walk `(week_index, order_index)` slots: `repeat` → repeat the single authored week for `program_duration_weeks`; `sequential` → lay N authored weeks once (optionally repeat the block). Emit events for non-rest slots (an `is_rest` slot per week is the single rest encoding). **Preserve** per-event `calorie_surplus_percentage` (`session.surplus ?? plan.default`), the window bounds, and upsert idempotency. Rewrite `recomputePlanCycleInfo` + `overwriteSavedPlan`'s cycle derivation to be week-scoped (legacy `week_index=0` keeps flat behavior).
3. **Nav + route** — add `{ name: "Programs", href: "/dashboard/programs", icon: <Lucide> }` to `lib/navigation.ts` after "Clients"; create `app/dashboard/programs/page.tsx` mounting the builder (`mode='library'`) + the browse/apply library (fold in `PlanCard`/`ApplyToClientDialog` from the old page); redirect `/dashboard/training-library` → `/dashboard/programs`.
4. **Shared builder tree** — `components/clients/training/program-builder/`: `program-builder.tsx` (orchestrator, `target` props), `week-list.tsx`, `week-block.tsx` (add/duplicate week, per-slot rest), `day-slot.tsx`, `exercise-card.tsx` (compact prescription + "expand to edit per set" + "add warm-up sets"), `set-row-editor.tsx` (per-set specs incl. set type/load/rest/drops), plus superset grouping and the note/video fields. Read-only render first (prove wiring against an existing template), then authoring. Reuse dnd-kit patterns from `draft-editor.tsx`. Follow `docs/newdesignsystem.md` tokens.
5. **`set_specs`/`video_url` write-side (DEFERRED FROM PHASE 1).** Phase 1 shipped the `set_specs`/`video_url` columns (mig 119), the snapshot capture, and the read-side `countWorkingSets`, but **not** the write-side splat (no source authored `set_specs` yet). Phase 2 is where authoring begins, so wire it here **exhaustively**, adding `set_specs` + `video_url` to **every** exercise clone/insert site (~16 across 12 service files — `grep 'from("training_exercises")\|from("coach_saved_exercises")'`). Classify each:
   - **Clone-from-existing** (splat `set_specs`/`video_url` verbatim): `library-placement-service.ts` (place plan `:145`, place session `:245`), `coach-saved-plan-service.ts` `promoteDraftToSaved` copy (`:303`), `coach-library-calendar-service.ts` (`:90`, `:159`), `training-session-service.ts` `cloneSessionForEvent` (`:432`, `:462`), `training-event-calendar-service.ts` `duplicateWeek` (`:445`).
   - **Input-accepting** (project the compact columns from `set_specs` via a NEW clamped `compactFromSpecs` — `sets = clamp(count(non-warmup specs), 1, 20)` to satisfy the `training_exercises.sets` CHECK; forbid all-warmup at authoring validation): `overwriteSavedPlan` (`:549`), `insertSavedExercises`, `addSavedExercise`, `updateSavedExercise`, AI `insertTrainingSessions`/`createTrainingPlanWithSessions`, `bulkReplaceExercises`/`replaceSessionExercises`, `addExercise`. Extend the input zod schemas (`savedExerciseInputSchema` etc.) + the D3a `inlinePlanBodySchema` to accept `set_specs`/`video_url`.
   - Add `expandSetSpecs` (log-form seeding) and thread the prescription's `set_specs`/`video_url` to the client via the events API + `normalizeExercise`/`PrescribedExerciseView` (deferred from Phase 1), plus `SetLog.setType`/`mapSetLogRow`, `setPerformanceSchema.setType`, and the client-driven `set_logs.set_type` write.
   - **Landmine-#2 defense = an end-to-end survival test**, not blind splatting: author a `set_specs`-bearing exercise → assert it survives save-to-library, overwrite, pristine apply, **inline apply**, AI-gen, from-calendar, duplicate-week, place-session, add/update-exercise. Enumerate the site list above as the test matrix.
   - **Also retire the last `saved_plan_id` reader** (`getClientTrainingPlan:154-179` splices rest rows from the template's `cycle_length`/`rest_pattern`): the week model puts self-describing cycle metadata on the placed plan, closing the Phase-1 "inline placement renders as a flat list (no rest rows)" display gap.

**Do NOT.** Touch the client-attached drawer/editor (Phase 5) beyond leaving it working. Implement saved-workout insertion (Phase 3) or duplicate-week **progression rules** (Phase 4 — plain duplicate-week is fine here). Put `week_index` on `training_events`.

**Tests to write.** `services/library-placement-service.test.ts` — repeat vs sequential, uneven weeks, per-slot rest, surplus propagation, idempotent re-place, legacy `week_index=0` fallback. `services/coach-library*.test.ts` — week-scoped cycle recompute. Component tests for `week-block` (add/duplicate) and `exercise-card` (compact↔expand, add warm-up).

**Verify.** Gates green. Manual: create a 3-week `sequential` program in `/dashboard/programs`, apply it to a test client, confirm the calendar shows 3 distinct weeks with correct rest days and surplus on every training day; confirm a legacy `repeat` plan still places identically to before.

**Commit message.** `feat(training): full-page multi-week Program builder + week model (builder S2, mig 121)`
