...
**`clients.goal_deadline` is not merely stale — it is unreachable.** `mapClientRow` (`lib/mappers.ts:72-73`) maps only `goalWeight` and `goalBodyFatPercentage`, so `client.goalDeadline` is **always `undefined`** and every `?? client.goalDeadline ?? null` fallback in the `resolveEffectiveGoal` call sites is dead code. Do not add it to the mapper to "fix" a deadline read — read `client_goals`.
