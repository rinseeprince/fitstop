...
- **Journey** (`/client/metrics`, `components/client-portal/metrics/metrics-hub.tsx`): progress hub — body metrics, habit progress + streaks, and trends. Its physique and wellness series come from `GET /api/client/progress` (`services/client-portal-progress.ts`), over the days asked for, counted back from the client's today, and read under the client's own session: each physique metric's day-values from the measurement log (`dayValues`), and each wellness score from the client's daily log, one value per day they logged it (`wellnessDayValues`), which `clients_select_own_wellness_logs` lets them read. The coach's Journey derives both through the same two kernels, so both apps chart the same value for the same day; a check-in's five wellness figures are the averages it reported and are on neither chart.
...
- No policy grants `anon`, `authenticated` or PUBLIC a write: every write goes through a route and `supabaseAdmin`, except activation's `clients` UPDATE, under the coach's session through "Coaches can update their own clients" (CONVENTIONS §8 → "RLS policies").
