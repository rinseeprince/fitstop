...
- **Where each anchor applies** (Sessions 7.82–7.84): client tz — plan placement RPCs (`p_today`), calendar move/duplicate/delete guards, plan promotion, the client home week, check-in gate/window, streaks/habit defaults, goal-pace `today`, check-in due/overdue. Coach tz — attention-feed window, coach "current week" metrics/history anchors, phase-review adherence bound. The plan-status RPCs additionally take `p_effective_from DATE DEFAULT NULL` coalescing to `p_today` (migration 110).
