...
- **An edit is resolved against the day as computed.** `materializeNutritionEventDays` (`services/nutrition-event-edit-service.ts`) reads the computed days once over the selection's span and writes only the selected dates: an absolute edit stores the coach's calories and macros verbatim on every selected day — the editor's Set targets tab is the macro balancer (`components/clients/nutrition/macro-balance.tsx`, opened on the first selected day's numbers), so every selected day gets the same four numbers, and a payload carrying calories alone, which only a raw API caller sends, holds protein and rebalances carbs and fat. A note-less edit keeps the day's standing note; an empty string clears it. A selected day no version covers has no computed day and is skipped.
