...
coaches
  ...
  └── clients                        -- coach_id FK, one coach per client
        ...
        ├── client_phases             -- journey BLOCKS (the coach-facing noun is "block"): name, focus, [starts_on, ends_on], archived_at — no target, no goal
...
- **Nothing computes from `focus`, and a block carries no target and no goal.** The client's goal lives in `client_goals` (see "Effective goal resolution") and is read by the Overview's status band, the Journey's Physique hero, the check-in review and the client's Program tab through the journey wire — never by a block: a goal can change mid-block and a block can be shortened, so which goal governed a block is a derivation the platform does not carry (owner, 2026-09-11). The card's Weight column is the block's weight change alone — the reading at its start and the latest, from the Journey's weight series (the measurement log's day-values, through `use-merged-metrics`), or a dash. `focus` never reaches the nutrition calculator, and the blocks routes never call `updateGoals` (pinned by test).
