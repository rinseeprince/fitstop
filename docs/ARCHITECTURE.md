...
**A logged day is re-snapshotted, by design** (owner decision 2026-08-26): `upsertNutritionLog` re-snapshots the day's target from the computed day on **every** food save, so a session moved onto or off a day — a coach move, an amendment, or the client's own move — changes the target the client sees at their next save, and the coach calendar reads the computed day. Freezing a logged day was designed and rejected: a frozen target keeps a training surplus on a day whose session has left. **The coach's replacement of a logged today is re-recorded at once** rather than waiting for that next save (owner, 2026-09-11): after a plan save whose window covers the client's today, and after a per-day edit or reset of today, `rerecordNutritionLogTarget` (`services/daily-log-card-service.ts`) rewrites today's row through the ONE snapshot helper `upsertNutritionLog` writes through — the target as the day is now computed, the adherence against the row's own consumed calories, the covering version's stamp — so the history table, the calendar, the client's day and the check-in week read the new target with the logged meals on top. Never a past day. A day nothing covers any more keeps the target it was logged under: the writer leaves its target columns in place and judges adherence against them, and `resolvePlanContextForDate` stamps the standing log's own version, so a started day stays open (`assertHasActivePlan` passes on the kept stamp). A failed re-record surfaces after the change has landed as `NutritionLogRerecordError` — "The change is saved, but today's food log still shows the previous target." — never as "failed to save".
...
3. **Days no version covers**: `null` — a gap between plans, or the stretch a delete opened; there is no target and a first food log is refused there, while a day already begun reads its own snapshot at level 1 and stays open (the writer keeps the target it was logged under)
