...
- **`prescribedExercises` is on the wire for a reason.** An exercise the client never touched is
  ...
  prescription — in order, group by group, issued in a `Promise.all` with the performed-session-name
