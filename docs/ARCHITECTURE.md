...
- **A date change can bring the calendar with it — never on its own.** Moving a block's end raises a confirm dialog on the blocks screen AFTER the dates have saved; nothing happens until the coach picks from it (`POST /api/clients/[id]/blocks/[blockId]/events`, `services/block-event-sync-service.ts`). It RECONCILES rather than replaying a diff — `fill` covers the block, `clear` removes the scheduled days that now sit outside it — so it needs no memory of the old window and re-running it changes nothing. **Order is load-bearing on the fill:** an optional headless re-price mints the version first, the training extension runs second (its new days carry the surpluses the nutrition generator reads per date), and the nutrition fill last. The extension CONTINUES the placed program's repetition using `training_plans.authored_slot_count` (migration 165) — resuming at `placedSlots % authoredSlotCount` rather than restarting the pass — and returns null for a block with no program or a plan placed before that column existed, where the coach is told to place one rather than handed a guessed program. Clears are bounded by the next block's start, floored at the client's today and scoped to `status = 'scheduled'`, and they deactivate the training slot rows behind the events they remove, because a placed plan's end is derived from its active row count.
...
1. **The furthest block that has not finished** by the anchor (`getFurthestBlockEnd`, `services/client-blocks-service.ts`) — a block is the time-bound program the coach sells, so its last day is the answer whenever there is one. No predicate on `starts_on`: a block set up for next week extends the horizon the moment it is saved.
...
Precedence, not a maximum: past the coach's declared bound there are deliberately no events, so a client between programs reads as quiet until the coach draws the next bound. A block that never actually bounded anything would be decoration. The consequence to know: a client with no block whose program ends in three weeks gets three weeks of targets, not eight.
