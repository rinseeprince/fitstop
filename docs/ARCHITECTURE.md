...
- **The two consistency surfaces each read their own fixed window**: the adherence dot rails a fortnight (`ADHERENCE_WINDOW_DAYS`), the wellness cards a week (`WELLNESS_WINDOW_DAYS`) — which is what their seven-point sparklines and their "not logged this week" copy are drawn for. Neither is selectable; long ranges live on the Journey tab.
...
| `GET …/daily-logs` | The five wellness cards | Through `useWellnessData(clientId, { daysBack, withHabitLogs: false })` — narrowed to 7 days, and the habit fetch stays off because nothing on the Overview reads habit logs |
