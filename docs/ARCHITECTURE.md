...
- **Blocks exist as data, but nothing on this page renders them yet.** `client_phases` (see "client_phases table" above) gives a client a chain of named, rate-carrying stretches of calendar; Session 3 of the client-goals-and-blocks workstream adds the "Goal & plan" panel that reads them. *(Corrected 2026-07-29 — this line previously read "No roadmap or phase concept exists", which migration 137 made false. The **roadmap** half stays true: roadmaps were removed outright in 2026-07-25's migration 133 and blocks did not bring them back.)* Until that panel lands, the status card's chips describe the client's active **training program** — plan name · `Week X of Y` (via `utils/plan-week.ts`) · Active/Ended. Read "block" as the nutrition/goal concept everywhere in this document; the training chips are about a *program*. "Ended" here means today is past the **authored duration** — a different derivation from the slot-count one the client reader and the amendment surface share (see "Client-side plan tier" above for why both exist).
