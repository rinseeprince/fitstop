...
2. **The value for a day is the reading written or edited last** — the latest live row for that client, metric and day by `updated_at`, a tie broken by id (`lib/measurements/day-values.ts`; owner decision D23). No source ranking: the coach editing after the check-in wins because the edit is the day's last touch, and a reading added after that edit wins in turn.
