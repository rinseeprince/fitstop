...
coaches
  ...
  └── clients                        -- coach_id FK, one coach per client
        ...
        │     ├── training_sessions   -- carries calorie_surplus_percentage (copied onto each event; a nutrition day reads it off the event)
        ...
        ├── nutrition_plans           -- DATE-RANGED VERSIONS, each a PLACEMENT with a stored end (mig 166); a day's target is COMPUTED from the version covering it -- there is no day table
        ...
        ├── nutrition_plan_notes      -- the plan-save notes, append-only (mig 147; nutrition_plan_id FK is SET NULL)
        │
...
- **A block owns its own window** (migration 164 — an owner reversal of the derived-start chain, 2026-09-04). The PUT sends `{ blocks: [{ id?, name, startsOn?, endsOn?, focus?, targetWeightKg? }] }` — every block carries BOTH dates and nothing is derived from its neighbour. **Gaps are allowed and mean nothing is planned** (the client is between programs; both placements read that as "no bound" and fall back). **Overlaps are refused** — in the service, with a sentence naming both blocks, and by the `client_phases_window_overlap` gist exclusion constraint, because "the block covering this date" decides the training placement window and the nutrition version's end and cannot have two answers. The service also rejects an end before its start, caps a block at 52 weeks of days, and refuses a NEW block opening in the past (a past-dated block prescribes nothing — placement and the nutrition save both refuse a past date). The payload carries every stored row, so checking it against itself IS checking it against the client's whole calendar. **Elapsed blocks (`ends_on <` client-today) keep their DATES pinned from storage** (they omit `endsOn` — elapsed history is not an input; a differing one 422s) **while their name/focus/target stay editable** (Session 3.6-C — the pin protects lived-day attribution, not typos in a finished label), and the **symmetric window floor** rejects any edit that would re-label lived days: a stored current block must still contain today; a stored future block may become current but never wholly past; new id-less rows land anywhere (history backfill). Removal is not expressible through the PUT.
- **A date change can bring the calendar with it — never on its own.** Moving a block's end raises a confirm dialog on the blocks screen INSTEAD of the save: every path through it completes the save, and dismissing it abandons the edit with the form still open, so dates are never stored beside a question the coach walked away from. **One question at a time** — extending asks whether to carry the plans on at all (*Extend the plans* / *Just the dates*), and only then how to price the targets (*Keep the same targets* / *Recalculate*, skipped when the block provably has no nutrition prescription). Nothing extends unless the coach says so; a single row of buttons that all extended the training made the choice unaskable, because the dialog had to claim the workouts carried on either way directly under a sentence saying they stopped. Shortening is one question (*Clear those days* / *Just the dates*). The dates land through the chain PUT and the calendar through `POST /api/clients/[id]/blocks/[blockId]/events` (`services/block-event-sync-service.ts`) — **sequential, not atomic**: one toast at the end, and the failure between them says the dates saved and the calendar did not. The sync writes `training_events` and moves both tracks' windows — which is what the nutrition days are computed from — so the call site invalidates the training and nutrition SWR areas as well as the blocks area. It RECONCILES rather than replaying a diff — `fill` covers the block, `clear` removes the scheduled sessions that now sit outside it and pulls both tracks' windows back to it — so it needs no memory of the old window and re-running it changes nothing. **The fill runs in three steps:** an optional headless re-price mints a nutrition version first, the training extension runs second, and the nutrition fill last. The re-price takes effect **tomorrow** for a running block (its own start for a future one) — never today, because the client may already have eaten against today's target; that save resolves its end against the re-dated block, so the version already reaches the block's end and the fill behind it leaves it alone. "Keep the same targets" changes no numbers: it moves the stored end of the version laid in the block to the block's new end (capped at the next queued version), and since every day is computed from the version, moving the end IS the fill; a block holding no version returns null and the coach is told to set targets. Shortening pulls every version reaching past the new end back to it and retires a queued version that now starts in the cleared stretch, so the days a version answers for always end where its window does. The extension CONTINUES the placed program's repetition using `training_plans.authored_slot_count` (migration 165) — resuming at `placedSlots % authoredSlotCount` rather than restarting the pass — and returns null for a block with no program or a plan placed before that column existed, where the coach is told to place one rather than handed a guessed program. Clears are bounded by the next block's start, floored at the shared deletion floor (below) and scoped to `status = 'scheduled'`, they deactivate the training slot rows behind the events they remove, cap every live program reaching past the new end at it and archive one queued to start in the cleared stretch (migration 167), beside the same two acts on the nutrition versions. The extension moves the program's stored end to the block's new end, capped at the day before a program queued inside the block, and lays its new sessions from the shared deletion floor, never onto a day the client has logged.
- **Delete = remove the row, move nothing** (`DELETE /api/clients/[id]/blocks/[blockId]`), unless the coach asks for the plans too. With windows chosen rather than chained there is nothing to re-anchor: the row goes and every other block stays where the coach put it, leaving a gap — a legitimate state. No mode, no realized `changes`, no consequence sentence to preview. Elapsed blocks 422 (archiving is how a finished block leaves the list). The block's program and targets are **deliberately left alone**: a block edit writes nothing on its own. **`?clearPlans=true` is that separate explicit act**, and it is SCOPED TO THE BLOCK'S OWN WINDOW: the nutrition versions laid in it (`clearNutritionPlansForClient` with the block window — the same function the nutrition calendar's own delete calls without one), the programs placed in it (`clearTrainingPlansForClient` with the block window — likewise the training calendar's own delete), then the block. **A later block keeps its own program and its own targets.** Both tracks answer "does this plan belong to the block?" from DATES alone, and correctly: a placement truncates a program to the block it is placed in, and a version's end is resolved to the block its start falls in, so both windows line up with the block's by construction — which is why no `block_id` is needed. A plan that merely CROSSES the block belongs to an earlier one and survives; the coach removes it from its own calendar. The two clears are independent — each ends or archives its own plans and writes nothing the other reads — so nutrition-then-training is the dialog's order, not a dependency. **Clearing the block's own days and leaving the plans standing is not an option**: a nutrition day is computed from the version covering it, so a version left standing still answers for every day in its window, and a program's upcoming sessions travel with the program. A block holding no plans is not an error — the coach asked for its plans to go and it had none. Any real failure stops before the block row is deleted, because losing the label while the prescription survives is the one outcome the UI cannot undo.
...
- **A setup surface reached from a block seeds its date picker with that block's start** (`useRoundTripBlockStart`, read through `readJourneyTrip` so the round-trip contract keeps one owner): the coach has already said which days they mean by clicking "place one" / "set targets", and typing the date again is asking them to repeat themselves. A SEED, never a binding — their own pick still wins, and it is floored at the shared deletion floor (see "The deletion floor" under Nutrition & Training Events) because both the placement and the nutrition save refuse a start before it, so a block already under way seeds today, or tomorrow once the client has logged today, rather than the day it began. **Route surface:** `GET`+`PUT /api/clients/[id]/blocks`, `DELETE …/blocks/[blockId]` — full coach chain (`coachApiRateLimit` → CSRF → `requireCoachOwnsClient`, foreign client 404), canonical kg on the wire, audit events `block.chain_update` / `block.delete`. The chain payload carries `clientToday` and `planStartFloor` — that deletion floor, resolved server-side because it depends on the client's logs — on every handler that echoes it (GET, PUT, PATCH, DELETE), because `useSeedClientBlocks` writes a mutation's response straight into the chain cache; both setup surfaces floor their date pickers on it, and a block itself is not constrained by it. RLS deny-all + `GRANT … TO service_role` only (CONVENTIONS §8). Plus the read-only **`GET …/blocks/facts`** (Session 3.2, `services/client-blocks-facts-service.ts`). **A block reports what is set only if it actually HAS DAYS inside it** — per track: on training, gated on the events, because a window says where a plan was placed to run and the events say which days it actually put on the calendar; on nutrition, on the computed days, which exist exactly where an active version covers a date — the same question asked of the day reader's output, so the gate stays one rule for both tracks and a block the coach has drawn but not set up never claims a program whose rows merely reach into it. The gate is bounded at both ends of the block (days after it are not its days), status-agnostic (a completed day is still a day) and NOT clamped to today (a current block whose program starts tomorrow has days). It is per-track because the two are set up separately: a block can hold workouts and no targets. Given days: per-block training programs (governing segments, latest-start-wins) + the nutrition PRESCRIPTION (owner-specified semantics, 2026-08-12, superseding the original events-modal): the plan VERSION covering the block's reference date — today for a current block, its final day for a past one, its first day for a future one — supplies daily calories (custom-macros override honoured), deficit (`tdee − calories`) and its own start (`startsOn`, rendered "from <date>" like the training column); hand edits and training surpluses are excluded by construction, era-resolution over the version windows. The changed-mid-block marker is read off the computed days (a baseline transition across consecutive unedited lived days — under computed days, a version boundary inside the block). **The "what happened" timeline renders `eras` instead**: one entry per plan VERSION that governed part of the block, each carrying the numbers off its OWN row. The version windows never overlap (the gist exclusion) and may leave gaps, so this is a plain intersection with the block window — no resolution rule, unlike training. It must not reuse the headline's calories/deficit: those read the reference-date version, so a later plan save would silently rewrite an entry dated in the past. A current block's eras stop at today (a queued change is a plan, not a thing that has happened yet); a future block's eras describe what is planned for it, and its timeline lists the queued prescription as "Nutrition set" exactly as it lists a queued program as started. An era matching the previous one's numbers is omitted. The training-dates read is **paged** (`lib/paged-fetch.ts`) — an unpaged read silently truncates at PostgREST's ~1000-row cap — and the nutrition days come from the day reader, one per date an active version covers. Kept off the chain GET so PUT/DELETE keep echoing that GET's exact payload.
...
- **`training_events.training_plan_id` is `ON DELETE SET NULL` + nullable** (migration 113), so a past or logged event survives its plan's hard-delete and is never orphaned by it. `nutrition_plan_notes.nutrition_plan_id` takes the same posture (migration 147).
...
> **One "Ended" definition (migration 167).** Every reader takes a program's end from the row: the client reader's `endsOn` and state, the amendment's `isFullyPast`, the attention feed's windows, the block facts and the nutrition placement's program fallback. The Overview's week counter counts from `program_duration_weeks`, the placed length, which placement records from the same window.
...
- **Three statements, one per act** (`services/nutrition-day-edits-service.ts`): a paged range read (the block facts read a whole journey span through the day reader), one upsert on `(client_id, date)` for every day of an edit (re-editing a day replaces its row in place and keeps its id), and one delete for every day of a reset, which returns how many days actually held one.
- **An edit is resolved against the day as computed.** `materializeNutritionEventDays` (`services/nutrition-event-edit-service.ts`) reads the computed days once over the selection's span and writes only the selected dates: an absolute edit stores the coach's calories (explicit macros win; otherwise protein is held and carbs and fat rebalance to the new total), a %-delta scales the day's displayed calories — the surplus stacked on the baseline when one exists, else baseline plus the legacy burn — floored at zero, and the split is scaled rather than protein held only when the coach says so. A note-less edit keeps the day's standing note; an empty string clears it. A selected day no version covers has no computed day and is skipped.
- **An edit on a day no version covers is dormant, not deleted**: the day reader yields no day there, so the row answers nothing until a version covers the date again.
...
| `nutrition_plan_notes` (mig 147) | client-visible **while its block is current**; the coach's calendar on its date and the Journey timeline, forever | plan-level, append-only | the plan-save note |

The second row's qualifier is the point, not a footnote. **The coach's Journey timeline renders every note forever; the client's Program tab renders only those inside their CURRENT block card.** Two surfaces, two lifetimes, one store — and the coach's calendar reads that same store by date: the computed day carries the note dated that day as `coachNote` (the newest when a date holds two), and nothing stamps it anywhere else. Every piece of coach-facing copy about this note — the builder drawer hint, the calendar popover — is worded against that difference and must stay that way: they name the condition ("it shows on their Program tab with the block it falls in") rather than asserting an outcome ("shown to Sam"), because a coach who believes the stronger claim writes an explanation that reaches nobody.
...
- `client_id` FK (CASCADE), `coach_id` FK (SET NULL), `nutrition_plan_id` FK (**SET NULL**), `effective_on` DATE, `body` TEXT, `created_at`. **No `updated_at`** — an immutable event table (CONVENTIONS §8); an `updated_at` would imply an edit path the append-only property forbids.
...
- **Client-scoped with a SET NULL plan FK because a version is not a stable anchor:** `nutrition_plans` rows are capped, replaced in place and archived by the save and delete paths, and a note is the history of the coach's decisions whatever became of the version it was written against. Same posture as `training_events.training_plan_id` (migration 113).
- RLS enabled with **no policies**, `GRANT ALL … TO service_role` only (CONVENTIONS §8). The `nutrition_plan_id` FK is indexed; a delete archives a version rather than removing it, so the index serves the seed teardown's cascade and any future removal.
- **Write path:** `recordPlanSaveNote` (`services/nutrition-plan-notes-service.ts`), called by the orchestrator after the RPC succeeds: one INSERT, dated the day the change takes effect. The failure is not swallowed — the orchestrator surfaces it as a `NutritionPlanError` saying the targets were saved but the note was not — and the retry is safe: the insert that failed wrote nothing, and the RPC replaces a version starting on the same day in place, so no second version is minted.
- **Read paths:** the day reader (`listNutritionPlanNotesInRange`, paged — the coach calendar's `coachNote` by date), `GET /api/clients/[id]/blocks/facts` (one of its parallel reads over the journey span, partitioned per block in memory) and `GET /api/client/journey` (the current block's window only).
...
- **Generated** at placement (the date-walk), by the amendment's rewrite of the future, and by the block extension
...
- **Per-day nutrition editing**: coaches edit computed days on the nutrition calendar over a `dates[]` selection (absolute or %-delta) via `PATCH /api/clients/[id]/nutrition/events/range`, which resolves the coach's numbers against each selected day as computed and writes one `nutrition_day_edits` row per day (`materializeNutritionEventDays`; the editing coach is stamped as `coach_id`); `PATCH …/nutrition/events/reset` (`resetNutritionEventDays`, over the same `dates[]` selection) deletes those rows and the plan's own numbers answer again — its response counts the days that held an edit. Both are server-guarded to `date >= clientToday`. The read is `GET …/nutrition/events?startDate&endDate`, the computed days over the range.

### The deletion floor

**One question, one answer: from which day may a client's training sessions be REMOVED, and from which day may a plan on either track START?** `resolveEventDeletionFloor` (`services/event-deletion-floor.ts`) is the only place it is answered — their today, or **tomorrow** if they have already touched today.

- **Replacing today is always fine; emptying it is the harm.** A placement overwrites the day in the same breath it clears it; an emptied day costs the client their session for the rest of it. So the training removals ask this, and so does every plan START (next bullet). Nutrition days are computed from the versions and removed by nothing, so on that track only the START asks it.
- **A plan may not START before it either — one function, both directions**, so a day the client has touched can be neither emptied nor re-prescribed. `POST …/place-from-library` (both program branches) and the nutrition save (`orchestrateNutritionPlanCreation`, the default start included) refuse a start before the floor with a 400 naming the client, the day they logged and the first day a plan can start; both date pickers floor on the same value (`planStartFloor`, on the blocks payload) and say why under the field; the block extension lays its new sessions from it. There is no override: the earlier warn-and-override on the training placement wrote the program's first session beside the one the client had already logged (the walk's upsert arbitrates on `(client_id, training_session_id, date)` and the completed event belongs to another session row), and the check-in counted the pair as a missed session. The union is deliberately over-strict in this direction too: a food log blocks a training start and a logged workout blocks a targets start. One consequence on the nutrition track: a same-day re-save is possible only while today is unlogged; afterwards the earliest start is tomorrow and the RPC caps the running version at today.
- **"Touched" is one question with two answers**, because the tracks record it differently: a `nutrition_logs` row for the date, or a `training_events` row for the date that has left `scheduled`. A computed nutrition day has no status to ask.
- **It fails CLOSED.** Either read erroring returns tomorrow: a day skipped is a stale row the next removal clears, a day emptied is not undoable.
- **A deleted plan governs nothing from today.** The delete ends the running version, or program, at yesterday and archives queued ones; every read resolves by date, so the hero, the Overview and the client's Program tab read "no plan" the moment it lands, while the past keeps its plan — and on the nutrition track that IS the removal of the days, since nothing stores one.
- **Callers, and they do no arithmetic of their own:** the block shorten (`clearScheduledEvents`), the training-plan clear (`clearTrainingPlansForClient` — the calendar's delete and the block delete's "and its plans") and the per-plan `DELETE /api/clients/[id]/training/[planId]` — plus, in the other direction, the two start guards (`place-from-library` and the nutrition orchestrator), the block extension's lay-from, and the blocks payload that carries the floor to the pickers. The nutrition clear does not ask it: nothing removes a nutrition day. A removal that starts at the floor needs **no second "skip the day they logged" filter** — that day is never in range, and adding one would defend a state that cannot occur.
- **The floor is the UNION across both tracks**, so a client who logged only food today keeps today's scheduled workout through a plan clear. Conservative by design; a single session is still removable on the calendar.
...
Inside a block the two tracks end on the same day by construction, because the placement window is that same block (below). They diverge only where the rows and the block disagree — a program placed before its block existed, or one whose block was re-dated afterwards without the calendar being brought along, since a block edit still writes nothing on its own.
...
- the **surplus** is the first session's `training_events.calorie_surplus_percentage` on the date — read off the event, never through the session FK — and the burn is that share of the baseline; with no percentage, the legacy flat sum of the sessions' `estimated_calories`;
...
- `coachNote` is the plan-save note dated the day, the newest when a date holds two;
- `dayOfWeek` comes from `nutritionDayOfWeek`, the ONE weekday spelling a caller picking a grid row must use.

The DTO is `NutritionEvent` (`types/check-in.ts`): `id` is the date (stable and unique per client — the edit routes address days by date, never by id), `nutritionPlanId` is the covering version and never null, and `status` is always `scheduled` (a computed day has no lifecycle; the field survives because the coach calendar's edit gates read it). Every day from the client's today onward is editable, and a coach edit on a day the client has already logged reaches them at their next food save, which re-snapshots the target. "Logged" for nutrition is the existence of a `nutrition_logs` row for the date (`getNutritionForDate`).

**The reader is batched, never per day.** `getNutritionEventsForDateRange` / `getNutritionEventForDate` (`services/nutrition-days-service.ts`) issue five reads for any range — the active versions overlapping it with their prescription (`getNutritionPrescriptionsForRange`), their grids (`getNutritionPlanGrids`), the training events in range (`getEventsForDateRange`, status-agnostic), the edits (paged) and the plan-save notes (paged) — then hand every date to the resolver with the version covering it. A date no active version covers yields NO day, which is what a gap between plans is: the client's food log is refused there (`assertHasActivePlan`). A single day is the range reader over one day. A month view is therefore a few small reads rather than one and a client's day a few point reads — collapsible into one RPC later if measured (owner, 2026-09-10).
...
**A logged day is re-snapshotted, by design** (owner decision 2026-08-26): `upsertNutritionLog` re-snapshots the day's target from the computed day on **every** food save, so a session moved onto or off a day — a coach move, an amendment, or the client's own move — changes the target the client sees at their next save, and the coach calendar reads the computed day. Freezing a logged day was designed and rejected: a frozen target keeps a training surplus on a day whose session has left.
...
1. **Logged days**: the `nutrition_logs` snapshot (written at log time and re-snapshotted from the computed day on every food save; authoritative)
2. **Unlogged days a version covers**: the computed day (`getNutritionEventForDate`, via `mapNutritionEventToDisplayTarget`, honoring `include_activity_burn` + `surplus_as_carbs`)
3. **Days no version covers**: `null` — a gap between plans, or the stretch a delete opened; there is no target and the client's food log is refused there
...
**The program fills its block** — a sequential date-walk, not a weekday map. `placePlaceablePlanOnCalendar` (`services/library-placement-service.ts`) builds `programSlots` = ALL sessions across ALL weeks sorted by `(week_index, order_index)`, then:

1. **Computes the window first** — `resolvePlacementWindowEnd()` (`services/program-event-walk.ts`): **the last day of the block covering the start date** when one covers it (`getBlockBoundForDate` answering `covering`), else `max(1, programSlots.length)` days (one pass of the authored program) capped at the day before the NEXT block when the start is in a gap (`getBlockBoundForDate` answering `next` — a cap, never a length, so a program placed in a gap stops at a block the coach has not set up rather than filling the gap or running into the block); either way **capped, never stretched**, by `getNextPlanStartCap` (the start of the next coexisting plan). **The block is the length knob**, so a program shorter than its block repeats to fill it and a longer one is cut at the block's end. A placement on a day no block covers and no block follows is unchanged from before blocks bounded anything.
   > The window is decided here and stored on the row (`effective_until`, migration 167); nothing derives an existing program's end from its rows. `resolveWindowCap` (same module) is the one bound placement and the amendment share: the covering block's end, else the day before the next block, either capped at the next plan.
2. **Expands to the window, then clones EVERY slot — training and rest.** `expandProgramToWindow` repeats the authored program until it covers the window and cuts a final partial cycle mid-program; each cycle's `weekIndex` is offset by the authored program's own week span so `(week_index, order_index)` keeps climbing, and a single pass is byte-identical to the authored program. **Cloned, never shared** — every cycle gets its own `training_sessions` rows, so cycle three can be progressed past cycle one; sharing would make one edit rewrite every cycle at once. Rows carry `day_of_week: null`, `week_index`, `is_rest`; rest rows are forced to name "Rest", null focus, null surplus, and carry no exercises. Exercises clone only for non-rest slots, splatting `set_specs`/`video_url` verbatim.
   > **The inserts are BATCHED and chunked**, because a one-week program in a 52-week block is 364 slots and a round trip each would take the placement past any request budget. Exercises are keyed back to their session on `(week_index, order_index)` rather than on the returned row order — Postgres does not promise `RETURNING` follows the `VALUES` order, and mis-linking would be silent. That makes the pair load-bearing, so placement **refuses a template carrying two slots at one coordinate** rather than handing one row both slots' exercises.
   >
   > `training_plans.program_duration_weeks` records the **placed** length, not the authored one — the Overview's plan chip derives its "Ended" date from that column and would otherwise contradict the calendar.
...
>
> **A program cannot start on a day the client has already logged.** `POST …/place-from-library` (plan and inline branches) refuses a start before the shared deletion floor — the client's today, or tomorrow once they have logged anything today (`resolveEventDeletionFloor`, see "The deletion floor") — with a 400 naming the client, the day they logged and the first day a plan can start, after the same past-date guard judged against the client's local today. `ApplyToClientDialog` floors its picker on the same value (`planStartFloor`, on the blocks payload; the timezone-derived today stands in until it lands), opens on it, and says why today is greyed under the field. There is no override: the earlier warn-and-override placed the program's first session beside the one the client had logged that day, and the check-in counted the pair as a missed session.
>
...
- **The rewrite** (all steps idempotent-or-compensated): insert fresh future rows first (verbatim `set_specs`/`video_url` splat, foreign-exercise-id belt) → soft-delete replaced rows, KEEPING elapsed positions and any row referenced by a surviving event (the belt that makes diverged plans safe) → delete future **scheduled** events under both placement-mirror predicates (plan-scoped unbounded + client-scoped window; **no `is_modified` filter** — manually-moved future events are re-laid, behind an explicit pre-save warning) → resume the shared date-walk (`services/program-event-walk.ts`, `startPosition: offset`) so every regenerated event carries its surplus/name snapshots → plan meta (`program_duration_weeks`, re-derived `frequency_per_week`, `updated_at` bump, and `effective_until` = the last day of the new grid — the window follows the rows, migration 167). The grid may keep the end it has but may not GROW past the cap placement takes (`resolveWindowCap`: the block the program was placed in, else the next block, either capped at the next program); a grid reaching past it is refused with a 422 naming the date, and the GET carries the cap as `windowCap` so the editor says so above the grid (owner, 2026-09-10). A mid-flight failure restores sessions, exercises, and the event snapshot, preserving the root error. Success records a `training_plan.amend` audit event; the nutrition days re-price themselves from the re-laid sessions on the next read.
...
left); otherwise, the reading as of the check-in's day 3 kg or more from the base weight of the
...
(a logged day's frozen target, else that date's computed day; a day no version
