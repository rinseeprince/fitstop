...
- **A retired version governs nothing.** The nutrition plan delete archives the versions whose days it removes (migration 166); every read resolves among active rows, so no cascade can regenerate a date the delete cleared.
