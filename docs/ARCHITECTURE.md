...
- **The builder draft holds groups** (`SessionDraft.groups: ExerciseGroupDraft[]`). A picked exercise joins as a straight-sets group of one; removing a group's last exercise removes the group (`normalizeDraft` drops an empty group too); a move never splits a group — a lone exercise's group moves to the first group boundary at or after the target, which for a session of lone exercises is exactly an array move, and an exercise that shares its group moves within it (`moveSessionExercise`, `program-builder-model.ts`). Every serializer emits `groups` (`groupDraftToInput`), and every write schema requires them (`savedExerciseGroupsSchema` / `bulkExerciseGroupsSchema`, `lib/validations/training.ts`: a group holds at least one exercise, a format is required, at most 50 exercises across a session's groups).
- **The assistant keeps them.** Its draft snapshot schema carries every group with its settings, so a session it copies or places from the server's working copy is built from groups the schema kept; `add_exercise` carries a fully materialized straight-sets group of one with a server-minted uid; its exercise patch schema is strict and names no group field. Positions it speaks (`exercisePosition`, `toIndex`) count a session's exercises in order, group by group.
