...
- **The date is set at activation.** The coach's picked date wins (the dialog prefills their today and they may backdate it); an existing stored date is kept rather than overwritten; the client's timezone today is the last resort. Corrected afterwards in one place — the Client & Schedule card's "Started" field, which routes through `updateClient` → `recordClientStart`.
