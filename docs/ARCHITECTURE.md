...
A single pure resolver, `resolveEffectiveGoal()` (`lib/goals/resolve-effective-goal.ts`), turns the live `client_goals` record into the goal that drives nutrition + pace. It is the **one** place display-unit goal weights are normalized to kg; a NULL weight means **maintenance** (`goalWeightKg: null`). `startDate` falls back to `today` when `client_goals.goal_start_date` (migration 104) is unset. Callers: `services/nutrition-plan-orchestrator.ts` (plan creation), `services/comparison-service.ts` (check-in weight pace — weight **and** deadline come from one scope), the nutrition GET's goal-drift check, the coach Metrics page, and the coach **Overview** (`components/clients/client-overview-tab.tsx`, via `hooks/use-client-goals.ts`).

**Coach-side reads go through `client_goals`, not the `clients.*` mirror.** The browser reads it through `useClientGoals` (`hooks/use-client-goals.ts`), which co-locates the key builder with `useInvalidateClientGoals` per CONVENTIONS §7 — any writer of `client_goals` must call that invalidator, since the goal is read by more than one component. **Still on the mirror, deliberately:** the whole client portal (`services/client-portal-progress.ts`, `services/client-portal-service.ts` → `components/client-portal/metrics/goals-section.tsx`) — `/api/client/**` has no goal endpoint at all, and moving it is its own task. **That is now the only mirror reader.** `hooks/use-nutrition-plan.ts` and `hooks/use-nutrition-builder.ts` were the other two and came off the mirror in task 2.4: `use-nutrition-plan.ts:150-156` resolves through `useClientGoals` + `resolveEffectiveGoal` and keeps `client.goalWeight` only as a trailing fallback, and `use-nutrition-builder.ts:222-224` reads the already-resolved `effectiveGoalWeightKg` rather than the client prop. *(Corrected 2026-07-29 — this sentence still listed both hooks as mirror readers "with no `client_goals` fallback", which had been backwards since 2.4.)*

**The cost of that remaining mirror is not theoretical.** A stale mirror makes the coach's Overview and the client's own portal report different goals for the same client, with nothing on either screen indicating disagreement — measured live on 2026-07-28, where the mirror read 77 kg / 33 % against `client_goals`' 90 kg / 9 %. Until the portal moves, treat any mirror-vs-`client_goals` divergence as a live user-visible bug, not as tidy-up.

### Goal rate arithmetic (`lib/goals/goal-rate.ts`)

The deadline-driven calculator (`calculateBaselineCalories`, `services/nutrition-service.ts`) takes a target weight plus a deadline and derives the weekly rate internally. `lib/goals/goal-rate.ts` is the **other direction** — rate → daily calorie delta → calorie target — plus the two-way `deriveRateFromTarget` / `deriveDeadlineFromRate` pair. Pure, no I/O, browser-safe.

- **The gender safety caps (0.75/1.0 kg-wk loss, 0.35/0.5 gain) and the calorie floor (1200/1500) live here once**, in `capWeeklyRate` / `applyCalorieFloor` / `getRateSafety`, and the calculator calls them. They were inline magic numbers duplicated across its cap and floor branches. `CALORIES_PER_KG` (7700) moved to `lib/constants.ts`. An unset `gender` resolves to the **male** envelope — pre-existing, preserved, and now asserted rather than implied.
- **Caps and floors are reported structurally** (`capped`, `capKgPerWeek`, `floored`, `floor`) as well as in the existing warning strings, which a per-row preview cannot get from a flat `string[]`.
- **Dates are day-number arithmetic** (`dateStringToDayNumber`), and emitted through `addDaysToDateString`; `today` is a **required parameter** with no `new Date()` fallback. This module is destined for the coach's browser while its dates sit on the *client's* calendar. The calculator's own date math parses `today` as local midnight against a UTC-midnight deadline and leans on `Math.round` to absorb the offset — correct on a UTC server, off by one at +12 and beyond. That arithmetic is **deliberately not changed** (exec-plan "already true, do not rebuild") and deliberately **not copied**; `goal-rate.test.ts` pins both facts across six zones.
- **Both directions now report the rate the target actually delivers.** `calculateBaselineCalories` used to return a *pre-floor* `weeklyRate` beside a *post-floor* `baselineCalories`, so a floored plan advertised a deficit it would never run — at TDEE 1700 a 1500 target runs 200 cal/day while the reported −0.385 kg/wk implies ≈423. Task 1.4 pinned that asymmetry rather than fixing it; **rider 2.8(d) fixed it** (`services/nutrition-service.ts:184-200`): when the floor bites, the rate is re-derived from the floored target via `rateFromDailyCalorieDelta`, mirroring what `calculateBaselineCaloriesFromRate` already did with `appliedRateKgPerWeek`. Unfloored inputs are untouched, which is why most of the golden matrix in `services/nutrition-service.caps-floor.test.ts` is unchanged. *(Corrected 2026-07-29 — this bullet still described the asymmetry as "pinned, not fixed" after 2.6 had fixed it.)*
- **No stored rate column exists, deliberately.** The goal stores target + deadline; a stored rate would let all three disagree.

**`clients.goal_deadline` is unreachable *through the mapper*, but the column is still read.** `mapClientRow` (`lib/mappers.ts:72-73`) maps only `goalWeight` and `goalBodyFatPercentage`, so `client.goalDeadline` is **always `undefined`** and every `?? client.goalDeadline ?? null` fallback in the `resolveEffectiveGoal` call sites is dead code. **The column itself is not dead**, though: `services/intake-review-service.ts:114` selects it raw and `:156-163` uses `client.goal_deadline == null` as a backfill guard — so it must keep being written (`update_client_goals_atomic`, migration 139, writes all three mirror columns for exactly this reason). *(Corrected 2026-07-29: an earlier revision of this line, and Task 1.3's STATUS block, both called the column flatly "unreachable". The mapped property is; the column is not.)* Do not add it to the mapper to "fix" a deadline read — read `client_goals`.
...
### client_phases table (migration 137)

The client's calendar cut into named stretches, each carrying a **rate**. The product word is **block**; the table keeps the older `phases` noun. A client with **no rows behaves exactly as before blocks existed** — that fallback is the whole reason this is safe to add to live clients.

- Columns: `client_id` FK (CASCADE), `name`, `starts_on`/`ends_on` (DATE), `rate_per_week_kg` (NUMERIC NOT NULL), `daily_targets` (JSONB, NULL until a plan is generated), `created_at`/`updated_at`. Index `(client_id, starts_on)`; RLS enabled with **no policies**, `GRANT ALL … TO service_role` (CONVENTIONS §8).
- **A block stores a rate and nothing else.** No target weight, no type enum, no `duration_weeks` (derive it from the dates), no `position` (order by `starts_on`), and **no status column** — "active" is derived from today against the dates, so a block cannot be marked active while its dates say otherwise. **No other table carries a `phase_id`**: nothing is stamped with the block that produced it, which is what lets a block be re-dated without orphaning rows.
- **No `UNIQUE (client_id, starts_on)`, deliberately.** A non-deferrable unique index is checked **per row**, so the delete-and-shift-back rewrite — several `starts_on` values whose *final* state is unique — fails the moment one shifted row transiently collides with a not-yet-shifted one, in an order Postgres does not define. Deferrability belongs to a CONSTRAINT; a plain or partial unique INDEX can never be deferred, so there is no safe variant. Overlaps are already structurally impossible: the client sends **lengths**, and the server chains them.
- **`daily_targets` is a 7-row weekday grid**, five keys per row (`day_of_week`, `calories`, `protein_g`, `carb_g`, `fat_g`) — the read subset `generateNutritionEvents` consumes, **not** the plan RPC's write shape. It deliberately omits `is_training_day`: that is a per-DATE fact from live training events and a weekday-keyed grid cannot represent it. Written by plan creation (see "Training → Nutrition cascade"), and **cleared by any edit that moves the window or the rate** (`carryDailyTargets` keeps it across a rename and nothing else) — which is what makes "non-null grid" a trustworthy promise that the grid matches its window.
  - **A browser client must resend an untouched rate as the STORED kg, byte-for-byte.** `carryDailyTargets` compares rates with `===` and `computePhasesFingerprint` hashes them, while a coach-facing form necessarily shows the rate in the client's display unit at 2dp — and kg → 2dp display → kg does **not** round-trip (−0.6 kg shows as −1.32 lb/wk and returns −0.5986…; `roundTo` alone loses a stored −0.625 as −0.62 even in kg). Because the `PUT` replaces the whole chain, a re-converted rate turns a pure rename into a numeric edit on **every** row: all the grids null, the fingerprint moves, and the per-date resolver falls back to the plan grid for every in-block date — the flattening the per-date generator exists to prevent. `toPhasesBody` (`components/clients/goal-plan/goal-plan-model.ts`) owns that lookup and is the reason the panel's rename is inert. Raising the display precision narrows the gap and never closes it.
- **The pure half lives in `lib/goals/phase-chain.ts`, not the service.** `chainPhases`, `getPhaseForDate`, `isPhaseElapsed`, `lastPhaseEnd` are synchronous and browser-safe, so a coach's live preview and the stored chain run the same functions and cannot disagree about where a block lands. `services/client-phases-service.ts` owns the DB half; `services/phase-fingerprint.ts` is server-only because `createHash` has no browser form. The split is "can the browser run it", not tidiness. **`getPhaseForDate` is list-based, not a query** — the per-date resolver calls it once per generated date, so callers load the blocks once and pass the array (CONVENTIONS §2: no query inside a per-item loop).
- **Bounds live in zod, not CHECK constraints** (migration 131's rule): `MAX_PHASE_WEEKS = 52`, `MAX_CHAIN_WEEKS = 104`, `MAX_PHASES = 12`, `MAX_ABS_RATE_KG_PER_WEEK = 5`. That rate bound is a sanity bound and **not** the gender safety cap — the rate the coach entered is stored verbatim and the cap surfaces in the preview, so clamping at the boundary would make that impossible. `MAX_CHAIN_WEEKS` is what bounds the nutrition horizon's worst case.
- **`updated_at` is stamped in app code on every write** — migration 137 has no trigger (the 132/134 posture; the last `update_updated_at_column()` trigger was migration 096). Omit the stamp and the column freezes at insert time, the exact defect migration 096 had to fix for `exercises`.
- Routes: **`GET`/`PUT`/`DELETE /api/clients/[id]/phases`**. `PUT` replaces the whole chain from `{ startDate, phases: [{ name, weeks, ratePerWeekKg }] }` — **lengths only**, and the schema is `.strict()`, so a client that tries to send date pairs gets a 400 rather than a chance to author an overlap. `DELETE { phaseId }` **re-chains rather than wiping**: earlier blocks keep their dates, later ones close the gap, and the response names the blocks whose dates moved so a confirm dialog can state the consequence before the coach commits. `GET` also returns a server-computed **`nutritionStale`** boolean (see `phases_fingerprint` below) rather than leaving the browser to re-derive the comparison and forget its custom-macros half. Writes are audited as `phase.update` / `phase.delete`.
- **Elapsed blocks are excluded from the coach-edit write entirely**, not rewritten with identical values, so no `PUT`/`DELETE` can touch them; removing or re-dating one is a 422 naming the block. The guard runs against the **client's** today (`getClientTodayString`), never the coach's device date, and against the dates `chainPhases` **computes** — validating the payload would miss the real case, where shortening an earlier block silently drags a finished one backwards. **Plan generation is not bound by that guard**, and the distinction matters: it recomputes and upserts `daily_targets` for *every* block including elapsed ones, moving their `updated_at` with it. So `client_phases.updated_at` is **not** a "was this finished block ever touched" signal. Harmless today, because an elapsed block's grid is never read back — regeneration is forward-only from the client's today — but a past-block readout must not assume it is showing the numbers the client actually ate.
- **This route never calls `updateGoals`.** Blocks save independently of the goal; `updateGoals` supersedes-and-inserts on every call, so routing a block edit through it would mint a goal version every time a coach renamed a block. Asserted structurally in the route's tests, because the coupling would be silent at runtime.
- **The `anon` / `authenticated` GRANTs Supabase's default privileges add on `CREATE TABLE` are present here** as they are on `client_metric_entries` and `client_notes`. Not exploitable — RLS is on with zero policies — but note that **RLS is the sole perimeter**; the "no privilege at all" belt CONVENTIONS §8 describes does not exist for any new table on this project. Verify with `npm run check:rls`, never from the docs.
...
The single durable `nutrition_plans` row per client is the **template** that generates `nutrition_events`. It holds the plan-level prescription — `baseline_calories`, `protein_target_g` / `carb_target_g` / `fat_target_g`, `diet_type`, `protein_target_g_per_kg`, the custom-macros override (`custom_macros_enabled` + `custom_calories`/`custom_protein_g`/`custom_carb_g`/`custom_fat_g`), the calculator inputs (`base_weight_kg`, `bmr`, `tdee`, `work_activity_level`, `training_volume_hours`), and the goal snapshot (`goal_weight_kg`, `goal_deadline` — `goal_source` was **dropped by migration 133** and is not a live column; it recorded which scope drove the calculation, and with one scope it was a constant) that drives the "active since {date}" / weight-drift banners. **Blocks did not bring `goal_source` back** (owner decision, 2026-07-29): the removed feature gave a phase its own goal columns, whereas a block carries a rate and no target weight, so the goal snapshot still resolves from `client_goals` in every case and the column would be the constant `'client'` that got it dropped.

**`phases_fingerprint` (TEXT NULL, migration 138)** is a hash over each block's `(starts_on, ends_on, rate_per_week_kg)` at generation time — **stale means current ≠ stored**. Renaming a block does not change the hash, so a rename raises no false alarm. It is stamped as the **LAST write of a generation** (grids → plan → events → fingerprint), which is the only ordering that lets the stored value mean *"every write above succeeded"*: every failure direction then leaves it stale — a visible "out of date" that clears on the next regenerate — rather than asserting a block set the client's events do not actually follow. It is deliberately **not** an RPC argument; an RPC that omits it under `DEFAULT NULL` would wipe a stored hash into the *silent-miss* direction, and the stamp has to outlive event generation, which needs the plan id the RPC returns. `preserveCalories` and custom-macros saves stamp **NULL** — neither ran the calculator, so no block drove them. The staleness rule itself has one home, `isNutritionStaleForPhases` (`services/nutrition-plan-service.ts`): no active plan and custom-macros plans are **never** stale (the coach typed those numbers by design and a permanent nag could never be cleared), and a read failure returns `false`, since a transient DB error must not manufacture an alarm. `nutrition_plan_daily_targets` is its **per-weekday grid** (`(nutrition_plan_id, day_of_week)` → `calories`, `protein_g`, `carb_g`, `fat_g`) — the source used to generate/regenerate the dense per-date `nutrition_events`. It is replaced **DELETE-then-INSERT** on each plan upsert and is **not** the display SOT (events are). The TDEE/macro formula that computes these values is implemented in `utils/nutrition-helpers.ts` (its calculator doc was retired — git history of `docs/NUTRITION_PLAN_CALCULATOR.md` holds the prose walkthrough). The plan is edited **in place** (`create_nutrition_plan_atomic` upsert, migration 115): a cascade/in-place regen (`p_recalc_snapshots=false`) preserves the goal snapshot + `created_at`/`effective_from`; only an explicit "Recalculate plan" (`p_recalc_snapshots=true`) re-stamps them.
...
- **Cascaded** when training events change, via `regenerateFutureNutritionEvents` over an explicit `NutritionRegenScope` (see "Training → Nutrition cascade")
...
- The training event-write routes — `place-from-library` (3 branches), `events/[eventId]/move`, `events/[eventId]/duplicate`, `events/[eventId]` (PATCH/DELETE), `[planId]/sessions/[sessionId]` (PUT/PATCH), `[planId]/amendment`, `[planId]` and the client-level `training` DELETE — invoke one consolidated helper, `cascadeNutritionAfterTrainingChange()` (`services/nutrition-event-service.ts`), which fetches the durable nutrition plan and calls `regenerateFutureNutritionEvents()`. That is 12 call sites, not 8.
- **Each route threads a `NutritionRegenScope`, not an anchor date.** Two shapes:
  - `{kind:"dates", dates}` — **exactly** these days, upserted with **no DELETE**. Used by every route that knows its affected dates: move = `[source, target]`, duplicate/surplus-PATCH/event-DELETE/library-session-drop = one date, `sessions/[sessionId]` = the session's future scheduled event dates (a scattered set, reported by `updateSurplusForFutureEvents` / `replaceSessionFull.surplusAffectedDates`). Skipping the DELETE is what closes the window in which those dates had no row at all — and a missing row is not blank, it resolves to `null` through `getPlanTargetForDate` and is snapshotted permanently into `nutrition_logs`.
  - `{kind:"from", from, to?}` — open-ended forward. **The DELETE and the regenerate derive from one computed range**, `[from, max(from + 56d, last block end, to)]`. Used by whole-program placement; by the amendment, which passes its `windowEnd` (the amended program's real last day, returned by `amendPlacedPlanFuture`); and by the two plan-deletion routes, which pass the last date the cancelled plan actually reached. Those ends come from the amendment's own `calculatePlacementEndDate` and from the events `cancelFutureEventsForPlan` just deleted (it returns `lastDate`) — **never from `training_plans.effective_until`, which is NULL on placed plans** (migration 114), so reading it would collapse the range back to the default horizon on exactly the long plans that need it.
  - **Known gap (amendment):** if a coach *shortens* a program, days the old window covered past the new `windowEnd` are not revisited and keep a stale training-day surplus. Closing it needs the pre-amendment window end plumbed out of the writer; deferred by owner decision 2026-07-29.
- The DELETE was previously unbounded above (`.gte` with no `.lte`) while the regenerate stopped at the horizon, so every cascade silently erased the days beyond it and never rewrote them. Bounding it is why past-horizon rows now survive (possibly stale) rather than vanishing.
...
- Coach-edited (`is_modified=true`) days are immutable across the cascade: the `from`-scope DELETE filters them out, and `generateNutritionEvents` re-reads them (`.in("date", dates)`, client-scoped because the upsert's conflict key is `(client_id, date)`) and drops them from the upsert. **`status` is not part of that protection** — the upsert overwrites whatever sits on a date, and nothing in the app writes a `nutrition_events.status` other than `"scheduled"`, so the DELETE's status filter and the upsert are currently equivalent. Do not rely on `logged`/`missed` surviving a cascade until something actually writes them.
- Baseline is preserved across a cascade — only `calorie_surplus_percentage` and `is_training_day` change.
- **Plan creation calculates once per block** (task 2.6). `orchestrateNutritionPlanCreation` runs the **rate-first** calculator (`computePhaseTargets` → `calculateBaselineCaloriesFromRate`) once per block — a block stores a rate and nothing else — and writes each block's 7-row grid to `client_phases.daily_targets` in one batched upsert **before** the plan RPC. The response carries one row per block with its own cap/floor state, so a capped rate is visible per row rather than silently applied. Write order is **grids → plan → events → fingerprint**, and `stampPhasesFingerprint` is deliberately the LAST write of the generation: the stored hash means "every write above succeeded", so every failure direction leaves it stale (a visible "out of date" that clears on the next regenerate) rather than asserting a block set the client's events do not follow. `preserveCalories` and custom-macros saves stamp **NULL** — neither ran the calculator, so no block drove them, and clearing the hash also stops a formerly block-driven plan from claiming those blocks are current.
- **Generation is per-date, not per-plan** (task 2.5). `generateNutritionEvents` takes a `NutritionTargetResolver` — `(date, dayName, isTrainingDay) => targets` — instead of closing over one set of plan numbers, which is what lets a cascade spanning several blocks write each block's own targets instead of flattening them all to one. Resolution order: **custom macros → the plan grid, blocks ignored entirely** (the coach typed those numbers and the calculator never ran); else **the block covering the date**, when its `daily_targets` grid has been generated; else **`nutrition_plan_daily_targets`**, which is byte-identical to pre-blocks behaviour. `is_training_day` is passed *into* the resolver, never read from a grid — it is a per-DATE fact from live training events, which is why `client_phases.daily_targets` deliberately omits it.
...
- **Blocks are authored from this page.** `client_phases` (see "client_phases table" above) gives a client a chain of named, rate-carrying stretches of calendar, and the status card's footer opens the **"Goal & plan" panel** (`components/clients/goal-plan/`) — a 780px right Sheet with a **Destination** section (start date, target weight, deadline ⇄ rate, goal body fat) and an optional **Route** section holding the block list. This is the **only** surface that creates, renames, re-times or deletes a block; everywhere else renders them read-only. Three rules are load-bearing: the coach enters **lengths in weeks, never dates** (`chainPhases` runs in the browser, the same function the server writes with, so the live readout and the stored chain cannot disagree); the panel issues **two independent writes** — the goals PUT only when a goal field actually changed, and the phases PUT — because `updateGoals` supersedes-and-inserts and would otherwise mint a goal version on every rename; and Save is enabled on **outstanding writes, not `isDirty`**, since a pristine form whose `goal_start_date` disagrees with `phases[0].startsOn` still has a goal write to make. *(Corrected 2026-07-29 — this line previously read "No roadmap or phase concept exists", which migration 137 made false. The **roadmap** half stays true: roadmaps were removed outright in 2026-07-25's migration 133 and blocks did not bring them back.)* The status card's chips describe the client's active **training program** — plan name · `Week X of Y` (via `utils/plan-week.ts`) · Active/Ended. Read "block" as the nutrition/goal concept everywhere in this document; the training chips are about a *program*. "Ended" here means today is past the **authored duration** — a different derivation from the slot-count one the client reader and the amendment surface share (see "Client-side plan tier" above for why both exist).
...
- **Goal chips come from `lib/goals/goal-state.ts`** (reached / beyond / gap). The under-vs-over wording needs the direction of travel, which only the call site's `start` value knows. **Their goal target comes from `client_goals`, not the client prop** — `client-overview-tab.tsx` reads it via `useClientGoals`, resolves through `resolveEffectiveGoal`, converts kg→display with `weightFromKg`, and passes it down; `ClientStatusCard` stays presentational and no longer touches `client.goalWeight`. A regression test pins that the card ignores the mirror. The card is also told when the read is still in flight (`isGoalLoading`) — the same trap as `CoachNotesCard` below: without it an empty-shaped goal renders as a confident "no goal".
