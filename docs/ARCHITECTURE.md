...
- **The date rules**, enforced by the functions against the CLIENT's today: a goal starts today or later; a deadline falls on or after its goal's start and before the next goal's start; a goal planned ahead may not start on or before the deadline of the goal before it. A goal set from today replaces the current one, which ends yesterday whatever its deadline said. The two deadline guards refuse with the goal in the way and the fixes (`lib/goals/goal-write-response.ts`): move the next goal to the day after the new deadline — offered only where its own deadline, which the write service reads onto the refusal, falls on or after that day — or delete it; or end the previous goal's deadline the day before the new start. A restore runs neither guard (TECHNICAL-DEBT.md).
...
- **Routes** (the full coach chain; audited as `goal.create`, `goal.update`, `goal.deadline`, `goal.rename`, `goal.delete` and `goal.restore`, never with a target): `GET` and `POST /api/clients/[id]/goals`, `PATCH` and `DELETE …/goals/[goalId]`, `PUT …/goals/[goalId]/deadline`, `PUT …/goals/[goalId]/name`, `POST …/goals/restore`, `GET …/goals/history`. A goal without the target its type needs is a 400; a refusal is a 409 carrying its sentence, its code, the goal in the way and the fixes; another client's goal is a 404. `GET` answers `{ current, planned, previous, clientToday }` — today's goal with the client's readings on its start day, the planned ones, the goal before today's, and the client's today.
...
| `GET …/goals` | The goal card, the chart's goal line and the goals sheet | `{ current, planned, previous, clientToday }` — today's goal (with the client's readings on its start day, which the chips measure from), the goals planned after it, the goal before today's and the client's today — via `hooks/use-client-goals.ts`, which owns the key builder, the matching area invalidator, the seed a goal write lands its answer with and the clear of the goal history beside it. The tab resolves `current` through `resolveEffectiveGoal` for the chart; the card and the sheet read the goals as stored |
| `GET …/goals/history` | The footer's Goal-history popover | **Lazy** — the SWR key stays `null` until the popover opens, so the Overview does not pay for it on every load. The goals before today's, newest first, bounded, each with its last day and the deadline it ended with |
