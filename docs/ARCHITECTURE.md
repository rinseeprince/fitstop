...
### Several sessions a day (migration 179)

**Any day on a client's calendar can hold several sessions, in order, each its own workout** — a morning run and an evening lift. Each is opened, logged and counted on its own, and nothing refuses a day for holding one.
...
- **Placement lays one session per day** — the date-walk writes each event into a window it has cleared, first on its day — and **Edit plan saves every session of a day** (see "Edit plan").
- **Per-day views show one row per workout.** The coach's Data table and the check-in's frozen `period_snapshot.training` come from `mapEventsToScheduleDays` (`utils/training-event-helpers.ts`): a row per session, a rest day one row; the table pages over those rows, newest day first with a day's workouts in their order. The Overview's training rail stays one dot per day (its counts count every session), and a nutrition day adds every session's surplus (see "The window is the row").
...
- The builder is a weeks × **Day 1–7** grid inside one `DndContext` (`MAX_WEEKS = 52`). Days are **positional, not weekdays** — nothing writes `day_of_week`. A day holds its sessions, in order, or is rest; there is no third state. A day holding several reaches the builder from the calendar, in Edit plan: the day cell stacks one card per session, each opened, edited, dragged and removed on its own. The builder adds a session only to a rest day; a session dragged onto a rest day moves there, and onto a day holding one it swaps with it only when each is alone on its day.
...
**A day's sessions.** A day holds its sessions in order, and a day on a client's calendar can hold several, so every tool that addresses a session by week and day also takes its place on the day (`session`, 1 first); the program state prints a day's places only when it holds several (`D3 Run(1ex) + Lift(4ex)`). `remove_session` removes one session (destructive, previewed like `clear_slot`), `clear_day` clears every session on the day, `add_session` adds only to a rest day, and `move_session` follows the builder's rule (`moveSessionToDay`, `program-builder-model.ts`, shared by the coach's drag and the op): onto a rest day it moves, onto a day holding one session it swaps when both are alone on their days, and any other day refuses it ("That day already has a session"). The snapshot carries at most 20 sessions a day.
