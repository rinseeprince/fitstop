...
> |---------|--------|----------------------|
> | Auth Model → "Database clients" | **Accurate** — the codebase is on **Shape B**: every query is `supabaseAdmin`'s, the route layer is the security perimeter, and the database is locked behind it (migration 201): RLS on every table with no policies, and no `anon`/`authenticated` privilege on any relation in `public`, so a request that reaches PostgREST directly (`/rest/v1/…`) with the browser-shipped public key is refused on every table, with or without a login. `service_role` bypasses RLS, so RLS is the lock on the public key's path and nothing on the app's. Check the current state with `npm run check:rls`, which reads the live catalog rather than the migration tree. | **CONVENTIONS.md §8** |
...
**Security.** RLS enabled, no policy, and no privilege on the table or its views for any role but `service_role` (`SELECT` and `INSERT`). Every read of the table and its views is the service role's, scoped by the client the route verified: the client's own through `services/client-portal-progress.ts` (`GET /api/client/progress`) and the profile embeds under `GET /api/client/me`, filtered on the client and never by source, so a client sees every reading about them; every coach read the same way. `npm run check:rls`.
...
- Every write goes through a route and `supabaseAdmin`; nothing writes through a session client (`lib/session-client-ownership.test.ts`).

**One data path, and a locked database.** Every query the app makes is the service role's, filtered in code by the coach or client id the route or the middleware verified; the session client and the browser client call `auth.*` alone. Behind the server the database holds no policy — RLS is enabled on every table in `public` and governs no app read, so it is the lock, not a rule set — and `anon` and `authenticated`, the roles the browser-shipped public key can act as, hold no privilege on any table, view or sequence in `public`, nor on a new one by default; every SECURITY DEFINER function is executable by `service_role` alone (the auth trigger `handle_new_user()` also by `supabase_auth_admin`, the role GoTrue inserts `auth.users` as). A request at `/rest/v1` with the public key is refused on every relation, with or without a login (`scripts/data-api-locked-proof.ts`). `npm run check:rls` holds it: RLS on every table, no policy in `public` or `storage`, no public-role privilege, every view `security_invoker` (CONVENTIONS §8 → "RLS policies").
...
- **RLS is enabled with zero policies, and `anon` and `authenticated` hold no privilege on the table** (migration 201 revoked the stock grants), so the publishable key that ships in the browser bundle is refused outright. Adding an `anon` policy or grant would open a direct write path bypassing the marketing form's honeypot and Turnstile. The marketing repo's secret key acts as `service_role`, which bypasses RLS and holds the table's grants — how the route still writes; its inserts run as `service_role` in PROD's `pg_stat_statements`. `npm run check:rls` holds all of this.
