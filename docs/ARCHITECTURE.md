...
A coach who placed a program on the wrong day moves it rather than deleting it and customising it again. Under the program's name the Plans hero shows "Starts <date>" — "Starts today" on its first day — while the program hasn't started, and "Next: <name>, starts <date>" for the program that starts after it, whether the hero's program is running or queued; each line carries a pencil (`components/clients/training/plan-start-line.tsx`). A program has started once its start is before the deletion floor (see "The deletion floor"): a running program shows no start line, and a program starting today loses its line once the client logs a workout.

- **The picker** is a month calendar in a popover (`start-date-calendar.tsx`), Monday first, opening on the current start: the start filled, the client's today ringed, every day before the floor greyed and unreachable. Picking a day closes it and moves the program in the same click; picking the current start sends nothing. There is no confirm step — a move loses nothing, and picking the old day moves it back.
...
- **Refresh.** On success the nutrition calendar is invalidated and the Overview, the attention feed and the block facts are cleared; the training area — the calendar and the hero's plan read — revalidates in place, and the pencil spins until it has, then "Program moved" is toasted. A refusal refreshes nothing and toasts "Program not moved" with its sentence.
