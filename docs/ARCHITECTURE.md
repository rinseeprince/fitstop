...
**Coach-side reads go through `client_goals`, not the `clients.*` mirror.** The browser reads it through `useClientGoals` (`hooks/use-client-goals.ts`), which co-locates the key builder with `useInvalidateClientGoals` per CONVENTIONS §7 — any writer of `client_goals` must call that invalidator, since the goal is read by more than one component. **Still on the mirror, deliberately:** the whole client portal (`services/client-portal-progress.ts`, `services/client-portal-service.ts` → `components/client-portal/metrics/goals-section.tsx`) — `/api/client/**` has no goal endpoint at all — plus `hooks/use-nutrition-plan.ts` and `hooks/use-nutrition-builder.ts`, which compute goal estimates from `client.goalWeight` with no `client_goals` fallback.
...
- **One known asymmetry is pinned, not fixed:** `calculateBaselineCalories` returns a *pre-floor* `weeklyRate` beside a *post-floor* `baselineCalories`, so `rate → calories → rate` does not round-trip through the floor (`services/nutrition-service.caps-floor.test.ts`). `calculateBaselineCaloriesFromRate` reports `appliedRateKgPerWeek` — what the returned target actually delivers — instead.
