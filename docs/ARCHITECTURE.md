...
- **Delete = remove the row, move nothing** (`DELETE /api/clients/[id]/blocks/[blockId]`). With windows chosen rather than chained there is nothing to re-anchor: the row goes and every other block stays where the coach put it, leaving a gap — a legitimate state. No mode, no realized `changes`, no consequence sentence to preview. Elapsed blocks 422 (archiving is how a finished block leaves the list). The block's training and nutrition **events are deliberately left alone**: a block edit writes nothing on its own, and clearing them is a separate explicit act.
