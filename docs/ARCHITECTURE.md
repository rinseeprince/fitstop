...
- **Both tracks are date-ranged placements; what still differs is where the END lives:**
...
> **Two "Ended" definitions exist, deliberately.** The client reader, the amendment surface (`isFullyPast`) and the attention feed's training-ending alert all derive the last day from the **slot count** via `calculatePlacementEndDate` / its pure half `placementEndDate`, so a client's app, their coach's editor and the feed agree on the day a program ends. The Overview's plan chip derives its own from **authored duration** via `utils/plan-week.ts` (see "Coach client Overview"). They diverge only on plans whose active row count doesn't match `program_duration_weeks × 7` — which is exactly the corruption `TECHNICAL-DEBT.md → "The amendment writer breaks one active row per slot position"` describes. Fixing that removes most of the divergence; unifying the two definitions is a separate decision.
...
- **Replacing today is always fine; emptying it is the harm.** A placement overwrites the day in the same breath it clears it, and a cascade rewrites a target the client's next food save re-snapshots. An emptied day costs them their target for the rest of it and stores a blank one on every save after that. So only REMOVALS ask this — regenerations and cascades still run from today.
...
- **A retired version governs nothing.** The nutrition plan delete archives every active version (migration 166); every read resolves among active rows, so no cascade can regenerate a date the delete cleared, and the hero, the Overview and the client's Program tab read "no plan" the moment it lands.
- **Callers, and they do no arithmetic of their own:** the block clear and the block delete's `clearEvents` (both through `clearScheduledEvents`), the nutrition plan clears (`clearNutritionPlansForClient` — the calendar's delete and the block delete's "and its plans"), and both training-plan clears (the `cancelFutureEventsForPlan` call sites). A removal that starts at the floor needs **no second "skip the day they logged" filter** — that day is never in range, and adding one would defend a state that cannot occur.
...
1. **The end of the block the START falls inside** (`getBlockEndCoveringDate`, `services/client-blocks-service.ts`) — a block is the time-bound program the coach sells, so its last day is the answer whenever the version opens in one. The block **covering** the start, never the furthest the client has: a later block the coach has not priced yet must not pull targets into itself, or its card reads "Not set" while its days already hold numbers. Its own plan save opens inside it and resolves this then, which is when the coach has actually said what it costs. The same function answers the training placement window, so both tracks read one bound.
2. **Else the furthest live training program's last day** (`getFurthestLiveProgramEnd`, `services/training-service.ts`) — `effective_from + its active day-row count - 1`, through the same `calculatePlacementEndDate` the client reader and the amendment surface use. Carries the `deleted_at IS NULL` / `status <> 'archived'` exclusions both sibling readers document, and **no date predicate**, so a queued program counts like a running one. **Not `training_plans.effective_until`** — that column is NULL on every placed plan by construction (the placement RPC omits it and says so).
...
1. **Computes the window first** — `resolvePlacementWindowEnd()` (`services/program-event-walk.ts`): **the last day of the block covering the start date** when one covers it, else `max(1, programSlots.length)` days (one pass of the authored program); either way **capped, never stretched**, by `getNextPlanStartCap` (the start of the next coexisting plan). **The block is the length knob**, so a program shorter than its block repeats to fill it and a longer one is cut at the block's end. A placement on a day no block covers is unchanged from before blocks bounded anything.
   > `resolvePlacementWindowEnd` DECIDES a window for a placement that does not exist yet; `calculatePlacementEndDate` (same module) DESCRIBES where an existing placement ends, from the rows it actually has. **Do not merge them.** They agree for anything placed under a block, because the rows are cloned to fill exactly that window — and they deliberately disagree for a program placed before its block existed, or one whose block was re-dated afterwards, where the ROWS are the truth and a block edit is still allowed to write nothing. `calculatePlacementEndDate` is what the client's `endsOn`, the amendment's `isFullyPast` and the nutrition horizon's program fallback all read.
...
| `GET …/overview-plan-summary` | Current-plan cards | `training`, `upcomingTraining`, `nutrition` and `upcomingNutrition` are independently nullable |
