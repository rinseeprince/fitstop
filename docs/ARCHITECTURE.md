...
- **Both setup surfaces carry a Block field above their start date** — the apply-to-client dialog and the nutrition drawer's settings form, one shared picker (`components/clients/metrics/blocks/block-start-picker.tsx`) over the pure option builder (`lib/blocks/block-start-options.ts`), fed by the chain payload and `planStartFloor` below. It lists a bare dash — the field's empty state, no block — then the client's blocks whose last day is on or after the shared deletion floor (see "The deletion floor" under Nutrition & Training Events), each with its range (`Cut · 6 Oct – 2 Nov`). Choosing a block FIXES the start: the plan begins on the later of the floor and the block's start — a block already under way starts it today, or tomorrow once the client has logged today, never on the day it began, because the placement and the nutrition save both refuse a start before the floor — and the date input is disabled while a block is chosen, so a plan placed in a block always begins where the block does (owner, 2026-09-10). With the dash the date is the coach's own, floored at the deletion floor (`min`, natively; the server keeps its own check). Changing the block discards a typed date. A coach arriving from a block card's "place one" / "set targets" — or, on a block already set up, from its "update plan" / "update targets" beside the value on its own line — the value, a dash, the teal word, exactly where "place one" sits after "No program placed" — which place a NEW plan from the block's first available day and supersede the standing one from there (the amendment, which edits a placed program in place, is reached from the Training hero alone) — finds THAT block preselected: `useJourneyRoundTrip` captures the trip's block id on arrival, in the same effect that strips the one-shot params, and the host hands it down (`roundTripBlockId` into the nutrition builder; `preselectedBlockId` through the draft provider to the dialog) — a preselection, never a binding, and a block no longer listed falls through to the dash. Purely UX and the same on both tracks: each placement resolves its own window from the block covering its start, so the field only starts the plan where the block the coach means begins. **Route surface:** `GET`+`PUT /api/clients/[id]/blocks`, `DELETE …/blocks/[blockId]` — full coach chain (`coachApiRateLimit` → CSRF → `requireCoachOwnsClient`, foreign client 404), canonical kg on the wire, audit events `block.chain_update` / `block.delete`. The chain payload carries `clientToday` and `planStartFloor` — that deletion floor, resolved server-side because it depends on the client's logs — on every handler that echoes it (GET, PUT, PATCH, DELETE), because `useSeedClientBlocks` writes a mutation's response straight into the chain cache; both setup surfaces floor their date pickers on it, and a block itself is not constrained by it. RLS deny-all + `GRANT … TO service_role` only (CONVENTIONS §8). Plus the read-only **`GET …/blocks/facts`** (Session 3.2, `services/client-blocks-facts-service.ts`). **A block reports what is set only if it actually HAS DAYS inside it** — per track: on training, gated on the events, because a window says where a plan was placed to run and the events say which days it actually put on the calendar; so a block the coach has drawn but not set up never claims a program whose rows merely reach into it; on nutrition a day's target is computed from the version covering it, so an active version overlapping the block IS a day inside it, and the versions read answers the gate on its own. The gate is bounded at both ends of the block (days after it are not its days), status-agnostic (a completed day is still a day) and NOT clamped to today (a current block whose program starts tomorrow has days). It is per-track because the two are set up separately: a block can hold workouts and no targets. Given days: per-block training programs (governing segments, latest-start-wins), and the nutrition VERSIONS whose windows overlap the block — the same list shape as training (owner, 2026-09-10): one entry per active version, in start order, each carrying the numbers off its OWN row (daily calories with the custom-macros override honoured, deficit = `tdee − calories`), its own window (`startsOn` / `endsOn`, the row's) and its **state** — `active`, `upcoming` or `ended` against the CLIENT's today, stamped server-side by the block's own date rule (`derivePlanState`, `lib/blocks/block-derivations.ts`; the facts route resolves the day exactly as the chain route does, so the browser never derives a plan's standing from a day it obtained elsewhere). Queued versions are included, a version that began before the block keeps its real start as a crossing program does, and a re-save with the same numbers is its own entry as a program placed twice is. Hand edits and training surpluses are excluded by construction — the plan row holds neither. Version windows never overlap (the gist exclusion) and may leave gaps, so the list is a plain intersection with the block window, no resolution rule, unlike training; a closed window is immutable, so an entry dated in the past never rewrites itself on a later save. **The card shows ONE entry per track — the headline** (`selectHeadlineFact`, `lib/blocks/block-headline.ts`, a precedence over the wire's states and never a date): the plan in force today, else the first one queued in the block, else the last one that ran — so a running block whose only plan starts tomorrow reads set, a future block shows the plan that will govern its first day, a finished block shows the plan that governed its last day, and a program that ended early with nothing queued after it still headlines its block, as ended, with "update plan" beside it (owner, 2026-09-11); anything queued later in the block is left to the timeline, and the day it takes over it becomes the headline because its state does. A state chip (Planned / Ended) follows the value when the headline is not in force, and the headline's own range — its `startsOn` – `endsOn`, in the grammar the card's header spells the block's ("24 Aug – 4 Oct"; one date for a plan that ran a single day, `formatBlockRange`) — sits under the value on both columns. **The "what happened" timeline lists every program and version that STARTS inside the block**, each with its range in the date column and its state chip (Active / Planned / Ended), a version with its numbers, whether the block has begun or not; the block's own "started" / "ended" rows keep their single date; a plan that began before the block has no entry. The training-dates read is **paged** (`lib/paged-fetch.ts`) — an unpaged read silently truncates at PostgREST's ~1000-row cap; nothing per day is read for nutrition. Kept off the chain GET so PUT/DELETE keep echoing that GET's exact payload.
