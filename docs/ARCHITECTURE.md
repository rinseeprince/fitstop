...
**Every body measurement is one row.** Weight, body fat, waist, hips, chest, arms and thighs (`metric_key`, CHECK-constrained; `lib/measurements/keys.ts` mirrors the two CHECKs), the `value` in canonical kg / cm / % (CONVENTIONS §20), `recorded_on` (the day the reading belongs to, on the CLIENT's calendar), `recorded_at` (when the row was written), `measured_at` (when the reading was TAKEN — null means the day is known and the time is not; the check-in writer sets it, a coach entry or an intake reading leaves it null), `source` (`check_in` / `coach_entry` / `client_log` / `intake`), `source_id` (the check-in id for a check-in's reading, and for a later correction of it; null otherwise), `note`, `created_by` (`coaches.id` for a coach entry — the audit actor), and the removal mark — `voided_at`, `voided_by` (`coaches.id`, a real foreign key, SET NULL) and `void_reason` (migration 160), null while the reading is live. No `updated_at`: a value is never rewritten, and the mark is its own timestamp (CONVENTIONS §8). Indexed `(client_id, metric_key, recorded_on DESC, recorded_at DESC)` for every per-client read, partially on `source_id` for the check-in fold, and partially on `voided_by`.

`services/measurements-service.ts` holds the table's two INSERT paths (`appendMeasurements`, `appendCorrection`) and every read below; `services/measurement-edits-service.ts` drives the three row actions of rule 8. The rules, each enforced once:

1. **Append-only.** `service_role` holds `SELECT` and `INSERT` and nothing else — the migration revokes the default privileges first, then grants. Nothing deletes a row and no value is ever rewritten; a correction is a new row on the same day. The one UPDATE the table sees is the removal mark of rule 8, set and cleared by the SECURITY DEFINER pair `void_measurement` / `restore_measurement` (migration 160), executable by `service_role` alone.
...
8. **A reading is corrected, removed or restored — never deleted** (owner decision D9). A wrong VALUE is corrected: `appendCorrection` inserts a `coach_entry` row carrying the original's metric, day, stamp and `measured_at` (only the number was wrong; a null would sort under the original once D10's rule arrives), so rule 2 makes it the day's value, the check-in fold reads it as the check-in's reading and every "now" surface follows, while the original stays in the history. Its unchanged-check compares against the reading's STANDING value of any source in the same day and stamp scope, not rule 3's same-source key — a correction exists to replace what stands. A reading that should never have existed is removed: `void_measurement` sets the mark, and refuses a row outside `p_client_id` (the route proves the coach owns the client and cannot prove the row does), a row already removed, and the client's only live weight (activation refuses without one and the pair cannot compute without one — correct it instead; body fat may go to none, the formula switches); `restore_measurement` clears it and refuses a live row. Each returns whether the row was, or becomes, the client's newest reading of its metric, and the service recomputes the energy pair on that for weight and body fat — the trigger appending a newest reading fires. Coach only, from the Journey's measurement log: Edit reading and Remove reading (behind the destructive-confirm dialog, which names the reading and says when it is the current reading or the baseline) on a live row, Restore reading on a removed one; audited as `measurement.correct`, `measurement.void` and `measurement.restore` with the metric and date only. The routes are `POST /api/clients/[id]/measurements/[measurementId]/{correct,void,restore}` on the full coach chain; a foreign row is 404, a state refusal 409, a value outside the metric's bounds 400. A change reaches the client on their next read; the coach's series, the client record (for a weight or body fat) and the stamped check-in's detail are invalidated together (`use-reading-actions.ts`).

**Energy follows the newest reading.** `appendMeasurements` calls `recalculateClientEnergy` when a row it wrote is the client's newest weight or body fat (`client_current_measurements` after the insert); a backdated row that is not the newest recomputes nothing. Correcting, removing or restoring a reading fires the same recompute when that reading is, or becomes, the newest (rule 8).
...
**Writers**, all through `appendMeasurements` — and a coach's correction through `appendCorrection` (rule 8): check-in submit (`check_in`, stamped); the Journey's "Log measurement" for a physique key, the metrics PUT and a `currentWeight` / `currentBodyFatPercentage` carried by `PATCH /api/clients/[id]` (`coach_entry`, dated the coach's today); the details sheet's Baseline fields (`coach_entry` dated ON the start date; an `intake` row dated today before activation); `createClient` (`intake`, the weight and body fat it was given, dated the coach's today); the intake sync (`intake`, dated the questionnaire's completion day on the client's calendar, only for a metric the client has no reading of). A coach's write is audited as `measurement.create`, a correction as `measurement.correct`, a removal and a restore as `measurement.void` / `measurement.restore` (metric and date only). The seeds write the log; `scripts/seed/teardown.ts` relies on the `ON DELETE CASCADE` from `clients`, because the app role has no `DELETE`.
...
Who triggers a recompute: `appendMeasurements`, when a row it wrote is the client's newest weight or body fat — the one path every reading writer takes (check-in submit, the Journey's Log measurement, the metrics PUT, the details sheet, `createClient`, the intake sync); `appendCorrection` and the void/restore RPC pair, when the corrected, removed or restored reading is, or becomes, the newest ("client_measurements table", rule 8); `updateClient`, when a profile input changes (height, gender, birth date, activity level); the metrics PUT, for an override instruction; the intake sync, after its profile fields land.
...
| Metrics (**labelled "Journey"**) | `MetricsTabContent` | Four panes via `?journey=` — **Physique** (the hero's switcher selects ONE metric, carried as `?metric=`, and the hero, the progression chart and the measurement log all describe it: the chart over the measurement log's day-values, the log listing EVERY reading of the selected metric — a check-in's value under a coach's same-day correction, a removed reading muted with who removed it — each with its source and note, its pager counting that metric's entries ("Showing 10 of 12 weight entries", the number on the hero's entries chip) and its empty state naming it; all from `GET …/measurement-series`: the day-values, the derived baseline, the start date, the readings list, readings before the start under "Before start". Three hover-revealed row actions per physique reading — Edit reading, Remove reading behind the destructive confirm, Restore reading on a removed one — see "client_measurements table" rule 8), **Wellness** (the same hero, chart and log over the merged check-in weekly averages ⊕ coach-logged `client_metric_entries`), each with the "Log measurement" modal (a physique key appends to the log, a wellness key upserts an entry), **Training** (`ExerciseDataView`, moved here from the Training tab in Session 7.1 — analytics live in Journey, prescription stays on its own tab), and **Blocks** (`client_phases`; see "Journey blocks"). `JourneySubtab` is deliberately WIDER than `MetricTab`: Training and Blocks key none of the metric shapes (`metricsByTab` / `logRowsByTab` / `DEFAULT_FOCUS`) and are mapped onto `"body"` by `toMetricTab()`, a whitelist so the next pane is safe without editing it |
