...
The single durable `nutrition_plans` row per client is the **template** that generates `nutrition_events`. It holds the plan-level prescription — `baseline_calories`, `protein_target_g` / `carb_target_g` / `fat_target_g`, `diet_type`, `protein_target_g_per_kg`, the custom-macros override (`custom_macros_enabled` + `custom_calories`/`custom_protein_g`/`custom_carb_g`/`custom_fat_g`), the calculator inputs (`base_weight_kg`, `bmr`, `tdee`, `work_activity_level`, `training_volume_hours`), and the goal snapshot (`goal_weight_kg`, `goal_deadline` — `goal_source` was dropped by migration 133) that drives the weight-drift / goal-drift banners. `nutrition_plan_daily_targets` is its **per-weekday grid** (`(nutrition_plan_id, day_of_week)` → `calories`, `protein_g`, `carb_g`, `fat_g`) — the source used to generate/regenerate the dense per-date `nutrition_events`. It is replaced **DELETE-then-INSERT** on each plan upsert and is **not** the display SOT (events are). The baseline/deficit calculator is `services/nutrition-service.ts` (pure — the browser previews through the identical module), macro splitting in `utils/nutrition-helpers.ts`, rate⇄calorie conversion in `utils/energy-conversions.ts`; the retired calculator doc's prose walkthrough lives in git history of `docs/NUTRITION_PLAN_CALCULATOR.md`. The plan is edited **in place** (`create_nutrition_plan_atomic` upsert — migration 115, rebuilt at 24 args by 139, conflict path amended by 143): every save unconditionally re-stamps the prescription **and** the goal-snapshot columns (139 removed the old `p_recalc_snapshots` conditional — every surviving caller is a genuine recompute), and since migration 143 **`effective_from` advances on the conflict path too**. `effective_from` therefore means **"when the current numbers took effect"**, not "when this plan row was born" (`created_at` keeps the birth date): `scheduledFor` (`effective_from > clientToday`) works for existing plans, the hero reads "Starts {date}" for a queued regenerate and truthfully "Active since today" after a same-day one, and `buildDailyTargetsFromPlan` gates its no-event template fallback to dates on/after `effective_from` so a future-dated save never renders the next prescription's numbers as current (events before `effective_from` keep the old prescription and are never gated).
...
- The upsert **rewrites** `baseline_calories` + macros from the current plan template on every covered date; "baseline is preserved" was never mechanically true — the same value usually lands because the template hasn't moved. After a future-dated regenerate it is not invisible: days before `nutrition_plans.effective_from` still carry the old prescription's numbers, and any cascade covering them overwrites with the new template's (`TECHNICAL-DEBT.md` → baseline leak).
