...
- `hasNutritionPlan` - a nutrition version covers the client's today OR one is queued (covering-or-future, migration 144 — the same predicate as the client log guard, so a coach who queued a first plan reads as ready on both surfaces)
