...
### client_goals table (migration 193)

**A goal is one row, dated.** `name` (a label for that goal, defaulted from its type and editable any time; nothing computes from it), `type` (the six in `GOAL_TYPES`, `lib/goals/goal-types.ts` — the questionnaire's list, which migration 193's CHECK mirrors), `target_weight` (kg) and `target_body_fat_percentage` — each optional in the table, the goal routes requiring the one the type needs (`GOAL_TYPE_SETTINGS`: a weight to lose weight or build muscle, a body fat for a recomp) — `description` (optional; the client's own words for the goal Sync metrics copies), `starts_on` (a day on the client's calendar), `source` (`coach`, or `intake` for the goal Sync metrics wrote) and `set_by` (the coach; SET NULL). One goal per client per day (`client_goals_one_per_day`). **No end is stored:** a goal runs until the next goal starts, so goals can neither overlap nor leave a gap, and a row dated ahead is a **planned** goal. `client_goal_deadlines` holds every deadline a goal has had — a date, or none — with the day it took effect (`effective_on`, one per goal per day) and who set it; the newest on or before a day is that day's deadline. A goal that has not started, or started today, has exactly one, dated its start.

- **Which goal, and which deadline, a day has** is decided once, by the pure `goalOnDay` / `deadlineOnDay` (`lib/goals/goal-timeline.ts`), over one read of the client's goals with their deadlines (`listClientGoals`, `services/client-goals-service.ts`): the latest goal starting on or before the day, and its newest deadline entry on or before it — a goal not started yet reads the deadline it starts with. Every "goal now" read asks with the client's today from `getClientTodayString`, so a planned goal reaches no "goal now" read before its day and takes over at the client's midnight on every surface at once. The reads: `getGoalForDate(clientId, day)`, `getCurrentGoal(clientId)`, `getGoalsOverview` (today's goal with the client's readings on its start day, and the planned ones) and `getGoalHistory` (the goals table, below).
- **What changes a goal.** Changing its targets or its type is a NEW goal — from today, or planned from a later day. Changing its deadline keeps the goal: the change is recorded against it, dated today; a planned goal's one entry is rewritten, a change back to yesterday's deadline removes today's entry rather than copying it, and an ended goal's deadline cannot change. Renaming keeps it: the name and the description are labels, editable on any goal. **A goal that has started never changes its type, targets or start day**; today's goal may be corrected today, and a planned goal is edited or moved freely until its day. Any goal can be deleted.
- **The date rules**, enforced by the functions against the CLIENT's today: a goal starts today or later; a deadline falls on or after its goal's start and before the next goal's start; a goal planned ahead may not start on or before the deadline of the goal before it. A goal set from today replaces the current one, which ends yesterday whatever its deadline said. The two deadline guards refuse with the goal in the way and a sentence saying what would clear it (`lib/goals/goal-write-response.ts`).
- **Every write is one of five database functions** — `add_client_goal` (from today or a later day), `edit_client_goal` (a planned goal or today's, rewritten whole), `set_client_goal_deadline`, `rename_client_goal`, `delete_client_goal` — SECURITY DEFINER and executable by `service_role` alone, each one transaction, serialised per client (an advisory lock; `rename_client_goal` locks its row), and writing nothing when nothing changed. The app role holds `SELECT` on both tables and nothing else; every read is the service role's, scoped by the route-verified client. `services/client-goal-writes-service.ts` drives the functions and maps a refusal ("code: message") to a `GoalWriteError`; `scripts/goal-functions-proof.ts` proves every rule on DEV inside a rolled-back transaction, each against a copy of its function with that rule taken out.
- **Delete is a hard delete** — a deliberate exception to CONVENTIONS §8's soft-delete rule, because nothing references a goal row — a sent check-in keeps its own copy of the goal it was judged against — and the goal before it simply covers its days again.
- **The two ways a first goal is set** both go through `add_client_goal` from the client's today: the manual Add client (`createClient` — the goals sheet's fields: its type, name, targets, deadline and description; the new client's today is read before the row is written, so a deadline before it is a 400 that leaves no client behind) and the intake's Sync metrics (the questionnaire's type, its targets, the client's own words as the description, and the deadline only while it is still ahead — a passed one is not copied, and the sync says so — written only when the client has no goal in force; unlike the routes, the sync does not require the target the type needs).
- **Routes** (the full coach chain; audited as `goal.create`, `goal.update`, `goal.deadline`, `goal.rename` and `goal.delete`, never with a target): `GET` and `POST /api/clients/[id]/goals`, `PATCH` and `DELETE …/goals/[goalId]`, `PUT …/goals/[goalId]/deadline`, `PUT …/goals/[goalId]/name`, `GET …/goals/history`. A goal without the target its type needs is a 400; a refusal is a 409 carrying its sentence, its code and the goal in the way; another client's goal is a 404. `GET` answers `{ current, planned, clientToday }` — today's goal with the client's readings on its start day, the planned ones, and the client's today; `GET …/history` answers the goals table.
- **The client's wires** carry the goal in force on the client's today, read by the service role with the route-verified client id: `goalWeight` / `goalBodyFatPercentage` on `GET /api/client/me` and `PATCH /api/client/settings` (in their place in `toClientSelfView`, `lib/mappers.ts`), `client.goalWeight` / `client.goalBodyFatPercentage` on `GET /api/client/progress` with the goal's `client.goalType` and the client's readings on its start day (`client.goalStartWeight` / `client.goalStartBodyFatPercentage`) — which way it points, for the Goals & Progress card's `goalProgressChip` — and `goal: { weightKg, deadline, name, type, bodyFatPercentage, description, readings }` on `GET /api/client/journey` — the last five for the client's goal card, `readings` being the newest readings and those on the goal's start day. `Client` carries no goal fields, and a planned goal reaches no wire before its day.

### Effective goal resolution

`resolveEffectiveGoal(goal)` (`lib/goals/resolve-effective-goal.ts`) turns the goal in force on a day into what the calculator and the pace checks read: the weight target (kg), the body-fat target and the deadline. No weight target means **maintenance** (`goalWeightKg: null`), and so, for the calculator, which needs both, does no deadline or a deadline on or before the day the plan takes effect. A goal has no start of its own for the calculator: the window a nutrition deficit is spread over begins at the day the plan takes effect (see "Nutrition plan versions"). Its callers, each with the goal it reads:

- `services/nutrition-calc-inputs.ts` — the goal in force on the day a nutrition plan takes effect, with that day's deadline: the save's effective date (`nutrition-plan-orchestrator.ts` calls `resolveNutritionCalcInputs` with it) and the drawer's Starts on, through the day read `GET /api/clients/[id]/nutrition/goal?date=` (`services/nutrition-goal-service.ts`), so a plan starting inside a planned goal is priced — in the preview and the save alike — for that goal (owner ruling 2026-09-22).
- `lib/nutrition/nutrition-out-of-date.ts` — the out-of-date rule: the goal in force on each of a version's days from the client's today, against the goal the version was built for (see "Nutrition plan versions").
...
- `components/clients/client-overview-tab.tsx` and `components/clients/metrics/hooks/use-merged-metrics.ts` — the Overview's chart and the Journey's Physique pane: today's goal from `GET /api/clients/[id]/goals` (`hooks/use-client-goals.ts`).
...
- **`status`** — POSITION. `approaching` | `achieved` | `overshot`, in the direction the goal is approached: the goal type's where it decides one, else `sign(goal − start)`. With neither there is no direction to overshoot in, so the answer is `approaching`.
- **`trend`** — TREND. Which way the recent check-ins moved the client relative to the target: `towards`, `away` or `unchanged` (an average change of zero) — `null` with fewer than two of them carrying the metric, never a guess.
...
**The goal strip's weight row, when the goal has a deadline, reads direction before speed** (`resolveGoalRowState`, `lib/check-in/review-figures.ts`; owner decisions 2026-09-24): `Reached` for a met goal; `Deadline passed` for a deadline gone by short of the target; then the trend — `Moving away`, `No change`, or `Too early to tell` (neutral) with no trend yet — and only a client moving towards the target gets the pace's words, `On track`, `Behind pace` or `Deadline unrealistic`. A met goal renders no remaining distance and no pace check. Once every goal that could be judged is met, the strip's footer suggests a new target while the goal judged is still the client's live one (`goalIsCurrent`), and says nothing once it has been replaced — never the nutrition note; a `No reading yet` row neither earns the note nor blocks it. The body fat row, and the weight row of a goal with no deadline, say only where the client stands and how far: `Reached`, the distance past the target, or the distance to go, muted.

### The goals table (Journey → Goals)

`?journey=goals` (`components/clients/metrics/goals/**`): a row per goal, newest first, so planned goals lead — its dates (its start to the day before the next goal starts; a date carries its year when it isn't this year, `formatHistoryDate`), its deadline as it ended or as it stands, its targets, the weight it ended at (today's goal: the newest, `goalEndWeight`) and its result — each opening onto what happened during it, in date order. Nothing is recorded for it: `getGoalHistory` (`GET …/goals/history`) reads the goals, the live programs from the day before the first goal (`getTrainingPlansOverlapping`) and the active nutrition versions from its first day, and `goalHistoryRows` (`lib/goals/goal-history.ts`) gives each line to the goal whose days hold it — a deadline change (one made the day a new goal took over never took effect, and is neither listed nor read); a version whose days meet the goal's, with its calories and the goal it was built for; a program starting, replacing the one that ended the day before, or ending with none starting the day after. The last goal's lines run on past today: a queued version and a program's scheduled end are listed on it.

**The result is derived, never stored** (`goalResult`, `lib/goals/goal-result.ts`): in the browser, in the viewer's units, from the measurement series, through the goal card's verdict (`goalState` in the type's direction), per target. An ended goal was reached on the first day a reading met its target; else it missed by the gap on its deadline, when that came while it ran with a reading by then; else it ended short by the gap on its last day. Today's goal reads the card's own chip, from the goals read's start readings; a planned goal is planned; a goal with no targets reads its weight change over its days, once a reading was taken after the one it runs from.

**Reads and writes.** The pane reads the table, the series and the goals read; a result waits for the series, and today's for the goals read too. A row's delete is the goals sheet's confirm, typed for today's goal and an ended one; it lands its answer in the goals read and refreshes the table in place, closing once it has — a refresh that fails drops the rows instead (`useGoalWrites(clientId, true)`). Every other goal write (the goals sheet, Sync metrics) and every writer that clears the block facts — computed from the same programs and versions — clears the table (`useClearClientGoalHistory`; `hooks/use-client-goals.test.ts` holds them to it).
...
6. **"Where they stand" is read at a date; "what this check-in reported" is what it sent.** The date is today on the Overview and the Journey, and the check-in's own day for its goal section (commit 8b), judged once when it is sent and frozen with it: goal position and the drift note take the reading as of that day — the check-in's own reading, else the newest live reading dated on or before it (`getReadingsAsOf`, the copy's builders alone) — while the Overview and the Journey take today's reading from the measurement series and the energy pair takes the client record's (rule 4). The review band, the AI prompt and the client's check-in detail take what the check-in sent (rule 5). The band's "vs last check-in" compares what this check-in sent with what the previous check-in sent and nothing else — a reading logged between two check-ins belongs to the Journey series and to "now", never to that comparison. A check-in submitted without a weight reported none: the band's cell shows its empty state while the goal strip judges the reading before it.
...
**Security.** Every read of the table and its views is the service role's, scoped by the client the route verified: the client's own through `services/client-portal-progress.ts` (`GET /api/client/progress`) and the profile embeds under `GET /api/client/me`, filtered on the client and never by source, so a client sees every reading about them; every coach read the same way.
...
- **The baseline is derived, never stored.** Per metric, the reading as of the start date: the latest live row on or before it, else the earliest after it — `client_baseline_measurements` (migration 158), read only through `getBaseline` (`services/measurements-service.ts`; `lib/measurements/baseline-ownership.test.ts` fails if anything else derives it). It cannot disagree with the series, because it IS a point of the series, and it cannot be edited into a number no reading carried. Every surface that shows it — the Overview band's "Since start", the Journey hero's "since start" figures, the check-in ribbon's change since start, `startingWeight` / `startingBodyFatPercentage` on `/api/client/me` and `/api/client/progress` — shows the reading's own date and source. A client with no start date has no baseline.
...
`nutrition_plans` holds **date-ranged versions, each a placement** (migration 166): N rows per client whose `[effective_from, effective_until]` windows never overlap and may leave gaps, every one carrying an end. Each version is the **template for its own era** — it holds the plan-level prescription — `baseline_calories`, `protein_target_g` / `carb_target_g` / `fat_target_g`, `diet_type`, `protein_target_g_per_kg`, the custom-macros override (`custom_macros_enabled` + `custom_calories`/`custom_protein_g`/`custom_carb_g`/`custom_fat_g`), the calculator inputs (`base_weight_kg`, `bmr`, `tdee`, `work_activity_level`; `training_volume_hours` is also stored but deprecated — accepted for backward compat and read by nothing), the goal it was built for (`goal_weight_kg`, `goal_deadline` — the weight target and deadline of the goal in force on its first day, which a calculated version's calories were priced from and the out-of-date rule reads for every version), and the two training-surplus settings (`include_activity_burn`, `surplus_as_carbs`, migration 196 — see "Display total") — plus its own `nutrition_plan_daily_targets` **per-weekday grid** (`(nutrition_plan_id, day_of_week)` → `calories`, `protein_g`, `carb_g`, `fat_g`), replaced with the version on every save of it, and the row a computed day takes its calories and macros from verbatim (see "The window is the row").

**A version is out of date from the first day, today onward, whose goal differs from the goal it was built for** (owner ruling 2026-09-22). "Differs" means the calories would come out different: a version records only the weight target and the deadline it was priced for, so a new goal with the same two — one that changes only its body-fat target, or only its name — leaves it standing, and a pair missing either one prices at maintenance, so two such pairs never differ. Days before today are history and never judged; a queued version is judged from its own start; a version that ends before a planned goal starts is never out of date because of it. One pure rule, `findNutritionOutOfDate` (`lib/nutrition/nutrition-out-of-date.ts`), over the client's versions with a day left (`getNutritionVersionGoalsFrom`) and their goals, judges each version's turning days — its first day from today and each goal start inside it, between which the goal in force cannot change — and reports the earliest day across the versions. `GET /api/clients/[id]/nutrition/goal/out-of-date` answers it (`services/nutrition-goal-service.ts`; the full coach chain), and the Overview's nutrition card, the Nutrition tab's Plans pane and the drawer show the one answer through one component (`NutritionOutOfDateNotice`). It names the goal from that day and what the calories still aim for — or, for calories the coach typed (`custom_macros_enabled`), which were judged the same way but priced for no goal, the day the goal changed (its start, or its latest deadline change, when that falls on or after the version's first day; else just that it has changed, as after a goal is deleted) and "The calories haven't changed since then." — or, for a change still to come or no goal, "The calories stay as they are." Its button: Regenerate when the problem starts today; "Set nutrition from <day>" when it starts later, which opens the drawer with Starts on already on that day — on the Overview through the drawer's one-shot `?edit=1&startsOn=` (see "Client page tab structure"), on the Plans pane by moving Starts on first. Nothing regenerates by itself. The notice closes with an ×, on any of the three: `POST …/nutrition/goal/out-of-date/keep` recomputes the rule and records its answer — the goal the version's calories are kept for, never one sent by the browser — in `nutrition_plan_kept_goals` (migration 197; once per version per goal pricing), and the rule reads a day whose goal prices like a kept one as fitting. So a closed notice stays closed everywhere until the goal on its days prices differently again; a notice that changed since the coach saw it is refused (409) and the current one shows.
...
**The start is judged first, in the orchestrator**: a version may not start in the past — the orchestrator refuses a past date and the RPC's belt refuses it again — and nothing else bounds it (owner, 2026-09-11): today is the coach's to replace whatever the client has eaten, so a save from today replaces the running version from today, logged or not, and nutrition never asks the deletion floor (see "The deletion floor"). **The write path** is `create_nutrition_plan_atomic` (28 arguments since migration 196: 166's 25 with `p_effective_until` required, 196's two required surplus settings `p_include_activity_burn` / `p_surplus_as_carbs`, plus 172's optional `p_coach_note`): a **caller-cooperative belt** refuses `effective_from < p_today`, an end before the start, and a save missing either surplus setting; every active version of the client is locked `FOR UPDATE`; the end is capped once more at the next queued version's start, as a race belt; the **predecessor** — the active version that started before and still reaches the new start — is capped at `new_start − 1` (convergence, never deletion); then a version starting on the **same day** is **replaced in place** (it keeps its id, so its note rows and a retry after a failed note insert land on one row) and otherwise the new version is inserted; then the version's daily-target grid is replaced. **A save is the RPC and nothing else** — the coach's note is one of the RPC's arguments and lands in the same transaction (migration 172): no day is written, because every day inside the window is computed from the row the moment it commits, a today the client has already logged included (the log stores no target), and the capped predecessor's tail past this version's end is answered by nothing from that same moment. Windows can never overlap: the gist exclusion `nutrition_plans_active_window_overlap` (scoped to `status = 'active'`) is the backstop that must never fire, and racing first saves collide on it loudly. There is no open row. A save dated BEFORE a queued version runs until the day before it and leaves it standing; the drawer's Starts-on field says so under the date. `effective_from` means **"when this version's numbers took (or take) effect"** — birth and effect coincide per version (`created_at` keeps the row's birth) — and, for a calculated version, **the day the deficit is spread from**: the calculator's window runs from the later of `effective_from` and the client's today to the day before the goal deadline — the deadline is the weigh-in, so 24 Sep to 31 Oct is 37 days and a deadline on or before the first day holds at maintenance (`deadline_passed`) — in the drawer's preview and in the save alike (`handleCalculatedPlan` hands the date to `generateNutritionPlan` as `startDate`; the drawer's "Starts on" setting hands the same date to the same pure module). **Resolution is by date everywhere**: `getNutritionPlanForDate` / `getNutritionPlanIdForDate` (via the shared `coversDate`), `getNextFutureNutritionPlan` (earliest queued), `getLatestNutritionPlan` (the latest-starting version — the drawer's seeds), `getActiveNutritionPlanId` (covering-today wrapper). The coach GET returns **three roles** — covering ("Active since", `hasCurrentTargets`), earliest-future (`scheduledFor`), latest (drawer seeds `latest ?? covering`) — and the client's `clientToday`, which the drawer's Starts on defaults to and floors on; a client whose every version has ended reads as having no plan. It carries no goal: the drawer reads the goal and the calculator's inputs for its Starts on day from `GET …/nutrition/goal?date=`, only while it is open. When this read fails, the drawer shows it failed with Try again and Generate stays off. **Delete = a save of nothing from today** (`clearNutritionPlansForClient`, the training clear's shape; owner decision 2026-09-10): the RUNNING version ends at YESTERDAY and keeps its past — on the calendar and on every block it ran in — a QUEUED version is archived (it never ran a day of its own), a FINISHED version is untouched, and no day statement is issued — ending the versions IS removing the days (`services/nutrition-plan-clear-service.ts`) — but the hand edits on the days it uncovers go with them: one delete of the `nutrition_day_edits` rows dated from today inside the ended versions' windows, issued FIRST so a failure leaves the versions whole for the retry, while past days keep their version and their edits. The block delete does the same for the versions laid in the block, and the block card's per-plan delete does it for ONE version by id (`clearNutritionPlanById` — the same retire path; the statements are spelled once). Yesterday, never today, deliberately: a version closed AT today would keep covering it, so the hero would go on saying "Active since" until midnight, and today is the coach's to end whatever the client has eaten (owner, 2026-09-11). Every read resolves by date among `status = 'active'` rows, so from today nothing covers a day — the block card's headline reads Not set, and the client's meals still save on those days with no target to judge them. The baseline/deficit calculator is `services/nutrition-service.ts` (pure — the browser previews through the identical module), macro splitting in `utils/nutrition-helpers.ts`, rate⇄calorie conversion in `utils/energy-conversions.ts`; the retired calculator doc's prose walkthrough lives in git history of `docs/NUTRITION_PLAN_CALCULATOR.md`.
...
- The unique constraint indexes every per-client range read; `coach_id` is indexed partially, for the FK's SET NULL.
...
**Display total**: `baseline * (1 + surplus/100)` when the version's `include_activity_burn` is on, else `baseline`. **The two surplus settings belong to each saved version** (migration 196; owner, 2026-09-23: a past day's target never changes, and the settings apply on save, never when a switch moves): "Apply training day surplus" (`nutrition_plans.include_activity_burn`) and "Add training calories as" (`nutrition_plans.surplus_as_carbs`) are fields of the nutrition drawer's save, written by `create_nutrition_plan_atomic` with the rest of the version, and a day is priced with the settings of the version covering it — the computed day carries them (`NutritionEvent.includeActivityBurn` / `surplusAsCarbs`), so every reader prices it the same way and a later save, which starts on the client's today at the earliest, never reaches an earlier day. A client with no version covering their today reports the defaults a first plan starts from (on, keep split — `DEFAULT_SURPLUS_SETTINGS`, `lib/nutrition/surplus-settings.ts`) on their own profile. How the surplus calories distribute across macros honors `surplus_as_carbs` via the shared `applySurplusSplit()` (`utils/nutrition-helpers.ts`): protein is held; **keep-split** (default) scales carbs+fat preserving the plan ratio; **carbs-only** holds fat and adds the surplus to carbs. The same helper backs both the event mapper (`mapNutritionEventToDisplayTarget`) and the plan-based "typical week" path (`buildDailyTargetsFromPlan`, whose no-event days take the covering version's settings).
...
- **Calendar operations** (training events): coaches can move an event to another date, clear a week, delete an event, and edit a day's session in the placed-session tray. Moved events get `is_modified = true`. **Nothing a coach does lands on or leaves a past day**, because a past day's nutrition target is priced from the sessions on it and never changes (owner, 2026-09-23): the move refuses a session sitting before the client's today as well as a target before it (`moveEvent`, `services/training-event-calendar-service.ts`), and a library session dropped on the calendar refuses a past day (`POST …/place-from-library`, `type: "session"`) — the calendar offers neither, and the server refuses both for any caller that skips it. **A day can hold several sessions**, and a moved one joins its new day last — see "Several sessions a day" above. **The tray's Save rewrites the session row the day's event points at** (`PUT …/sessions/[sessionId]` → `replaceSessionFull`). Every writer that puts a day on the calendar — placement, the plan editor's save and a library session dropped on a day — gives the day a row of its own, so a save changes that day alone; changing a session across many days is the plan editor's (see "Edit plan"). The response carries the saved session, which the tray writes into its own read before it refreshes the training area and closes, so the day reopens on what was saved.
...
2. **Unlogged days a version covers**: the computed day (`getNutritionTargetsForDateRange` over one day, via `mapNutritionEventToDisplayTarget`, honoring the covering version's `include_activity_burn` + `surplus_as_carbs`)
...
- **A chart follows what was actually logged when that differs from the type**, on columns, never
  ...
  distance, where the fastest counts for the best times (at race distances on an Endurance or Erg
  exercise — see PRs below); a time logged with NO distance is a hold,
...
- **The point is one shape for every type.** `ExerciseProgressionPoint` (`types/training.ts`) is one
  ...
  table, so the next marker or column is a row in a table, not a migration. The window and the bound —
  twelve sessions unless a count or a date window is given — are migration 120's, and the logs it
  reads are the exercise's by `exercise_log_identity` (above); the session
...
- **Every chart has a table of its sessions beneath it** (`exercise-sessions-table.tsx`, shared by
  ...
  `3 × 1 km`, `64 × 3 × 40 m`, `3 × 0:30` — `5 km` for a piece and `6 × 800 m` for a run of pieces
  of one distance, `1:30 · 1:20` for holds; the shape only, never the times, which are the Time and
  Pace figures (owner, 2026-09-21)) — then a fixed
  ...
  its tooltip ("5 Rep Max · 105 kg", "5 km · 20:05"; `recordsHeldBy`, `utils/exercise-records.ts` —
  every record names the session that set it, so a 5.02 km run holding the 5 km record is starred
  whatever its logged distance). A **row opens its workout**: the coach's the session log dialog, set by set
...
- **PRs are the type's bests, every kind the logs carry.** `get_exercise_prs` (migration 192) returns
  ...
  whose hint names what the type logs (`PR_EMPTY_HINTS`: "Log a time over a race distance…" for
  Endurance and Erg).
- **The Overview announces every kind** ("Since your last visit", `services/client-activity-feed-service.ts`;
  owner, 2026-09-20) off the records the PR cards show, so the two agree, race distances included:
  for each exercise the new sessions logged (`exercisesLoggedIn`, by the same identity) it reads the
  records, and for one whose record a new session now holds it reads them again as they stood BEFORE
  the new sessions (`get_exercise_prs` with `p_exclude_dates`, the sessions' attribution days,
  excluded in SQL) — the second read happens only where a record changed hands. A record a new
  session holds that beats the record of the same kind, at the same distance, as it stood is a `pr`
  item carrying its `kind` (`prItemsFor`; `PrActivity`, `types/coach-brief.ts`); of the rep maxes
  only the heaviest weight is announced, and a first-ever value is a first, not a PR. The row reads
...
- `exercise_type` (`TEXT NOT NULL DEFAULT 'strength'`, migration 185) — **the exercise's type**: `strength`, `bodyweight`, `endurance`, `erg`, `carry_sled` or `holds`, defined once as `EXERCISE_TYPES` in `utils/exercise-types.ts`, which the CHECK mirrors (`utils/exercise-types.test.ts` reads the migration). A fact about the catalog row, never copied onto a library or client exercise row or a log snapshot — those reference the catalog by `exercise_id`, and the columns the coach chose are the prescription's own (`prescribed_fields`). It decides the column preset an exercise starts on when it is added to a session — the keys are the presets' keys, so `presetColumnsForType` (`utils/column-presets.ts`) is a lookup — and the markers its progress chart leads with (`EXERCISE_TYPE_MARKERS`, `utils/exercise-progress-markers.ts`; see "Exercise progress: charts and PRs"). Every row has one. A coach-made exercise starts as Strength and is edited in the exercise form's Type field (global rows stay read-only). That form is the only way an exercise is made: the builder's exercise search offers a name the catalog doesn't have as "Use “…”", which opens the form on that name (`AddExercisePopover`), and the exercise it creates joins the session being edited; a save matches exercise names to the catalog (`resolveExercises`) and leaves a name it can't match with no catalog link, never adding to the catalog. The global catalog is classified by name in migrations 185 and 189: compound and isolation are Strength except the carries, sleds and holds picked out by name and the exercises done with bodyweight alone, which are Bodyweight — a "Weighted" variant stays Strength and a static stretch is Holds (owner, 2026-09-21) — and the cardio and plyometric exercises were classified by hand (owner, 2026-09-19 — the bikes sit with the rower and ski as Erg, the climbers and elliptical are Endurance, Double Under and Jumping Jack are Bodyweight, the med ball throws and weighted jumps are Strength). `scripts/data/exercises.csv` carries the same classification in its Type column, row for row, which the same test proves by running the migrations' name lists in order, and `scripts/seed-exercise-catalog.ts` refuses a row with any other value. The Exercises tab's cards read the type after the muscle group and equipment; the client catalog reads (`GET /api/client/exercises` and the delta feed) carry it. Category stays separate free text, set in the exercise form; the assistant's exercise search names a compound as one (`search_exercises`).
...
A chat assistant docked in the program builder executes natural-language edits on the coach's draft ("duplicate week 1 three times, adding 2.5 kg to the bench each week"). Its shape is **unlike every other AI feature here** — one-shot generation writes to the DB; this writes nothing.
...
**Weeks.** `duplicate_week` copies a week exactly — how many times, and after which week. A progression or a deload is the copies edited afterwards with the exercise tools: the model works the numbers out, and its reply quotes the values the program then holds.
...
- **Journey** (`/client/metrics`, `components/client-portal/metrics/metrics-hub.tsx`): progress hub — body metrics, habit progress + streaks, and trends. Its physique and wellness series come from `GET /api/client/progress` (`services/client-portal-progress.ts`), over the days asked for, counted back from the client's today, and read by the server for the client the route verified: each physique metric's day-values from the measurement log (`dayValues`), and each wellness score from the client's daily log, one value per day they logged it (`wellnessDayValues`). The coach's Journey derives both through the same two kernels, so both apps chart the same value for the same day; a check-in's five wellness figures are the averages it reported and are on neither chart.
- **Program** (`/client/program`): the client's **goal card** first (`components/client-portal/program/goal-card.tsx` — the goal in force, block or no block: its name, each target with how far to go, its deadline and its description, from `GET /api/client/journey`), then the client's current training plan + nutrition plan cards, with the **week view** above them (`components/client-portal/program/training-week-layout.tsx`): the current training week from `GET /api/client/training/week`; tap a session, then a day, to move it. A session put on a day that holds some joins it, after them, in the order the client moves them — the order the write lists them and the server lands them — so a swap is two moves, and any rearrangement can be saved. Save is ONE `POST /api/client/training/events/layout` carrying every changed session with the day it was read on, so a 409 (the week changed since it was read) shows the server's sentence with a Reload. The arithmetic — the week with unsaved moves applied and the write — is the pure `lib/week-layout.ts`; the component only renders and taps. Tap-to-move, no drag: the web app is the harness.
...
Portal services follow the service-layer default (CONVENTIONS §8): **`supabaseAdmin` with a caller-verified scope.** The `/api/client/**` routes resolve `clientId` through `requireClientAuth(request)` (`lib/require-client-auth.ts`: the `clients` row whose `user_id` is the one the session validated, active only, read through `supabaseAdmin`) and pass only that authenticated id down; services filter on it with `.eq("client_id", clientId)`. The client's own profile is read the same way: `getClientForCurrentUser` (`services/client-portal-service.ts`, `GET /api/client/me`) reads the row by that id, while the client is active. The filter is each read's whole scope; `lib/session-client-ownership.test.ts` holds every reader to the service role.

### API surface

**Reads:** `GET /api/client/day-summary?date=` (home payload `{ training[], nutrition, wellness, habits }`, `no-store`) · `GET /api/client/training/week?date=` (`ClientTrainingWeek`, `types/client-training-week.ts` — every session in the check-in-anchored week containing `date`, by date and each day's sessions in the day's order, each with a `state` of done / today / upcoming / missed derived against the client's today; a day can hold several, so a week can hold more than seven; `no-store`. Powers the session picker and the week view, and is exactly the set a layout write may touch) · `GET /api/client/training/events/[eventId]` · `GET /api/client/daily-logs/[date]/{wellness,nutrition}` · `GET /api/client/habits` + `GET /api/client/habits/logs` (habits are **not** under `/daily-logs/[date]`) · `GET /api/client/training-plan` (date-resolved; carries `state`/`startsOn`/`endsOn` — see "Client-side plan tier") + `GET /api/client/nutrition-plan` (Program tab) · `GET /api/client/journey` (Program tab's goal card and blocks; carries `currentBlockNotes: { blockId, notes[] } | null` — the coach-note visibility policy is enforced **here on the wire**, not in the renderer, so RN inherits it rather than re-deriving it; see "The two notes" above) · `GET /api/client/training/exercise-history` (bounded full return) · `GET /api/client/check-ins` (**keyset-default**, opaque base64url `{createdAt,id}` cursor via `lib/cursor.ts`; legacy `?offset=` opt-in).
...
- `revalidateOnFocus: false` — except a read whose writer is in someone else's browser: the training calendar refetches when the coach comes back to the page (a client moves their own sessions), as the check-in queue and the Journey's wellness series do
...
|-----|-----------|-------------|
| Overview | `ClientOverviewTab` | Seven sections, top to bottom: identity row · status band (whole-journey chart ∥ four structural cells) · Needs attention + Since your last visit · Current plan · Adherence (three dot rails) · Daily wellness (five cards) · Coach notes. Every editable fact lives in the details sheet — the goal in the goals sheet — not on the page. See "Coach client Overview" below |
| Metrics (**labelled "Journey"**) | `MetricsTabContent` | Five panes via `?journey=` — **Physique** (the hero's switcher selects ONE metric, carried as `?metric=`, and the hero, the progression chart and the measurement log all describe it: the chart over the measurement log's day-values, the log listing ONE ROW PER READING of the selected metric — newest day first, within a day the most recently written first, each with its source, its note and its change against the previous day's standing value, a removed reading muted with who removed it and when; its pager counting that metric's readings ("Showing 10 of 12 weight entries") and its empty state naming it; all from `GET …/measurement-series`: the day-values, the derived baseline, the start date, the readings list, readings before the start under "Before start". Three hover-revealed row actions — Edit reading and Remove reading on any live reading (Remove behind the destructive confirm), Restore reading on a removed one — see "client_measurements table" rule 8), **Wellness** (the same hero, chart and log over the client's own daily logs — one value per score per day from `GET …/wellness-series`; its log rows carry no action; its hero's Total change is the last 7 days' average against the client's first week of entries, "Too soon to compare" while the two share a day, "Not enough entries" while either holds fewer than three), **Training** (`ExerciseDataView`, moved here from the Training tab in Session 7.1 — analytics live in Journey, prescription stays on its own tab: an exercise's chart by its type's markers, the table of its sessions beneath it, and its PRs, see "Exercise progress: charts and PRs"), **Blocks** (`client_phases`; see "Journey blocks"), and **Goals** (see "The goals table"). **Under a metric pane's hero, three cards keep one label and one window for every client** (`utils/metric-derived-stats.ts`): the last 7 days' and the last 30 days' averages of the entries dated in them — ending the client's today, `clientToday` on both series — each against the same span before it, and card 3 by metric: the goal on weight and body fat, the lowest score of the last 30 days on a wellness score (the highest for stress and soreness), the last 90 days' average on a girth. A wellness average needs three entries, else "Not enough entries"; a measurement's needs one. **Each pane loads only what it shows**: `MetricsTabContent` reads nothing itself, and each pane calls its own reads — Physique the measurement series, the goal and the blocks (the chart's bands; `usePhysiqueMetrics`, `PhysiquePane`), Wellness the wellness series and the blocks (`useWellnessMetrics`, `WellnessPane`), Training its exercise reads, Blocks the blocks and their facts, Goals the goals table, the measurement series and the goal — so a pane's data loads when the pane first opens (`metrics-tab-content.fetch.test.tsx` records the reads each pane requests). **"Log measurement" sits on the Physique pane alone**: its dialog lists the seven physique metrics from the catalog (`BODY_METRIC_DEFINITIONS`), so opening it reads nothing, and opens on the pane's selected metric. A reading appends to the log (`useLogMeasurement`); the pane on screen is refreshed IN PLACE — the dialog spins until the new reading is on it — and a store no pane on screen shows is CLEARED (CONVENTIONS §7), so the next Physique or Goals view, or the Overview, opens on its loading state and never on the old reading. The dialog is local state, so a browser Back while it is open leaves it open over the pane the address now names, and a save submitted there refreshes that one. `JourneySubtab` is deliberately WIDER than `MetricTab`: Goals, Training and Blocks key none of the metric shapes (`DEFAULT_FOCUS`) and are mapped onto `"body"` by `toMetricTab()`, a whitelist so the next pane is safe without editing it |
...
building it. Its infinite reader keys **page n on page n-1's `nextCursor`**, and that derivation is
...
COUNT would buy nothing. The reader also revalidates on focus and revalidates its first page — the
...
Five params are **one-shot**, consumed and stripped by the surface that receives them: `?edit=1` opens the Nutrition plan drawer — with `?startsOn=<day>` beside it, the drawer opens with its Starts on already on that day, captured in the same update that opens it so the drawer never opens on another day first (the Overview's "Set nutrition from 19 Oct", `nutritionDrawerParams`; a day that is not a real date is ignored) — and `?returnTo=journey&returnBlock=<id>` names the Journey block to return to on a **successful save** (`hooks/use-journey-round-trip.ts`: `useJourneyRoundTrip` on the drawer consumes all three; `useJourneyReturnBlock` on the Training apply tray captures the block and strips the return params alone, since `?apply=1` is the tray's address and stays); the return lands on `?journey=blocks&block=<id>`, and `?block=<id>` opens that block's card on the Blocks pane, winning over the default-expanded current block (`hooks/use-journey-focus-block.ts`). Stripping on arrival is not tidiness — the whole query is carried across every tab change, so a `returnTo` outliving its own flow would bounce a coach to Journey after a **later, unrelated** save, and a lingering open-param would re-open the surface on every hand-return to the tab (Radix unmounts inactive `TabsContent`, so each visit is a fresh mount). The drawer's hook drops the return target and the start day on any close without a save and on a hand open; the tray's captured block survives the pick and the editor's arrow (both replace the address) and is cleared by the X and by a hand open. Both ways in — the empty state's "place one" / "set targets" and the set state's "update plan" / "update targets" — go through one handler per track and are offered on **current and future** blocks only; elapsed and archived keep plain text (`blockAcceptsSetup`). The fifth, `?editGoals=1` (`OPEN_GOALS_SHEET_PARAM`, `hooks/use-goals-sheet-trip.ts`), opens the Overview's goals sheet on arrival; the check-in goal strip's `Set new goals` sends a coach there with `checkIn` cleared in the same navigation, so the Overview's address carries no check-in.
...
**Every editable fact lives in the details sheet** (`components/clients/details/**`), opened by the identity row's pencil, the "Set a schedule" action and the activation banner's Client-profile row — **except the goal, which has its own sheet** (below). The page itself is read-only; there is no inline editing and no dark editable surface.

**The goal card and the goals sheet** (`components/clients/overview/goal-cells.tsx`, `components/clients/goals/**`). The band's goal cells are the goal card: the goal in force — its name, its type where the name does not say it (`goalTypeBesideName`), the goal planned next — its targets, each with its chip, and its deadline, under the type's label (`GOAL_TYPE_SETTINGS.deadlineLabel`). The card's pencil, and `?editGoals=1`, open the goals sheet, local state keyed per opening: today's goal and the planned ones, each with Edit and Delete, and Plan a goal, one form open at a time in place of what it edits. The form (`goal-fields.tsx` over `use-goal-draft.ts`, shared with the manual Add client) asks for the type, the name (the type's until the coach types one), then the target the type needs and any the goal already holds (`goalFormTargets`), a start day for a new or planned goal floored on the client's today, the deadline — optional for every type — and the description, and warns under a target pointing the other way from its type (`targetAgainstType`). Targets are required and bounded only where the save writes them: a goal that started before today keeps its own unless the save makes a new goal, so a target it has always lacked never blocks its deadline. A save makes the writes `goalSaveWrites` (`lib/goals/goal-form.ts`) calls for: an add for a new goal; a whole rewrite for a planned goal or today's; for a goal that started earlier, its deadline then its labels, or — its type or a target changed — a new goal from today. A refused write leaves the form open with the rule's sentence while the fields and the planned goals are as they were when refused. Delete sits behind the destructive confirm, typed (DELETE) for the current goal. Every write's answer is landed (`useGoalWrites`) — the last in the tick its surface closes, and the first of a save's two writes (a started goal's deadline, then its labels) at once, so a failed second leaves the form open on "Partly saved" — a save closes the sheet, a delete its confirm: seeded into the goals read (`useSeedClientGoals`) and never refetched, with the goals table (`useClearClientGoalHistory`), the `/nutrition/goal` area and every check-in comparison (`useClearCheckInComparisons`) cleared. A goals read that fails over goals already on screen leaves them there. The band's footer links to the goals table (`?journey=goals`).

Eight SWR reads back it, all coach-scoped under `/api/clients/[id]/`:

| Endpoint | Serves | Notes |
|---|---|---|
| `GET …/overview-brief` | Needs attention, the activity feed, the identity row's check-in cluster, the wellness flags | Never moves the `last_viewed_at` anchor; a first visit, which has none, starts it (below). Key from `hooks/use-client-overview.ts`, whose `useClearClientOverview` every calendar writer and every measurement writer on the Journey calls (cleared, not revalidated: the rows are definite answers) |
| `GET …/goals` | The goal card, the chart's goal line and the goals sheet | `{ current, planned, clientToday }` — today's goal (with the client's readings on its start day, which the chips measure from), the goals planned after it and the client's today — via `hooks/use-client-goals.ts`, which owns the key builder, the matching area invalidator, the seed a goal write lands its answer with and the clear of the goals table beside it. The tab resolves `current` through `resolveEffectiveGoal` for the chart; the card and the sheet read the goals as stored |
| `POST …/overview-brief/seen` | "Mark seen" | The ONLY writer that moves `coach_client_views.last_viewed_at`; returns `{ lastViewedAt }` nested under `data` |
| `GET …/nutrition/goal/out-of-date` | The nutrition card's out-of-date line, between its title and its numbers, on the running and the queued card | The out-of-date rule's answer (see "Nutrition plan versions"), `hooks/use-nutrition-goal.ts` — the key builders and the clearer of the `/nutrition/goal` area, which every goal, reading and profile writer calls, and the three plan writers (the drawer's save, the calendar's delete, the blocks screen's trims and deletes); `hooks/use-nutrition-goal.test.ts` scans for the goal and reading writers. Cleared, not revalidated: the line is a definite answer. Regenerate opens the drawer from today, "Set nutrition from <day>" from that day, both in one navigation (`?nutrition=plans&edit=1[&startsOn=]`) |
...
- **The anchor moves only on "Mark seen".** A page load never moves it, so it cannot silently clear the coach's unread feed. The first visit, which has nothing unread, starts it: the brief writes the anchor only when none exists (`startLastViewed`, an insert-if-absent on the `(coach_id, client_id)` key, stamped by the database's clock like every row the feed compares it with) and answers it like any later visit — "You're all caught up" — so every load of the Overview gives the same answer and everything logged after it lands in the feed. The first-visit state ("First time viewing this client") shows only while starting fails, and the next visit tries again.
...
- **Goal chips come from `lib/goals/goal-chip.ts`** over `lib/goals/goal-state.ts` (reached / beyond / gap) — the client's goal card says the same words — measured from the client's reading on the goal's START day (`current.startReadings`), in the goal type's direction (`goalDirection`, `lib/goals/goal-types.ts`: losing weight counts down, building muscle up, a recomp's body fat down; any other type, the side of the start the target sits on). The under-vs-over wording follows that direction. The footer's "Since start" is the client's origin, not the goal's: it measures from the baseline.
...
Because the route layer is the perimeter, every authenticated route manually verifies the ownership chain before calling a service. Auth proves identity, not permission — never skip the ownership step because authentication succeeded.
...
- **Table**: `audit_logs`. Fields: `actor_id` (coach/client id, or null for system), `actor_role` (`trainer` | `client` | `system`), `action` (dotted key, e.g. `goal.create`), `target_table`, `target_id`, `client_id` (tenant scope), `metadata` (small non-sensitive context — never health PII), `ip_hash` (SHA-256 prefix, never the raw IP), `created_at`. Indexed `(client_id, created_at DESC)` and `(actor_id, created_at DESC)`.
- **Usage**: routes call `recordAuditEvent(...)` **fire-and-forget** (`void`-prefixed) after a successful, already-authorized write. Action names come from `AUDIT_ACTIONS` (`lib/constants.ts`). Live call sites include client invitation/activation, every goal write (`goal.create`, `goal.update`, `goal.deadline`, `goal.rename`, `goal.delete`), training placement, the plan editor's save, a program's start-date move, measurements, nutrition plan creation, the nutrition plan deletes (the calendar's and the per-version one), and intake metrics sync. Failures go to Sentry, never to the user.
...
Coach reviews intake
  ...
  -> "Sync metrics" pushes height/age from client_intake onto the profile,
     appends the intake's weight and body fat to the measurement log as
     `intake` readings (only for a metric the client has no reading of), and
     sets the client's first goal from the questionnaire — its type, targets,
     their own words, and the deadline while it is still ahead — when they have
     no goal in force (see "client_goals table").
...
| 2 | Your Goals | Primary goal type, target weight (conditional), goal body fat % (optional), deadline, the goal in the client's own words, motivation |
...
**A sent check-in is frozen in time** (owner ruling 2026-09-22): everything it shows is what it showed when the client sent it, whatever the coach does
...
the goal's start, the client's baseline, status, trend and pace, the deadline) — numbers, worded
when read, so a sent check-in reads in the current words; the reading as of the
...
question in the wording the client saw. A later shape is a new version beside the earlier ones:
version 2 records a row's trend as a word, or none yet; version 1 recorded yes or no, which reads
as towards or away.
...
the two days, and, muted, the days to go — `Today` on the day, then the days since, the track
full. With no deadline, `No target to track progress against` stands there. With no goal in force
on the check-in's day the rail stands
over the empty state, "No goals have been set for {name} yet.", whose `Set goals` link opens the
goals sheet as `Set new goals` does. **One advisory
footer, and goals outrank nutrition**: every judged goal met (a `No reading yet` row neither earns
the note nor blocks it) → "Goal met - consider setting a new target." with the `Set new goals`
button while the goal judged is still the one in force today (`goalIsCurrent`), and nothing once it
has been replaced — a page about a goal since replaced never invites replacing it again. The button
sends the coach to the Overview with `?editGoals=1` and
`checkIn` cleared (the goals sheet opens on arrival, and the Overview's address carries no
...
that day), `goalProgress` — the goal in force on the check-in's day by name, type and start day (`goal`, null
when none was), and position, trend and pace status per target against it, measured from the goal's
start reading, nothing projected — all from the check-in's copy, plus the live `goalIsCurrent` —
and seven `changes`: weight, body fat and the
...
- **Only the marketing repo's secret key writes it**, acting as `service_role`. The publishable key in the browser bundle is refused, as on every table, and an `anon` policy or grant must never be added: it would open a direct write path around the marketing form's honeypot and Turnstile.
