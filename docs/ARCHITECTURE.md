...
coaches
  ...
  └── clients                        -- coach_id FK, one coach per client
        ...
        ├── nutrition_plans           -- DATE-RANGED VERSIONS, each a PLACEMENT with a stored end (mig 166)
...
- **Both tracks are date-ranged placements with the END on the row:**
  ...
  - **Nutrition = coexisting date-ranged versions, each placed with an END on the row.** N `nutrition_plans` rows per client whose `[effective_from, effective_until]` windows never overlap (`nutrition_plans_active_window_overlap`, a gist exclusion scoped to active rows) and may leave gaps; `effective_until` is always written (migration 166). The coach never supplies the end — `resolveNutritionPlacementEnd` resolves it at save exactly as a training placement window is resolved: the block covering the start, else the furthest live program's end, else eight weeks, capped at the next queued version's start. A save caps its predecessor at `new_start − 1` and replaces in place a version starting on the same day (`create_nutrition_plan_atomic`); a delete ends the running version at yesterday and archives queued ones (see "Nutrition plan versions"). "The active version" resolves **by date** via the same `coversDate`; a day no version covers has no target, and the client's food log is refused there. One target per day is correct, not a limitation (`nutrition_events UNIQUE(client_id, date)`). Per-day coach edits are **materialized onto the events** (`is_modified`), never minted as versions. Inside a block the two tracks end on the same day by construction, because both resolve the same bound; a version saved in a gap between blocks falls through to the program, then to the fixed window.
...
- **Client-scoped with a SET NULL plan FK because both alternatives destroy it.** the nutrition clear (`clearNutritionPlansForClient`) spares nothing on `nutrition_events` — not `is_modified`, not `coach_note` — and `nutrition_plans` rows are themselves capped, replaced in place and archived by the save and delete paths. Same posture as the events-as-SOT rule that event→plan FKs are SET NULL (migration 113).
...
- `status`: the CHECK allows `scheduled` / `logged` / `missed`, but **the product never advances a row past `scheduled`** — no service, RPC or trigger writes the other two (a live probe of dev found every product row `scheduled`; its 365 `logged` rows are one seeded fixture year). "Logged" for nutrition is the existence of a `nutrition_logs` row for the date (`getNutritionForDate`; the day rule no longer asks the question — see "Date-edit permissions"). The coach calendar's edit gates key on `status === "scheduled"`, so every real day is editable, and a coach edit on a logged day reaches the client the same way the cascade does — at their next food save, which re-snapshots the target
...
**One question, one answer: from which day may a client's calendar events be REMOVED?** `resolveEventDeletionFloor` (`services/event-deletion-floor.ts`) is the only place it is answered — their today, or **tomorrow** if they have already touched today.

- **Replacing today is always fine; emptying it is the harm.** A placement overwrites the day in the same breath it clears it, and a cascade rewrites a target the client's next food save re-snapshots. An emptied day costs them their target for the rest of it and stores a blank one on every save after that. So removals ask this, and so does every plan START (next bullet) — regenerations and cascades still run from today.
...
- **"Touched" is one question with two answers**, because the tracks record it differently: a `nutrition_logs` row for the date, or a `training_events` row for the date that has left `scheduled`. A nutrition event's own status can never answer it — it never leaves `scheduled` (see the `status` bullet under "Nutrition event fields").
...
- **A deleted plan governs nothing from today.** The delete ends the running version, or program, at yesterday and archives queued ones; every read resolves by date, so no cascade can regenerate a date the delete cleared, and the hero, the Overview and the client's Program tab read "no plan" the moment it lands, while the past keeps its plan.
- **Callers, and they do no arithmetic of their own:** the block clear and the block delete's `clearEvents` (both through `clearScheduledEvents`), the uncovered-days sweep for the one stretch that can open on today (`sweepUncoveredNutritionDays`), the nutrition plan clears (`clearNutritionPlansForClient` — the calendar's delete and the block delete's "and its plans"), and both training-plan clears (the `cancelFutureEventsForPlan` call sites) — plus, in the other direction, the two start guards and the block extension above, and the blocks payload that carries the floor to the pickers. A removal that starts at the floor needs **no second "skip the day they logged" filter** — that day is never in range, and adding one would defend a state that cannot occur.
...
**There is no delete-the-days-but-keep-the-plan act on either track.** Deleting from the training calendar ends the program the client is on *and* removes their upcoming sessions; deleting from the nutrition calendar ends the version *and* clears the upcoming targets — the running plan at yesterday, a queued one archived, a finished one untouched. Both are labelled "Delete plan" for that reason — a plan left standing with no days would be restored by the next cascade.

### Training → Nutrition cascade
- The 8 training event-write routes — `place-from-library`, `events/[eventId]/move`, `events/[eventId]/duplicate`, `events/[eventId]` (DELETE), `[planId]/sessions/[sessionId]` (PUT), `[planId]/amendment`, `[planId]` and the client-level `training` DELETE — invoke one consolidated helper, `cascadeNutritionAfterTrainingChange()` (`services/nutrition-event-service.ts`), which fetches **every active version with days in the scope** — for a from-scope, every one reaching the anchor or later — and hands each the same scope — `regenerateFutureNutritionEvents()` clamps to the version's own `[effective_from, effective_until]` window, so the loop IS the segmentation and a training edit inside an old era rebuilds those days from **that era's** grid. From-scopes additionally sweep the days no version covers — between windows and past the last one — from the anchor as far as the furthest of the versions' ends, the caller's `to` and the client's last sweepable day, one range delete per stretch (`sweepUncoveredNutritionDays`, shared with the nutrition save); the stretch that opens on the anchor itself starts at the deletion floor, the only place a logged today can be in range. The version-lookup error is surfaced loudly (a failed read must never impersonate "no plan"); per-version regeneration failures are still Sentried-and-swallowed (`TECHNICAL-DEBT.md` → cascade entry 2). Each route threads a **`NutritionRegenScope`** naming the dates its change actually touched. Routes that know their exact dates pass `{kind:"dates"}` (move = `[source, target]`; duplicate / library-session drop = `[targetDate]`; event delete = the deleted day; a session surplus edit = the session's future event dates) and get a **pure upsert with no DELETE** — those days never lose their row. The old delete-then-regenerate left every covered date row-less across four network round trips, and `getPlanTargetForDate` resolves a missing row to null, which `nutrition_logs` snapshots permanently. Open-ended changes (whole-program placement, the plan-clear DELETEs, the amendment floor) pass `{kind:"from"}`: per version, a bounded DELETE + regenerate over `[max(from, effective_from), effective_until]` — the window stored on the row (see "The window is the row"). A version never writes past its own end. A plan deletion reaches further than any version: `cancelFutureEventsForPlan` returns the last date it deleted and the two plan-clear routes pass it as the scope's `to`, which widens only the gap sweep, so a stale training surplus past every version's end is removed rather than rebuilt. `to` only ever widens, never shortens.
...
How far a `{kind:"from"}` generation writes `nutrition_events` is the version's own `effective_until`, stored on the row at save (migration 166) and read from there by `regenerateFutureNutritionEvents` — no block, program or fixed window is consulted per call, and nothing is cached because nothing is derived. The end is resolved ONCE, at save, by `resolveNutritionPlacementEnd` (see "Nutrition plan versions"):
...
The two declared-bound reads degrade to null on error (logged + Sentried) rather than throwing, so a failed lookup falls back to the fixed window instead of failing a coach's save. `{kind:"dates"}` scopes resolve nothing: they upsert the named days that fall inside the version's window and drop the rest.
