...
- **A block owns its own window** (migration 164 — an owner reversal of the derived-start chain, 2026-09-04). The PUT sends `{ blocks: [{ id?, name, startsOn?, endsOn?, focus?, targetWeightKg? }] }` — every block carries BOTH dates, nothing is derived from its neighbour, and an end is never later than the one stored (below). **Gaps are allowed and mean nothing is planned** (the client is between programs; both placements read that as "no bound" and fall back). **Overlaps are refused** — in the service, with a sentence naming both blocks, and by the `client_phases_window_overlap` gist exclusion constraint, because "the block covering this date" decides the training placement window and the nutrition version's end and cannot have two answers. The service also rejects an end before its start, caps a block at 52 weeks of days, and refuses a NEW block opening in the past (a past-dated block prescribes nothing — placement and the nutrition save both refuse a past date). The payload carries every stored row, so checking it against itself IS checking it against the client's whole calendar. **Elapsed blocks (`ends_on <` client-today) keep their DATES pinned from storage** (they omit `endsOn` — elapsed history is not an input; a differing one 422s) **while their name/focus/target stay editable** (Session 3.6-C — the pin protects lived-day attribution, not typos in a finished label), and the **symmetric window floor** rejects any edit that would re-label lived days: a stored current block must still contain today; a stored future block may become current but never wholly past; new id-less rows land anywhere (history backfill). Removal is not expressible through the PUT.
- **Shortening a block can bring the calendar with it — never on its own.** Moving a block's end earlier raises a confirm dialog on the blocks screen INSTEAD of the save: every path through it completes the save, and dismissing it abandons the edit with the form still open, so dates are never stored beside a question the coach walked away from. One question (*Clear those days* / *Just the dates*). The dates land through the chain PUT and the calendar through `POST /api/clients/[id]/blocks/[blockId]/events` (`mode: "clear"`, `services/block-event-sync-service.ts`) — **sequential, not atomic**: one toast at the end, and the failure between them says the dates saved and the calendar did not. The clear RECONCILES rather than replaying a diff — it removes the scheduled sessions that now sit outside the block and pulls both tracks' windows back to it — so it needs no memory of the old window and re-running it changes nothing. It is bounded by the next block's start, floored at the shared deletion floor (below) and scoped to `status = 'scheduled'`; it deactivates the training slot rows behind the sessions it removes, caps every live program reaching past the new end at it and archives one queued to start in the cleared stretch (migration 167), pulls every nutrition version reaching past the new end back to it and retires a queued version that now starts in the cleared stretch, so the days a version answers for always end where its window does, and removes the hand edits on the days they lose. The sync writes `training_events` and moves both tracks' windows — which is what the nutrition days are computed from — so the call site invalidates the training and nutrition SWR areas as well as the blocks area. **A block's end never moves later.** The PUT refuses it with a sentence, for current and future blocks alike, and the form caps the end date at the stored end; more time is a new block after it, with its own program and targets.
...
- **Generated** at placement (the date-walk) and by the amendment's rewrite of the future
...
- **Blocked states:** an ended plan (window fully past) can't be amended — 422, entry disabled ("apply a new program" is the gesture). Shrinking the grid below the elapsed days is 422. An all-rest future is legal (an explicit deload).
