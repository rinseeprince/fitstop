...
| `GET …/goals` | The status band's goal targets, chips and deadline, the chart's goal line, and the details sheet's goal seeds | `{ current, planned }` — today's goal (with the client's readings on its start day, which the chips measure from) and the goals planned after it — via `hooks/use-client-goals.ts`, which owns the key builder and the matching area invalidator. The tab resolves `current` through `resolveEffectiveGoal` and passes the result down; the sheet seeds from `current` itself |
...
- **Goal chips come from `lib/goals/goal-state.ts`** (reached / beyond / gap), measured from the client's reading on the goal's START day (`current.startReadings`), in the goal type's direction (`goalDirection`, `lib/goals/goal-types.ts`: losing weight counts down, building muscle up, a recomp's body fat down; any other type, the side of the start the target sits on). The under-vs-over wording follows that direction. The footer's "Since start" is the client's origin, not the goal's: it measures from the baseline.
