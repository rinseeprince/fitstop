...
value, a track and a state column read direction before speed (see "Goal progress and pace"):
...
itself in their place, its name, its type where the name doesn't say it (`goalTypeBesideName`) and
`No target to track progress against`. With no goal in force on the check-in's day the rail stands
...
that day), `goalProgress` — the goal in force on the check-in's day by name and type (`goal`, null
