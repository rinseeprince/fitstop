...
- **Delete = remove the row, move nothing** (`DELETE /api/clients/[id]/blocks/[blockId]`), unless the coach asks for the plans too. With windows chosen rather than chained there is nothing to re-anchor: the row goes and every other block stays where the coach put it, leaving a gap — a legitimate state. No mode, no realized `changes`, no consequence sentence to preview. Elapsed blocks 422 (archiving is how a finished block leaves the list). The block's training and nutrition **events are deliberately left alone**: a block edit writes nothing on its own. **`?clearPlans=true` is that separate explicit act**, and it fires THREE THINGS THE COACH CAN ALREADY DO together — delete the nutrition plan (`orchestrateNutritionPlanDeletion`), delete the training plan (`clearAllTrainingPlansForClient`, shared with the training calendar's own delete), delete the block. Nutrition runs FIRST: its delete closes the covering version at the day before the deletion floor, so the training clear's cascade then governs nothing on those days and rebuilds none of them. Reusing those two paths is deliberate — a rule for which plans "belong to" a block is how a pointer architecture arrives by the back door. **Clearing the block's own days and leaving the plans standing is not an option**, because the plans regenerate those days on the next cascade and the delete undoes itself. The cost is accepted and stated in the dialog: both deletes run from the floor FORWARD with no upper bound, so a LATER block's program goes too and that block survives as an empty label. A 404 from the nutrition path (nothing left to delete) is not a failure; any other error stops the delete, because losing the block while its plan survives is the one outcome the UI cannot undo.
