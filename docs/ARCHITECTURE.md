...
- **Both tracks are date-ranged; what still differs is HOW windows are made (migration 144 aligned nutrition with training's date-resolved model — an owner reversal of the earlier "deliberate asymmetry", 2026-08-11):**
  ...
  - **Nutrition = a contiguous chain of versions, closed derivationally — and generating only inside the block its window opens in.** N `nutrition_plans` rows tile the client's timeline; the coach never supplies an end date — each save closes the open version at `new_start − 1` and inserts the next (`create_nutrition_plan_atomic`, close-and-insert, mig 144), so overlaps and gaps are unexpressible through the API (a gap exists only after a delete). One open version per client (`idx_nutrition_plans_open_unique`); a gist exclusion constraint (`nutrition_plans_active_window_overlap`) physically refuses overlapping active windows. "The active version" resolves **by date** via the same `coversDate`. One target per day is correct, not a limitation (`nutrition_events UNIQUE(client_id, date)`). Per-day coach edits are **materialized onto the events** (`is_modified`), never minted as versions. **A version generates events only inside the BLOCK its `effective_from` falls in** — the same shape training has, where a placement truncates its events to the block while `training_plans.effective_until` is never written at all: the bound lives in the GENERATION, not the row. The version still GOVERNS the days past that block (the row stays open, so the client's food log resolves rather than being refused); it simply does not materialise a prescription there. Without that clamp an open version reaches forward for ever, and a training placement anchored in a LATER block drags the previous block's numbers onto days the coach never priced — the block's card then truthfully reports targets nobody set. A version laid in a GAP has no block bound and keeps its own window; the tighter of the two bounds always wins.
...
- **A version's window ends where its events end.** The nutrition plan delete closes the covering version at `floor − 1`, so no version is left governing a date whose event it removed — otherwise the next cascade legitimately regenerates that date and the delete is silently undone.
- **Callers, and they do no arithmetic of their own:** the block clear and the block delete's `clearEvents` (both through `clearScheduledEvents`), the nutrition plan delete (`orchestrateNutritionPlanDeletion`), and both training-plan clears (the `cancelFutureEventsForPlan` call sites). A removal that starts at the floor needs **no second "skip the day they logged" filter** — that day is never in range, and adding one would defend a state that cannot occur.
...
How far forward a `{kind:"from"}` generation writes `nutrition_events`, resolved fresh on every call in `resolveNutritionHorizon` (`services/nutrition-event-service.ts`) and **stored nowhere** — no column holds it and nothing caches it:

1. **The end of the block the ANCHOR falls inside** (`getBlockEndCoveringDate`, `services/client-blocks-service.ts`) — a block is the time-bound program the coach sells, so its last day is the answer whenever the days being written sit in one. The block **covering** the anchor, never the furthest the client has: a later block the coach has not priced yet must not pull targets into itself, or its card reads "Not set" while its days already hold numbers. Its own plan save anchors inside it and resolves this then, which is when the coach has actually said what it costs. The same function answers the training placement window, so both generators read one bound.
2. **Else the furthest live training program's last day** (`getFurthestLiveProgramEnd`, `services/training-service.ts`) — `effective_from + its active day-row count - 1`, through the same `calculatePlacementEndDate` the client reader and the amendment surface use. Carries the `deleted_at IS NULL` / `status <> 'archived'` exclusions both sibling readers document, and **no date predicate**, so a queued program counts like a running one. **Not `effective_until`** — that column is NULL on every placed plan by construction (the placement RPC omits it and says so).
3. **Else a fixed 8 weeks** from the anchor (`calculateNutritionEndDate`) — all a client with neither a block nor a live program has.

Inside a block the two tracks end on the same day by construction, because the placement window is that same block (below). They diverge only where the rows and the block disagree — a program placed before its block existed, or one whose block was re-dated afterwards, since a block edit still writes nothing.

Precedence, not a maximum: past the coach's declared bound there are deliberately no events, so a client between programs reads as quiet until the coach draws the next bound. A block that never actually bounded anything would be decoration. Two consequences to know: a client with no block whose program ends in three weeks gets three weeks of targets, not eight; and a generation anchored in a GAP between blocks falls through to the program, because no block covers it.

Both reads degrade to null on error (logged + Sentried) rather than throwing, so a failed lookup falls back to the fixed window instead of failing a coach's write after it has committed. `{kind:"dates"}` scopes never resolve a horizon at all.
