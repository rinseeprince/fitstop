...
- **A setup surface reached from a block seeds its date picker with that block's start** (`useRoundTripBlockStart`, read through `readJourneyTrip` so the round-trip contract keeps one owner): the coach has already said which days they mean by clicking "place one" / "set targets", and typing the date again is asking them to repeat themselves. A SEED, never a binding — their own pick still wins, and it is floored at the shared deletion floor (see "The deletion floor" under Nutrition & Training Events) because both the placement and the nutrition save refuse a start before it, so a block already under way seeds today, or tomorrow once the client has logged today, rather than the day it began. **Route surface:** `GET`+`PUT /api/clients/[id]/blocks`, `DELETE …/blocks/[blockId]` — full coach chain (`coachApiRateLimit` → CSRF → `requireCoachOwnsClient`, foreign client 404), canonical kg on the wire, audit events `block.chain_update` / `block.delete`. The chain payload carries `clientToday` and `planStartFloor` — that deletion floor, resolved server-side because it depends on the client's logs — on every handler that echoes it (GET, PUT, PATCH, DELETE), because `useSeedClientBlocks` writes a mutation's response straight into the chain cache; both setup surfaces floor their date pickers on it, and a block itself is not constrained by it. RLS deny-all + `GRANT … TO service_role` only (CONVENTIONS §8). Plus the read-only **`GET …/blocks/facts`** (Session 3.2, `services/client-blocks-facts-service.ts`). **A block reports what is set only if it actually HAS DAYS inside it** — per track: on training, gated on the events, because a window says where a plan was placed to run and the events say which days it actually put on the calendar; so a block the coach has drawn but not set up never claims a program whose rows merely reach into it; on nutrition a day's target is computed from the version covering it, so an active version overlapping the block IS a day inside it, and the versions read answers the gate on its own. The gate is bounded at both ends of the block (days after it are not its days), status-agnostic (a completed day is still a day) and NOT clamped to today (a current block whose program starts tomorrow has days). It is per-track because the two are set up separately: a block can hold workouts and no targets. Given days: per-block training programs (governing segments, latest-start-wins), and the nutrition VERSIONS whose windows overlap the block — the same list shape as training (owner, 2026-09-10): one entry per active version, in start order, each carrying the numbers off its OWN row (daily calories with the custom-macros override honoured, deficit = `tdee − calories`) and its own start, rendered "from <date>" like the training column. Queued versions are included, a version that began before the block keeps its real start as a crossing program does, and a re-save with the same numbers is its own entry as a program placed twice is. Hand edits and training surpluses are excluded by construction — the plan row holds neither. Version windows never overlap (the gist exclusion) and may leave gaps, so the list is a plain intersection with the block window, no resolution rule, unlike training; a closed window is immutable, so an entry dated in the past never rewrites itself on a later save. **The "what happened" timeline lists each version that STARTS inside the block** as "Nutrition set" on its date, then "Nutrition changed", exactly as it lists a program started inside the block, whether the block has begun or not. The training-dates read is **paged** (`lib/paged-fetch.ts`) — an unpaged read silently truncates at PostgREST's ~1000-row cap; nothing per day is read for nutrition. Kept off the chain GET so PUT/DELETE keep echoing that GET's exact payload.
