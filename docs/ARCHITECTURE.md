...
- `services/comparison-service.ts` — the check-in review's goal strip: the goal version in force at the check-in's instant (`getGoalAsOf`), resolved with `today` = the check-in's day on the client's calendar, so weight **and** deadline come from one version and the clock is the check-in's (docs/MEASUREMENT-LOG-PLAN.md commit 8b). It composes its input from that `client_goals` row alone — no `clients.*` mirror leg — so a check-in older than every version has no goal on its page. The Overview and the Journey resolve the live goal at today.
