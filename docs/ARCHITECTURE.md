...
|-----|-----------|-------------|
| Overview | `ClientOverviewTab` | Six sections, top to bottom: identity row · Progression (window control + status band, chart ∥ cells) · Needs attention + Since your last visit · Signals · Current plan · Coach notes. Every editable fact lives in the details sheet, not on the page. See "Coach client Overview" below |
...
**The page has one window control**, on the Progression rail (`lib/overview/window.ts`, 30/60). It governs the two PERIOD surfaces — the progression chart and the Signals card, including its expanded panels — and deliberately nothing else. Goal targets, the energy pair, the deadline, the plan's week and the next check-in are STRUCTURAL: they describe a client rather than a period, and re-cutting them by a window would be meaningless. Two figures sit inside windowed surfaces but are not windowed, and each says so in its own copy: the status band's footer delta is lifetime (`Since start:` — mandatory, not decorative), and a wellness flag chip is a trigger's verdict over its own fixed window (`Flagged: high for 3 days`).
...
| `GET …/adherence?days=` | The Signals card's training / nutrition / habits rows AND their panels | `days` clamped to **[7, 60]**; rails are index-aligned with `dates`. Carries the window's nutrition means (`calories` / `protein`, each with its OWN day count — a day logged before a plan existed has a null target and is excluded from both) and `habits.perHabit`. **`perHabit` rides here rather than on `/habits/logs` on purpose:** `logHabit` upserts a row only when the client acts, so a habit ignored for the whole window has no log rows and a logs-derived grid drops it silently — the adherence read already holds this client's active habits, so the roster is free here and complete |
| `GET …/measurement-series?days=` | The progression chart | The SAME two tables and the SAME `buildMetricPoints` tie-break as the Physique view, bounded to the window and merged server-side. Physique gets there by eagerly paging the whole check-in history with `select("*")`; on the Overview that is years of AI-annotated rows to draw eight dots. Window anchor is `getClientTodayString` — the same client-local anchor `getClientAdherence` uses, so the chart and Signals cannot disagree by a day |
...
- **Two window-scaling rules, both of which only bite above 14 days.** The training day strip wraps into a weeks × weekdays grid above 21 days, keeping the day-of-week column alignment that is the only thing its labels buy; `WellnessSparkline` drops its interior dots above 20 points and keeps the last-point marker, which is the one carrying tone.
