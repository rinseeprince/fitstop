...
- **Callers, and they do no arithmetic of their own:** the block clear and the block delete's `clearEvents` (both through `clearScheduledEvents`), the uncovered-days sweep for the one stretch that can open on today (`sweepUncoveredNutritionDays`), the nutrition plan clears (`clearNutritionPlansForClient` — the calendar's delete and the block delete's "and its plans"), and both training-plan clears (the `cancelFutureEventsForPlan` call sites). A removal that starts at the floor needs **no second "skip the day they logged" filter** — that day is never in range, and adding one would defend a state that cannot occur.
