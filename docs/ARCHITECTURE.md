...
**Security.** RLS enabled. Every read of the table and its views is the service role's, scoped by the client the route verified: the client's own through `services/client-portal-progress.ts` (`GET /api/client/progress`) and the profile embeds under `GET /api/client/me`, filtered on the client and never by source, so a client sees every reading about them; every coach read the same way. The table keeps one policy, `clients_view_own_measurements` — `SELECT` for `authenticated`, scoped by `clients.user_id = auth.uid()` — which no read uses. `npm run check:rls`.
