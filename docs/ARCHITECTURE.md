...
|-----|-----------|-------------|
| Overview | `ClientOverviewTab` | Six sections, top to bottom: identity row · status band (whole-journey chart ∥ four structural cells) · Needs attention + Since your last visit · Signals (fixed 14 days) · Current plan · Coach notes. Every editable fact lives in the details sheet, not on the page. See "Coach client Overview" below |
...
- **Signals is a FIXED fortnight** (`SIGNALS_WINDOW_DAYS`, `lib/overview/window.ts`). It answers "where is this client at", which is a glance — the selectable 30/60 buried the days a coach can still act on inside a quarter of history. Long ranges live on the Journey tab.
...
| `GET …/adherence?days=` | The Signals card's training / nutrition / habits rows AND their panels | The Overview passes `SIGNALS_WINDOW_DAYS`; the route clamps to **[7, 60]**, wider than any caller needs; rails are index-aligned with `dates`. Carries the window's nutrition means (`calories` / `protein`, each with its OWN day count — a day logged before a plan existed has a null target and is excluded from both) and `habits.perHabit`. **`perHabit` rides here rather than on `/habits/logs` on purpose:** `logHabit` upserts a row only when the client acts, so a habit ignored for the whole window has no log rows and a logs-derived grid drops it silently — the adherence read already holds this client's active habits, so the roster is free here and complete |
| `GET …/measurement-series?from=` | The progression chart | The SAME two tables and the SAME `buildMetricPoints` tie-break as the Physique view, merged server-side. `from` is the client's start date, sent by the browser because it already holds the client record — so bounding costs no round trip, and it is semantic too: a measurement recorded before they started is not part of the journey. Validated through `validateDateParameter` before it reaches a PostgREST filter (`daily-logs` and `habits/logs` take the same shape of param and do NOT validate it — a gap there, not a pattern to copy). Two unpaged selects of four and five columns; Physique pages the same history at `select("*")` over 37 columns in requests of 20 |
| `GET …/daily-logs` | The Signals card's wellness panel | Through `useWellnessData(clientId, { daysBack, withHabitLogs: false })` — the habit fetch stays OFF; `perHabit` above is why |
...
- **Five wellness metrics**, Soreness included, in the Signals card's wellness panel. Stress and soreness are inverted (lower is better) through `getWellnessTone()` in `utils/wellness-color-thresholds.ts` — the single source for that inversion, shared with `components/check-in/mini-bar-sparkline.tsx`. **Sleep has no trigger in `lib/wellness-triggers.ts`, so it can never flag; do not invent one.** The wellness row carries **no percentage and no bar**: there is no composite wellness score in this codebase, only per-metric tones, and one invented here would be new business logic wearing the same shape as three real numbers. It leaves those slots EMPTY rather than reflowing, so the four rows still read as one set.
- **Two density rules.** `WellnessSparkline` drops its interior dots above 20 points and keeps the last-point marker, which is the one carrying tone; the progression chart drops its raw dots above 40 readings for the same reason. The training day strip has no such rule and needs none — it is one row of `SIGNALS_WINDOW_DAYS` columns. (It briefly wrapped into a weeks × weekdays grid, which only earned its keep at the 30/60 window that is gone.)
