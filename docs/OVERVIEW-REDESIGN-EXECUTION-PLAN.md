...
- ~~**A partial goal PATCH silently clears the other goal field.**~~ **FIXED 2026-07-28** (Client-Goals-and-Phases Session 1.1). `PATCH /api/clients/[id]` with only `goalWeight` dual-wrote `client_goals` through `updateGoals({ goalWeight, goalBodyFatPercentage: undefined })`, which superseded the active goal with a row whose body-fat target was NULL — and the `clients` cache followed. Reproduced three times during the goal-chip smoke and repaired by re-PATCHing both fields. The root cause was `updateGoals`' presence test: `hasOwnProperty` is true for a key present with value `undefined`, and all four object-literal callers build a fixed key set. Presence now means present **and** not `undefined` (`services/client-goals-service.ts:74-89`), so **callers no longer need to send both fields** — the sibling carries forward. Explicit `null` still clears (`null !== undefined`), so the Goals editor's deliberate clears are unaffected.
