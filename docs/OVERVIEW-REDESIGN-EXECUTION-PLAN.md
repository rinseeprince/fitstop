...
ActivityItem =
  ...
  | { type: 'measurement';       at: string; metricKey: string; value: number; previousValue: number | null }
  // ⊘ `unit` was dropped from this payload (units canonicalization). Values are
  // canonical kg/cm; the label resolves at the render boundary from the VIEWER.
...
- **Feed sources** (all `> lastViewedAt`, merged newest-first): `check_ins.created_at`; `client_metric_entries` (previous value = the prior same-`metric_key` row by `entry_date`; values are canonical kg/cm — mig 132 stores no per-row unit, and the label resolves at the RENDER boundary from the viewing coach's preference, not the client's); `session_logs.created_at` with `completion_quality != 'skipped'` (exerciseCount = count of its `exercise_logs`); PR detection per decision 5 — for each exercise appearing in the new sessions' non-warmup `set_logs`, compare its new max weight against the pre-anchor all-time best (reuse the `get_exercise_prs` RPC per exercise and filter its rows to `completed_at` before the anchor, or an equivalent scoped aggregate). Skip PR computation entirely when new sessions > 20. Weight units follow the viewer: `formatLoad` for a PR (it snaps), `formatWeight`/`formatLength` for measurements — see `CONVENTIONS.md` §20.
...
| Client settings dialog | new `overview/client-settings-dialog.tsx` | Standard Teal-Summit Dialog; profile fields (gender, height, started date, phone — height has NO unit select; it renders in the coach's own units and converts on submit) → existing `PATCH /api/clients/[id]`; check-in day → existing `PATCH .../check-in-config`; toast + mutate `useClient` |
...
3. **`metricEntryUnit()` added to `lib/metrics/metric-entry-definitions.ts`.** The contract's "unit resolved at read time from client prefs" had no server-side implementation — the only unit map lived in a client-component hook. Mirrors it exactly (weight → `clients.weight_unit`; girths pinned to `in` as the coach surface does; `%`, `/5`, `/10`). — **⊘ Since deleted (units canonicalization, 2026-08-07).** `clients.weight_unit` no longer exists and pinning girths to inches was itself a bug: stored girths are centimetres. The unit now comes from the VIEWER via `getUnit(preference)`; see `CONVENTIONS.md` §20.
