 ...
 * matters — the regen skips `is_modified` days), then regenerates exactly that
 * date so the now-unfrozen day is re-derived from the plan. Other edited/logged
 * days are untouched because they are not in the date list at all.
 */
 ...
 * regenerating, since the regen skips is_modified rows. Returns the days reset.
 */
