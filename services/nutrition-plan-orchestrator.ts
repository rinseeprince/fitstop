 ...
 * client's Program tab all read "no plan" the moment it lands. The client's
 * meals still save on those days, with no target to judge them by.
 *
...
export async function orchestrateNutritionPlanCreation(
  ...
  // today; a logged today reads the new target at once, because a day's
  // target is computed and the log stores none. The deletion floor is
  // training's alone.
  ...
  // A save is the RPC and nothing else: every day inside the window is
  // computed from the row the moment it commits, a logged today included.
  return body.customMacrosEnabled
    ? handleCustomMacros(clientId, coachId, body, calcInputs, validatedData, effectiveUntil)
    : handleCalculatedPlan(
        clientId,
        coachId,
        body,
        client,
