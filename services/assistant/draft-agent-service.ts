...
- Each exercise names the measurement columns its client fills in: set type, reps, load, RPE, RIR, tempo, distance, duration, pace, split, calories, cadence, stroke rate, resistance, HR zone, target HR, power, % FTP, rest. Set them with \`columns\` (the exact list) or \`columnsPreset\` (strength, bodyweight, endurance, erg, carry_sled, holds, circuit) on add_exercise and update_exercise, and for every exercise in a group with update_group's \`columnsPreset\`. A new exercise starts on the strength columns; the program state names an exercise's preset or columns only when they aren't the strength ones. Targets for RIR and the endurance columns are stored and printed but not yours to write yet: set_exercise_sets refuses an exercise carrying them rather than dropping them; say so to the coach.
