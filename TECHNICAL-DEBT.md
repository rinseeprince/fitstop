...
| 6 | `getClientNutritionTargets` couples a read to a write + uses session-scoped Supabase | `services/client-portal-service.ts:59-137`, callers including `app/api/client/nutrition/route.ts`, `app/api/client/nutrition-plan/route.ts` (Session 2.9) | Two related smells in one function. **Smell 1**: line 77 calls `promoteNutritionPlanIfReady` which can archive the active plan, promote a planned plan, and regenerate future nutrition events — a meaningful write inside a function called `get*`. Every caller silently inherits the side-effect. The intent ("lazy promotion so callers always see the right active plan without a cron") is valid, but the bundling is opaque. **Smell 2**: the function uses `createPortalClient` (session-scoped, RLS-bound) where CONVENTIONS §8 calls `supabaseAdmin` the default for services. Functionally identical for the access pattern in normal flow (both apply `.eq("client_id", clientId)` against a `requireClientAuth`-verified id; RLS is belt-and-suspenders), but legacy by convention. **Fix pattern**: extract a pure `composeNutritionTargets(plan, dailyTargetRows, trainingPlan, trainingEvents, clientFlags)` builder (admin-scoped reads inside). Provide two orchestrators: `getClientNutritionTargetsWithPromote(clientId)` (promote + fetch + compose; what existing callers want) and `getClientNutritionPlanReadOnly(clientId)` (fetch + compose; what plan-overview GETs want). Session 2.9 accepted the reuse for scope discipline and logged the trade-off. Not urgent — promote-on-read is the desired behavior for the card today, and the legacy auth pattern carries no security delta under Shape B. Tackle next time a nutrition-read endpoint needs a side-effect-free read or when the service is touched for unrelated reasons. | Open |
