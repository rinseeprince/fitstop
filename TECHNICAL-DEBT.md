...
> Context: a full read-path trace on 2026-07-30 (every Supabase client factory enumerated, every `.from()` receiver resolved) established that the app then had **two** data paths. It has one now: every query, function call and storage call is the service role's, and a session client calls `auth.*` alone (`lib/session-client-ownership.test.ts`; docs/DATA-ACCESS-LOCKDOWN-PLAN.md commits 3–5). The policies stay until commit 6, which closes this section.
>
...
- **`scripts/assert-rls.ts:103`'s premise — "this app's entire data path is service_role" — is true of the code since 2026-09-25 and not yet of the gate.** Its `ANON_REACHABLE_ALLOWLIST` at `:107` is empty and the gate still judges policies one by one rather than holding the database to none; commit 6 of docs/DATA-ACCESS-LOCKDOWN-PLAN.md adds the clauses that hold it (no policy, no public-role privilege) and makes the header say so.

  The session-read inventory this entry kept is **empty**: 0 session reads, 0 session writes, held by `lib/session-client-ownership.test.ts` (every query, function call and storage call in the app is `supabaseAdmin`'s). Re-read 2026-09-25 after commit 5 moved the last four — the middleware's `profiles` reads and the auth helpers' `coaches` and `clients` lookups — onto the service role keyed on the validated user id; the 2026-07-30 trace (two independent derivations plus two adversarial passes) had found 37 anon call sites, 36 SELECT + one UPDATE, across six tables.

  The four universal gates — the middleware's role read, the seam's coach and client lookups — read through the server now, so the three policies migration `137_comment_universal_gate_policies.sql` labelled as gates (`profiles`, `coaches`, `clients`) govern no app read; commit 6 drops them with the rest.
