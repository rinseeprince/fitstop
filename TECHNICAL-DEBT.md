...
- **Write rules on 23 tables** - Hardened 2026-09-24 (migration **200**). 55 write rules let a signed-in user write their own rows straight through the Data API with the browser's anon key and their own JWT, skipping every route's rule: a client could rewrite a closed week's logs and post a check-in, a coach could hard-delete a client. Nothing used them; all 55 are dropped. The one write rule left, "Coaches can update their own clients" on `clients`, had no user since activation writes through the service role (`a36a095a`); 201 dropped it.
- **Every policy, and every public-role privilege** - Hardened 2026-09-25 (migration **201**, `6010ee99`). Once commits 3–5 moved every session read onto the server, the 46 SELECT rules in `public` and 6 rules on `storage.objects` governed no app read, yet each was a door the browser-shipped public key could open with a login; and the stock `GRANT ALL … TO anon, authenticated` on 42 tables and a view left RLS as their only perimeter. 201 drops every policy, revokes every `anon`/`authenticated`/PUBLIC privilege on every table, view and sequence in `public` and postgres's default privileges that would hand a new one to them, and makes every SECURITY DEFINER function `service_role`-only (the auth trigger also `supabase_auth_admin`). RLS stays on every table as the lock. `scripts/data-api-locked-proof.ts` proved the Data API open before and refused on every relation after. **PROD has not received 200 or 201** (it is at 184 with 185–201 pending): push them only after the app code of commits 3–5 (`a36a095a`, `6f76957a`, `534d852b`) is deployed there — an older build's session reads depend on the rules 201 drops and would fail the moment the push lands.
- **How to check this list is still true:** `npm run check:rls`. It reads the live catalog (not the migration tree) and asserts RLS on every public table, no policy in `public` or `storage`, no `anon`/`authenticated`/PUBLIC privilege on a public relation (nor by default on a new one), and `security_invoker` on every view. The tree is not trustworthy here — see the drift incidents recorded below.
...
- **Two orphaned `upsert_daily_log_atomic` overloads** (6-arg 057:7, 8-arg 059:18) are still in the catalog on both projects — read from the live `pg_proc` on 2026-09-25, `service_role`-only like every SECURITY DEFINER function there — with zero callers, and since migration 173 a body that names food-log columns that no longer exist. Drop them in their own migration.

### Opened by the 2026-07-30 read trace — auth and scoping items still open

> The trace found a second, anon-key data path. It is gone: every query, function call and storage call is the service role's, a session client calls `auth.*` alone (`lib/session-client-ownership.test.ts`), and migration 201 dropped every policy and every public-role privilege (commits `ea186720`, `a36a095a`, `6f76957a`, `534d852b` and `6010ee99`; `scripts/assert-rls.ts` clauses 4 and 5 hold it). What follows is what the same trace opened that the lockdown did not address.
...
| 4 | Monitor RLS query performance | `supabase/migrations/015_*.sql`, `supabase/migrations/044_*.sql` | Nested subquery RLS policies on `training_exercises`, `nutrition_plan_daily_targets`, `set_logs`, `exercise_logs` and `session_logs` joined through several tables and might have degraded at scale. **Resolved 2026-09-25:** migration 201 dropped every policy; no RLS predicate runs on any query, and every app query is `service_role`'s, which bypasses RLS. | Resolved |
