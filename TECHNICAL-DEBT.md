...
- **The day reader's training read is unpaged.** `getNutritionEventsForDateRange` (`services/nutrition-days-service.ts`) reuses `getEventsForDateRange`, one `select("*")` over `training_events` in range, so a span past PostgREST's ~1000-row cap silently loses the TRAIN badges and burns of its tail days. Only the block facts read a span that long (a whole journey), and their has-days gate and Changed marker do not depend on the training read. Page it when a journey can hold more than ~1000 sessions.
- **`GET /api/clients/[id]/blocks/facts` reads versions, notes and training events twice** — its own reads beside the day reader's. Constant round trips (about nine, up from five), never per row. Collapse the two if it ever shows in traces.
...
- **Two code comments still describe the retired generator as current**: `app/api/clients/[id]/blocks/[blockId]/events/route.ts` (the "order is load-bearing" note — the fill's order is a convenience now that a version's end moves rather than days being written) and `services/weekly-nutrition-service.ts:27` (a precedence ending in "the plan's weekday template", a fallback in `buildNutritionSummary` that no covered date reaches). Comment-only; fix when either file is next touched.
...
### P2 - Deferred
- **No DELETE/edit path for `client_metric_entries`** — the coach-logged wellness entries (mood, energy, sleep, stress, soreness; migration 159 narrowed the table's CHECK to those keys). A mistaken entry can only be replaced by re-logging the same metric and date; it cannot be removed. A physique reading is a row in `client_measurements`, which the Journey's measurement log corrects, removes and restores (`docs/ARCHITECTURE.md` → "client_measurements table", rule 8); a wellness entry has no such path.
