...
- **Non-day-decision `{ data }`-only swallows left after 7.85** (out of that session's tz scope, same silent-failure smell): the pre-RPC plan-ID captures in `services/phase-transition-service.ts` (~L275–294; a swallowed error silently skips event cleanup), the old active/planned plan lookups in `services/library-placement-service.ts` (~L75–88, same effect), and the child-row "logged" check in `services/daily-log-permissions-service.ts` (~L90; a swallowed error reads as never-logged). Harden opportunistically.
