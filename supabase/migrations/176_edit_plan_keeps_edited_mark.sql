-- =============================================================================
-- 176: Edit plan's save keeps the edited mark on a day it leaves as it was.
--
-- A day the editor saves unchanged keeps its edited mark; a day the coach
-- changes in the editor loses it. The mark is training_events.is_modified,
-- the calendar card's pencil.
--
-- Same signature and grants as migration 175; the body differs in one place.
-- A session day in p_days may carry "unchanged": true, set by the only caller,
-- services/plan-edit-service.ts, when the day as saved is the day as laid (its
-- name, focus, notes, duration, surplus and exercises). The caller judges it
-- against the calendar it read, and this function's stale check refuses the
-- save unless that calendar is still the one the editor opened.
--
-- The caller decides the days:
...
--      its scheduled event. A kept event keeps its edited mark when its day is
--      unchanged and loses it otherwise; a new event has none.
...
COMMENT ON FUNCTION public.edit_training_plan_atomic(UUID, UUID, DATE, DATE, TEXT, TEXT, INTEGER, INTEGER, JSONB, JSONB) IS
  'Edit plan''s save: rewrites a placed program from its first editable day in one transaction, refusing when the calendar changed since the editor opened; a day saved unchanged keeps its edited mark. Called only by services/plan-edit-service.ts.';
