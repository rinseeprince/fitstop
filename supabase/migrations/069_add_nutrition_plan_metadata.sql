-- =============================================================================
-- Migration 069: Add nutrition plan metadata columns (coach_notes, goal_source)
-- and update create_nutrition_plan_atomic RPC to accept them.
-- Backward compatible: new params default to NULL.
-- =============================================================================

-- A) Add columns to nutrition_plans
ALTER TABLE nutrition_plans ADD COLUMN coach_notes TEXT;
ALTER TABLE nutrition_plans ADD COLUMN goal_source TEXT;

COMMENT ON COLUMN nutrition_plans.coach_notes IS 'Optional note written by coach at plan creation or regeneration';
COMMENT ON COLUMN nutrition_plans.goal_source IS 'Which goal drove the calorie calculation: "phase" or "client"';

-- B) Update create_nutrition_plan_atomic RPC with two new parameters
